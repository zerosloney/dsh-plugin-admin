...
**npm:** [`dsh-plugin-admin`](https://www.npmjs.com/package/dsh-plugin-admin) · v1.10.0 · MIT
...
The plugin rides the live Cordis Context with zero dsh imports. Public surfaces (`ctx.commands` / `ctx.shell` / `ctx.agents` / `ctx.sessionPersistence` list-stat-open / `ctx.credentials`) are stable; five deeper couplings rely on dsh/cordis internals and are guarded by mount-time fail-loud shape probes or runtime degradation:
