...
- **💾 Storage** (order 32) — switch the storage backend between the shipped `json` and the opt-in `sqlite`: install (`pnpm add` + insert row), swap (`storage-domain.config.backend` rewritten in place, refused unless the package is really on disk and the row is mountable), and a read-only session-format card (`SESSION_FORMAT_VERSION = 3` + the migration chain). Session-log migration (JSONL ↔ SQLite) is explicitly out of scope.
- **🧩 Agent Presets** (order 33) — manage `<dshHome>/.agent-presets/`: list shipped (read-only) and user presets, edit `agent.cordis.yml` + `preset.yml` (atomic writes, shape-validated: every composition entry must carry a `name:`), create/delete user presets, and pick the default — **settings-first** (`settings.default`), falling back to a single-key patch mutation that preserves sibling keys and comments.
- **🪝 Codex Hooks Bridge** (order 34) — one-click install/uninstall of the second stock bridge, `@deepseek-ai/dsh-hooks-codex` (Codex-format hooks, `$DSH_HOME/hooks.codex.json`): pnpm add + the `- insert: hooks-codex` row, legacy bare rows upgraded in place, idempotent re-install, and a lifecycle fully independent of the Claude-Code bridge. The panel reports mounted / installed-not-mounted / not-installed from the host's own `listHooks()` payload.
...
The suite runs twenty-one scripts: bundle load + slot registration + full panel interactions (`self-check`), host contracts (`host-check`), MCP probe cache, update-reminder persistence, subagents host+client, command hooks, project commands/hooks, the todo dock (git pure functions against a real temp repo), webhook triggers (validation matrix + full HTTP handler paths), overlays, the schedule dock + harvested bell, the Loader runtime inventory sub-panel, the workspace / skills / web-search / storage / agent-preset admin pages (patch round-trips against a temp profile, stubbed pnpm), the Codex hooks bridge, and `integration-check` — 51 source-level contract probes against a real dsh checkout.
