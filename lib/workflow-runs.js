...
export function createRunRegistry(deps) {
  ...
  function runPath(runId) { return joinPath(dir, `${runId}.json`) }

  function loadRecord(runId) {
    return readJsonIfExists(runPath(runId))
  }
  ...
  async function stop(runId, reason) {
    ...
    // 等运行落定，journal 写完。
    if (handle.done) await handle.done.catch(() => {})
    return { stopped: true, reason: reason || 'stopped' }
  }
  ...
  async function amend(runId, newScript, opts = {}) {
    ...
    const handle = active.get(runId)
    if (handle) await stop(runId, 'amended')
