...
export function createRunner(deps) {
   ...
   * realm 内的转发闭包注入，脚本摸 facade 成员（agent / shell / …）的
   * `.constructor` 只会得到 realm 自己的 Function，拿不到宿主 realm 的求值能力
   * （否则一个 host 闭包就够逃逸）。
   *
   * 注意边界：这条保护只覆盖 facade 成员本身。**跨 realm 传进来的宿主对象**
   * （`args`，以及 agent() / shell() 返回的宿主值）仍带着宿主原型，脚本用
   * `args.constructor.constructor('return process')()` 一步就能拿回宿主进程——
   * 逐字验证见项目研究文档《plugin-source-research.md》的安全态势一节。
   *
