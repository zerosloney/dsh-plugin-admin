...
export function resolveLedgerCap(raw) {
  return typeof raw === 'number' && Number.isSafeInteger(raw) && raw >= 100 && raw <= USAGE_LEDGER_CAP_MAX
    ? raw
    : USAGE_LEDGER_CAP
}
 ...
 * @param {object} [options] - `{ path, enqueue, now, cap }` (`cap` overrides
...
export function createUsageLedger(options = {}) {
  ...
  return {
    ...
    async record(rows, options = {}) {
      return await persist(mergeUsageLedger(ensure(), rows, now(), { markAbsent: true, protect: options.protect, cap }))
    },
    /**
     ...
     */
    async upsert(rows) {
      return await persist(mergeUsageLedger(ensure(), rows, now(), { markAbsent: false, cap }))
    },
