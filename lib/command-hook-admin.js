...
/**
 * Serialize the bridge mount row this plugin authors. `configPath` is the
 * absolute hooks.json this panel manages — the bridge resolves it relative to
 * the process cwd otherwise, so the absolute form is the only safe default.
...
export function applyCommandHookAdmin(ctx, options = {}) {
   ...
   * shape the official /plan handler steers.
   * @param entry - the managed command entry.
  ...
  /**
   * Find the mounted stock Claude-Code hooks bridge entry, if any.
   * @returns the loader entry, or undefined when the bridge is not composed.
   */
  function findBridgeEntry() {
    ...
    for (const entry of loader.entries()) {
      ...
      if (typeof name === 'string' && name.includes('hooks-claude-code')) return entry
    }
    return undefined
  }
   ...
   * entry point — passing the entry's own unchanged config object gives a
   * pure restart, and `noSave=true` keeps persistence hooks from writing the
   * profile patch file. `Entry.refresh()` only starts entries that are not
   * yet running, so it covers the pending case alone. Never throws — the
   * outcome reaches the panel, which claims "已生效" only when true.
   * @returns { mounted, reloaded, error? }.
   */
  async function reloadBridge() {
    const entry = findBridgeEntry()
    if (entry === undefined) return { mounted: false, reloaded: false }
    try {
      if (entry.fiber !== undefined && entry.fiber !== null) {
        if (typeof entry.fiber.update !== 'function') {
          return { mounted: true, reloaded: false, error: 'dsh 版本缺少 Fiber.update()，无法热重启' }
        }
      ...
      return { mounted: true, reloaded: false, error: 'dsh 版本缺少 Entry.refresh()，无法启动桥' }
    } catch (error) {
   ...
   * - bridgeRowPresent: a bridge mount row exists in the profile patch file.
  ...
  function listHooks() {
    ...
    return {
      ...
      bridgeMounted: entry !== undefined,
      bridgeInstalled: bridgePackageInstalled(),
      bridgeRowPresent: findBridgeRows().length > 0,
  ...
  function bridgePackageInstalled() {
    try {
      ...
      return Object.prototype.hasOwnProperty.call(dependencies, BRIDGE_PACKAGE)
    } catch {
      return false
    }
  }
  ...
  async function bridgeInstall() {
    ...
    return enqueue(async () => {
      ...
      if (!bridgePackageInstalled()) {
        output = await runPnpm(profileDir, ['add', BRIDGE_PACKAGE])
        action = 'installed'
      ...
      if (findBridgeRows().length === 0) {
        ...
        let next
        if (placeholder !== -1) {
          next = [...lines.slice(0, placeholder), ...bridgeRowLines(hooksPath), ...lines.slice(placeholder + 1)].join('\n')
          if (!next.endsWith('\n')) next += '\n'
        } else {
