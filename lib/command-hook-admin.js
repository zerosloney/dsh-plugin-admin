...
/**
 * The second stock hooks bridge — Codex format. Same install/remove
 * shape as `BRIDGE_PACKAGE`, but the underlying hooks.json file is a
 * different one (Codex-format) and the row id carries a distinct name
 * so the two mounts compose side-by-side without collision.
 */
export const CODEX_BRIDGE_PACKAGE = '@deepseek-ai/dsh-hooks-codex'
/** Patch-row id used when this plugin mounts the Codex bridge. */
const CODEX_BRIDGE_ROW_ID = 'hooks-codex'
...
function readCommandFile(commandsDir, name) {
  ...
  // Symmetry with the RPC save path (normalizeCommandInput rejects an empty
  // prompt): a file whose prompt is blank must not join the live registry
  // only to fail at invocation time with the panel still showing it active.
  if (entry.fileError === undefined && entry.prompt.trim() === '') entry.fileError = '提示词为空'
  return entry
}
...
/**
 * Stable, content-derived hook id. The former positional ids
 * (`event/groupIndex/hookIndex`, `disabled/index`) drifted whenever a sibling
 * entry was deleted, so two panels editing concurrently could resolve a stale
 * id to the WRONG entry after one of them deleted; the content hash survives
 * index shifts and self-heals id drift on the next list.
 * @param prefix - id namespace: the event name (active store) or 'disabled'
 *   (sidecar store).
 * @param entry - { event, matcher, command, timeoutSec } whose content is hashed.
 * @returns e.g. `PreToolUse/3fa2b91c` or `disabled/3fa2b91c`.
 */
function hookEntryId(prefix, entry) {
  const hash = createHash('sha256')
    .update(`${entry.event}\0${entry.matcher ?? ''}\0${entry.command}\0${entry.timeoutSec === null || entry.timeoutSec === undefined ? '' : String(entry.timeoutSec)}`, 'utf8')
    .digest('hex')
    .slice(0, 8)
  return `${prefix}/${hash}`
}
...
/**
 * Assign stable ids in place; exact duplicates within one store get a `~n`
 * suffix so ids stay unique (React keys, single-entry deletes) while the
 * first occurrence keeps the bare id.
 * @param entries - flat entries WITHOUT ids yet.
 * @param makeBase - (entry) => the bare content id.
 * @returns the same entries with `id` set.
 */
function assignStableHookIds(entries, makeBase) {
  const seen = new Map()
  for (const entry of entries) {
    const base = makeBase(entry)
    const n = seen.get(base) ?? 0
    seen.set(base, n + 1)
    entry.id = n === 0 ? base : `${base}~${n}`
  }
  return entries
}
 ...
 * @returns { root, entries, fileError? } — the parsed root (or {}) and flat
 *   entries; `fileError` is set instead of throwing when the file exists but
 *   is not valid JSON, so one corrupted file degrades instead of bricking
 *   every hooks RPC.
 */
function readActiveHooks(hooksPath) {
  let root
  try {
    root = readJsonOrNull(hooksPath)
  } catch (error) {
    return { root: {}, entries: [], fileError: `JSON 解析失败：${String(error)}` }
  }
  if (root === null || typeof root !== 'object' || Array.isArray(root)) root = {}
  // The bridge accepts both `{ hooks: {...} }` and the bare event map; we
  // author the bare map (our own file), and read either form. Precedence
  // when a hand-authored file carries BOTH: the wrapper wins — top-level
  // event keys are ignored on read AND dropped on the next save
  // (rebuildActiveHooks preserves only wrapper events plus unknown
  // non-event keys). Keep managed events inside a single form.
  ...
  // Entries carry stable content ids now (the legacy format this replaces
  // was positional: `event/groupIndex/hookIndex`).
  return { root, entries: assignStableHookIds(entries, entry => hookEntryId(entry.event, entry)) }
}

/**
 * Read the DISABLED sidecar into flat entries.
 * @param disabledPath - the sidecar path.
 * @returns { entries, fileError? } — flat disabled entries; `fileError` is
 *   set instead of throwing when the sidecar is not valid JSON.
 */
function readDisabledHooks(disabledPath) {
  let root
  try {
    root = readJsonOrNull(disabledPath)
  } catch (error) {
    return { entries: [], fileError: `JSON 解析失败：${String(error)}` }
  }
  ...
  return {
    entries: assignStableHookIds(entries, entry => hookEntryId('disabled', entry)),
  }
}

/**
 * Refuse hook mutations while either store is unreadable: writes rebuild the
 * file from the parsed tree, so proceeding would silently replace malformed
 * content the admin may still want to recover by hand.
 * @param hooksPath - the active hooks.json path.
 * @param disabledPath - the disabled sidecar path.
 */
function assertHooksStoresWritable(hooksPath, disabledPath) {
  const active = readActiveHooks(hooksPath)
  if (active.fileError !== undefined) {
    throw new Error(`hooks.json 解析失败，已拒绝修改以保护文件内容（${active.fileError}）。请手工修复或删除该文件后重试：${hooksPath}`)
  }
  const disabled = readDisabledHooks(disabledPath)
  if (disabled.fileError !== undefined) {
    throw new Error(`hooks.disabled.json 解析失败，已拒绝修改以保护文件内容（${disabled.fileError}）。请手工修复或删除该文件后重试：${disabledPath}`)
  }
}
...
/**
 * Validate a user-supplied matcher the way the bridge would: `*` is the
 * match-all sentinel; a literal alternation of identifiers (`read|edit`) —
 * the same `[A-Za-z0-9_|]` set the bridge's CLAUDE_LITERAL uses, so a matcher
 * carrying `-`/`:` is NOT a literal list but a non-anchored regex — passes as
 * a plain identifier list; anything else must compile as a JavaScript RegExp.
 * An invalid regex would make the stock bridge reject its whole config at
 * load — refuse it here instead.
...
function assertMatcher(event, matcher) {
  ...
  if (matcher === '*') return
  if (/^[A-Za-z0-9_|]+$/.test(matcher)) return
  try {
    new RegExp(matcher)
  } catch (error) {
...
/**
 * Serialize the Codex bridge mount row this plugin authors. Same `- insert:`
 * wrapper as `bridgeRowLines` — a bare top-level `- id:` would be a
 * non-targeted override the loader drops because no base bundle defines
 * `hooks-codex`. `configPath` points at the Codex-format hooks file
 * (`<dshHome>/hooks.codex.json` by default).
 * @param configPath - absolute Codex hooks file path.
 * @returns YAML lines.
 */
function codexBridgeRowLines(configPath) {
  return [
    '- insert:',
    `    - id: ${CODEX_BRIDGE_ROW_ID}`,
...
/**
 * Whether a top-level block mounts the Codex hooks bridge.
 * @param lines - patch file lines.
 * @param block - block span.
 */
function isCodexBridgeBlock(lines, block) {
  for (let i = block.index; i < block.endIndex; i++) {
    if (/^\s*name:\s*['"]@deepseek-ai\/dsh-hooks-codex['"]\s*$/.test(lines[i])) return true
  }
  return false
}
...
/**
 * The `configPath` scalar a bridge block declares, or null when the block
 * carries none (or the value parses to something other than a non-empty
 * string). The bridge reads ONLY this key at apply time, so the panel must
 * compare the declared path against the hooks file it edits — a row pointing
 * elsewhere means every panel save "reloads" a bridge that is still reading
 * the other file.
 * @param lines - patch file lines.
 * @param block - block span.
 * @returns the declared path, or null.
 */
function bridgeConfigPathOf(lines, block) {
  if (block === undefined || block === null) return null
  for (let i = block.index; i < block.endIndex; i++) {
    const match = /^\s*configPath:\s*(.+?)\s*$/.exec(lines[i] ?? '')
...
/**
 * Whether two paths denote the same file: separators unified, trailing
 * slashes dropped, and — on win32 — compared case-insensitively. Purely
 * lexical on purpose: the panel compares the patch's declared path against
 * the file it manages, and neither is required to exist yet.
 * @param a - one path (may be null).
 * @param b - the other path (may be null).
 */
...
/**
 * Rewrite a bridge block's `configPath` to the given path: an existing
 * `configPath:` line keeps its indentation and gains the new
 * JSON-stringified value; a block without one gets the line inserted after
 * its `config:` line (or after the `name:` line when no config block
 * exists). Every other line survives verbatim.
 * @param lines - patch file lines.
 * @param block - the bridge block span.
 * @param configPath - the absolute hooks file this panel manages.
 * @returns the next lines.
 */
function rewriteBridgeConfigPath(lines, block, configPath) {
  const blockLines = lines.slice(block.index, block.endIndex)
  let replaced = false
  for (let i = 0; i < blockLines.length; i++) {
    const match = /^(\s*)configPath:\s*(.+?)\s*$/.exec(blockLines[i] ?? '')
    if (match === null) continue
    blockLines[i] = `${match[1]}configPath: ${JSON.stringify(configPath)}`
    replaced = true
    break
  }
  if (!replaced) {
    let anchor = -1
    for (let i = 0; i < blockLines.length; i++) {
      if (/^\s*config:\s*$/.test(blockLines[i] ?? '')) { anchor = i; break }
    }
    if (anchor === -1) {
      for (let i = 0; i < blockLines.length; i++) {
        if (/^\s*name:\s*\S/.test(blockLines[i] ?? '')) { anchor = i; break }
      }
    }
    const indentMatch = anchor === -1 ? null : /^(\s*)/.exec(blockLines[anchor])
    // The entry's key indent: the id line's indent plus one level (2 spaces,
    // the shape bridgeRowLines authors).
    const idIndent = /^(\s*)- id:/.exec(blockLines[0] ?? '')
    const keyIndent = (idIndent !== null ? idIndent[1] : indentMatch !== null ? indentMatch[1] : '') + '  '
    const insertAt = anchor === -1 ? blockLines.length : anchor + 1
    blockLines.splice(insertAt, 0, `${keyIndent}configPath: ${JSON.stringify(configPath)}`)
  }
  return [...lines.slice(0, block.index), ...blockLines, ...lines.slice(block.endIndex)]
}
...
// profileDependencyInstalled, manifestDependencySpec, hostPeerLinkSpec,
// writePnpmOverride, harnessLockstepVersion, and ensureProfileDependency are
// imported from patch-utils.js. They are re-exported here for backward
...
export function applyCommandHookAdmin(ctx, options = {}) {
  ...
  // Codex-format hooks file: distinct path so the two bridges' hooks.json
  // copies don't collide. The panel does not yet offer a Codex-format
  // editor surface; the file is reserved for users who hand-author Codex
  // hooks and want the bridge to consume them. v1 bridge scope: install
  // + remove only.
  const codexHooksPath = typeof settings.codexHooksPath === 'string' && settings.codexHooksPath.trim() !== ''
    ? settings.codexHooksPath
    : join(dshHome(), 'hooks.codex.json')
    ...
    for (const name of commandFileNames(commandsDir)) {
      ...
      try {
        commandDisposers.push(ctx.commands.register({
          ...
          // The core field is `attachments` (CommandInputDescriptor); the
          // store's `images` key stays the plugin-internal file format.
          ...(entry.inputHint !== null || entry.images
            ? { input: { hint: entry.inputHint ?? INPUT_HINT_FALLBACK, ...(entry.images ? { attachments: true } : {}) } }
            : {}),
  ...
  let watchDisposer = () => {}
  try {
    const watcher = watch(canonicalWatchPath(commandsDir), { persistent: false })
   ...
   * boot-private Cordis plugin registry (`ctx.registry` — `ctx.get('loader')`
   * is the MODULE loader in this dsh build and carries no plugin entries), and
  ...
  function findBridgeFibers() {
    ...
    try {
      for (const [, runtime] of registry.entries()) {
        // Exact-name whitelist: the stock bridge declares `name = 'hooks-claude-code'`;
        // the package-name form is tolerated in case a build registers the
        // full id. A substring match would also catch unrelated plugins
        // whose names merely contain the fragment.
        const name = String(runtime?.name ?? '')
        if (name !== 'hooks-claude-code' && name !== BRIDGE_PACKAGE) continue
  ...
  /**
   * Enumerate every fiber of the stock Codex-format hooks bridge — the
   * `findBridgeFibers` twin. The plugin declares `name = 'hooks-codex'` (the
   * row id); the package-name form is tolerated for builds that register the
   * full id. A distinct whitelist on purpose: a substring match would also
   * catch the Claude bridge and report it as the Codex one.
   * @returns the Codex bridge fibers carrying an update().
   */
  function findCodexBridgeFibers() {
    const registry = ctx.registry
    if (registry === undefined || typeof registry.entries !== 'function') return []
    const fibers = []
    try {
      for (const [, runtime] of registry.entries()) {
        const name = String(runtime?.name ?? '')
        if (name !== CODEX_BRIDGE_ROW_ID && name !== CODEX_BRIDGE_PACKAGE) continue
        for (const fiber of runtime?.fibers ?? []) {
          if (fiber !== undefined && fiber !== null && typeof fiber.update === 'function') fibers.push(fiber)
        }
      }
  ...
  async function reloadBridge() {
    ...
    try {
      ...
      // A row whose configPath points elsewhere reloads a bridge that keeps
      // reading the OTHER file: say so instead of letting the panel's green
      // "已生效" stand unrebutted.
      const { lines: patchLines } = readPatchLines(profileDir)
      const compliantRow = findBridgeRows().find(block => isInsertShapedBlock(patchLines, block))
      const declared = compliantRow === undefined ? null : bridgeConfigPathOf(patchLines, compliantRow)
      if (compliantRow !== undefined && !samePath(declared, hooksPath)) {
        ctx.logger?.warn?.(`command-hook-admin: 桥的 configPath（${declared ?? '缺省'}）与面板管理的 hooks 文件（${hooksPath}）不一致——本次保存不会生效，请在面板点「安装/修复桥」`)
      }
   ...
   * - the `codex*` twins carry the same three facts for the Codex bridge, whose
   *   row id / package / hooks file are all distinct from the Claude bridge's.
   ...
   */
  function listHooks() {
    const activeRead = readActiveHooks(hooksPath)
    const disabledRead = readDisabledHooks(disabledPath)
    const { lines } = readPatchLines(profileDir)
    const compliantBridgeRow = findBridgeRows().find(block => isInsertShapedBlock(lines, block))
    const bridgeRowPresent = compliantBridgeRow !== undefined
    const codexBridgeRowPresent = findCodexBridgeRows().some(block => isInsertShapedBlock(lines, block))
    // The bridge reads only the row's own configPath, so a row pointing at a
    // different file makes every panel save a no-op that still reports green.
    // `configPath` is required by the bridge's schema: a row without one is
    // reported as a mismatch (null path), never as "fine".
    const bridgeConfigPath = compliantBridgeRow === undefined ? null : bridgeConfigPathOf(lines, compliantBridgeRow)
    const configPathMatches = !bridgeRowPresent
      ? true
      : bridgeConfigPath !== null && samePath(bridgeConfigPath, hooksPath)
    const fileErrors = [
      activeRead.fileError !== undefined ? `hooks.json：${activeRead.fileError}` : null,
      disabledRead.fileError !== undefined ? `hooks.disabled.json：${disabledRead.fileError}` : null,
    ].filter(Boolean)
    return {
      hooksPath,
      ...
      bridgeConfigPath,
      configPathMatches,
      // The Codex bridge reported through its own fields: the panel used to
      // read `codexBridgeInstalled` off this payload, which never carried it,
      // so the card fell back to "未安装" (and could never uninstall).
      codexHooksPath,
      codexBridgePackage: CODEX_BRIDGE_PACKAGE,
      codexBridgeMounted: findCodexBridgeFibers().length > 0,
      codexBridgeInstalled: codexBridgePackageInstalled(),
      codexBridgeRowPresent,
      ...(fileErrors.length > 0 ? { fileError: fileErrors.join('；') } : {}),
      hooks: [...activeRead.entries, ...disabledRead.entries],
    }
  }
  ...
  function locateHook(id) {
    ...
    const { entries: disabled } = readDisabledHooks(disabledPath)
   ...
   * entry between them when it changed. The read-modify-write runs on the
   * shared serial queue — the same one guarding bridgeInstall/bridgeRemove —
   * so two concurrent RPCs cannot interleave their reads and silently drop
   * one update.
  ...
  async function saveHook(raw) {
    ...
    await enqueue(() => {
      assertHooksStoresWritable(hooksPath, disabledPath)
  ...
  /**
   * Remove one hook from whichever store holds it. Rides the shared serial
   * queue like saveHook (see above).
  ...
  async function deleteHook(id) {
    await enqueue(() => {
      ...
      if (existing.enabled) {
        ...
        writeDisabledHooks(readDisabledHooks(disabledPath).entries.filter(entry => entry.id !== id))
      }
    })
    const reload = await reloadBridge()
    return { ...listHooks(), reload }
  ...
  async function saveCommand(raw) {
    ...
    if (typeof raw?.originalName === 'string' && raw.originalName !== entry.name) {
      // Trust-boundary guard on the rename path: the originalName travels
      // straight into a filesystem rm, so a value outside the registry's
      // name grammar (e.g. "../../../some/other.json") would silently reach
      // outside the command directory. Reuse the same grammar deleteCommand
      // applies to `name`.
      if (!COMMAND_NAME.test(raw.originalName)) {
        throw new Error(`原命令名称不合法：${raw.originalName}`)
      }
      oldPath = join(commandsDir, `${raw.originalName}.json`)
      if (!existsSync(oldPath)) throw new Error(`原命令文件不存在：${raw.originalName}.json`)
      if (existsSync(path)) throw new Error(`命令 ${entry.name} 已存在`)
    }
    // Write the new file BEFORE removing the old one: a failed write then
    // leaves the original command intact instead of losing both.
    ...
    if (oldPath !== null) rmSync(oldPath)
    reRegisterCommands()
    return listCommands()
  }
  ...
  /**
   * Whether the Codex bridge package is a dependency of the profile.
   * @returns true when present in the profile manifest dependencies.
   */
  function codexBridgePackageInstalled() {
    return profileDependencyInstalled(profileDir, CODEX_BRIDGE_PACKAGE)
  }
  ...
  /**
   * Find the profile patch blocks that mount the Codex bridge.
   * @returns array of { index, endIndex } block spans.
   */
  function findCodexBridgeRows() {
    const { lines } = readPatchLines(profileDir)
    return topLevelBlocks(lines).filter(block => isCodexBridgeBlock(lines, block))
  }
  ...
  async function bridgeInstall() {
    ...
    return enqueue(async () => {
      ...
      const { lines: currentLines, patchPath: currentPatchPath } = readPatchLines(profileDir)
      const compliantRow = bridgeBlocks.find(block => isInsertShapedBlock(currentLines, block))
      const hasCompliantRow = compliantRow !== undefined
      if (hasCompliantRow) {
        // A compliant row that points at ANOTHER file (hand-mounted before
        // this panel existed) silently no-ops every save the panel makes.
        // Install is the user's explicit "mount it for THIS panel" gesture,
        // so it repairs the path in place — the row keeps its shape and
        // every other line.
        const declared = bridgeConfigPathOf(currentLines, compliantRow)
        if (!samePath(declared, hooksPath)) {
          writePatch(currentPatchPath, rewriteBridgeConfigPath(currentLines, compliantRow, hooksPath))
          row = 'config-path-repaired'
        }
      }
      if (!hasCompliantRow) {
        ...
        // appended after (same trap the mcpAdmin upsert documents; see
        // appendTopLevelBlocks).
        writePatch(patchPath, appendTopLevelBlocks(kept, [bridgeRowLines(hooksPath)]))
  ...
  /* ----------------------- codex bridge package lifecycle ----------------- */

  /**
   * Install the Codex-format hooks bridge into the profile: `pnpm add` the
   * package when it is not a dependency yet, then author the mount row in
   * `cordis.patch.yml` pointing `configPath` at `<dshHome>/hooks.codex.json`.
   * Mirror of `bridgeInstall` (Claude Code-format) — same `- insert:`
   * wrapper shape, same id-targeted drop of legacy bare rows, same shared
   * serial queue.
   *
   * @returns { ok, action, row?, output?, codexHooksPath }.
   */
  async function codexBridgeInstall() {
    if (runPnpm === null) throw new Error('command-hook-admin: codexBridgeInstall requires the host pnpm runner')
    return enqueue(async () => {
      const dep = await ensureProfileDependency(profileDir, CODEX_BRIDGE_PACKAGE, runPnpm, reconcileBundles, harnessLockstepVersion(profileDir))
      const action = dep.state === 'installed' ? 'installed' : 'already-installed'
      const output = dep.output
      const bridgeBlocks = findCodexBridgeRows()
      ...
      if (!hasCompliantRow) {
        ...
        const drop = new Set()
        for (const block of topLevelBlocks(lines)) {
          if (isCodexBridgeBlock(lines, block)) {
            for (let i = block.index; i < block.endIndex; i++) drop.add(i)
          }
        }
        const kept = lines.filter((_, i) => !drop.has(i))
        writePatch(patchPath, appendTopLevelBlocks(kept, [codexBridgeRowLines(codexHooksPath)]))
        row = bridgeBlocks.length > 0 ? 'upgraded' : 'inserted'
      }
      return { ok: true, action, row, codexHooksPath, ...(output !== '' ? { output } : {}) }
    })
  }

  /**
   * Uninstall the Codex-format hooks bridge: drop every Codex bridge mount
   * row from the profile patch file, then `pnpm remove` the package when
   * it is a dependency. Mirror of `bridgeRemove` (Claude Code-format).
   *
   * @returns { ok, action, rowsRemoved, output? }.
   */
  async function codexBridgeRemove() {
    if (runPnpm === null) throw new Error('command-hook-admin: codexBridgeRemove requires the host pnpm runner')
    return enqueue(async () => {
      const rows = findCodexBridgeRows()
      let rowsRemoved = 0
      if (rows.length > 0) {
        const { lines, patchPath } = readPatchLines(profileDir)
        const drop = new Set()
      ...
      let output = ''
      let action = 'row-removed'
      if (codexBridgePackageInstalled()) {
        output = await runPnpm(profileDir, ['remove', CODEX_BRIDGE_PACKAGE])
        action = 'uninstalled'
        if (reconcileBundles !== null) reconcileBundles()
      }
      return { ok: true, action, rowsRemoved, ...(output !== '' ? { output } : {}) }
    })
  }
  ...
  ctx.effect(() => { ctx.provide(SERVICE_KEY, service) }, 'plugin-admin/commandHooks: provide')
...
export function commandHookInvocations() {
  ...
  return [
    ...
    descriptor('hooks/codexBridgeInstall', 'codexBridgeInstall', []),
    descriptor('hooks/codexBridgeRemove', 'codexBridgeRemove', []),
  ]
}
