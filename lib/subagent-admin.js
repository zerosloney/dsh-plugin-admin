...
import { PROFILE_PATCH_FILENAME, makeSerialQueue, profileDirOf, topLevelBlocks, writePatch, yamlScalar } from './patch-utils.js'
...
/** Hard cap on one panel-authored follow-up message (guards pathological RPC payloads). */
const RUNTIME_PROMPT_MAX_CHARS = 32_000
...
/** Largest delay Node timers represent without clamping（对齐 dsh-timeout 的
 * MAX_TIMER_DELAY_MS：超过它 setTimeout 立即触发，宽限期形同虚设）。 */
...
/** Delegation-result cap for the generic CLI stdout collector (tail-keep on overflow). */
const CLI_STDOUT_COLLECT_MAX_BYTES = 2 * 1024 * 1024
/** Diagnostic-tail cap for the generic CLI stderr collector. */
const CLI_STDERR_COLLECT_MAX_BYTES = 64 * 1024
...
const CONFIG_KEYS = ['provider', 'toolName', 'modelSelectionSettings', 'enableRunInBackground', 'backgroundMode', 'agentOptions', 'persona', 'toolFilter', 'maxDepth']
...
export function serializeEntryLines(entry, indent = 4) {
  ...
  if (config.modelSelectionSettings === true) {
    out.push(`${pad}    modelSelectionSettings: true`)
  }
...
// topLevelBlocks is imported from patch-utils.js
...
export function validateCliConfig(backend, config) {
  ...
  if (config.disposeGraceMs !== undefined
    && (typeof config.disposeGraceMs !== 'number' || !Number.isFinite(config.disposeGraceMs)
      || config.disposeGraceMs <= 0 || config.disposeGraceMs > MAX_TIMER_DELAY_MS)) {
    throw new Error(`disposeGraceMs 必须是正数且不超过 ${MAX_TIMER_DELAY_MS} 毫秒（Node 定时器上限，对齐宿主 assertPositiveFinite 契约；当前：${JSON.stringify(config.disposeGraceMs)}）`)
  }
...
export function validateGenericCliBackend(backend, taken) {
  ...
  if (backend.disposeGraceMs !== undefined
    && (typeof backend.disposeGraceMs !== 'number' || !Number.isFinite(backend.disposeGraceMs)
      || backend.disposeGraceMs <= 0 || backend.disposeGraceMs > MAX_TIMER_DELAY_MS)) {
    throw new Error(`disposeGraceMs 必须是正数且不超过 ${MAX_TIMER_DELAY_MS} 毫秒（Node 定时器上限；当前：${JSON.stringify(backend.disposeGraceMs)}）`)
  }
  if (backend.cwd !== undefined && (typeof backend.cwd !== 'string' || backend.cwd.trim() === '' || !isAbsolute(backend.cwd))) {
    throw new Error(`cwd 必须是非空绝对路径：${JSON.stringify(backend.cwd ?? null)}`)
  }
 ...
 * text in/out, no persona/toolFilter/depth/outputSchema/agentOptions support.
      ...
      const result = new Promise((resolve) => {
        try {
          handle = subprocess.spawn({
            ...
            // SubprocessOutputMode's collect disposition is a bounded
            // { maxBytes } object — a bare 'collect' string duck-types through
            // spawn's isCollect check into OutputCollector(undefined), whose
            // cap comparisons are all NaN-false: the stream then never trims.
            stdio: {
              stdin: 'ignore',
              stdout: { maxBytes: CLI_STDOUT_COLLECT_MAX_BYTES },
              stderr: { maxBytes: CLI_STDERR_COLLECT_MAX_BYTES },
            },
            graceMs,
...
export function validateEntryInput(entry, env) {
  ...
  // The switch makes dsh's tool-subagent require the `subagentModelSelection`
  // service in the Host scope at load time (only the web-app bundle mounts
  // it): without this gate the row saves fine and the NEXT boot dies in the
  // package's own apply. Same live-capability posture as the provider /
  // toolFilter checks below.
  if (config.modelSelectionSettings === true && env.modelSelectionAvailable !== true) {
    throw new Error('modelSelectionSettings 需要 Host 侧挂载 model-selection-settings 服务（web-app bundle 默认提供）；当前部署缺失，请取消勾选或安装对应 bundle')
  }
  ...
  // 镜像 dsh tool-subagent 的同款门（modelSelectionSettings 要求 agentOptions
  // 能力）：选择 UI 落盘的就是 agentOptions 形状的 provider/model 路由，拒绝
  // agentOptions 的后端同样兜不住它。
  if (config.modelSelectionSettings === true && provider.capabilities.agentOptions === false) {
    throw new Error(`执行后端 "${providerName}" 不支持 agentOptions，而 modelSelectionSettings（模型选择 UI）依赖它；请换后端或取消勾选`)
  }
  ...
  if (config.toolFilter !== undefined) {
    ...
    // 两侧皆空的名单不构成约束：dsh 载入层只拒 allow/deny 双缺，allow:[] 这类
    // 空数组会被放行到运行时。这里更严一层，省下一次注定落空的委托。
    if ((allow ?? []).length === 0 && (deny ?? []).length === 0) {
      throw new Error('toolFilter 已配置但 allow/deny 都没有列出任何工具——空名单等于没有约束；请填写名单，或直接去掉工具约束')
    }
  ...
  if (config.agentOptions !== undefined) {
    ...
    // 与 persona/toolFilter/maxDepth 同款能力门：镜像 dsh tool-subagent 的
    // "provider does not support child agentOptions" 拒绝。
    if (provider.capabilities.agentOptions === false) {
      throw new Error(`执行后端 "${providerName}" 不支持 agentOptions（provider/model/maxTokens 覆盖）；请去掉模型指定或换后端`)
    }
  }
...
/**
 * One live session's events via the Session's synchronous snapshot reader.
 * Session exposes no public `.events` property (that field has never existed
 * on the class), and the reader itself is dsh-deprecated (note
 * 2026-09-09-deprecate-synchronous-session-event-reads) — it stays the only
 * way to see a RUNNING child's freshest tail here: its `subagent/descriptor`
 * is appended at creation, and the persisted replay lags the backend flush.
 * Read-only and failure-tolerant: any reader drift degrades to [].
...
// profileDirOf is imported from patch-utils.js
...
export function applySubagentAdmin(ctx, enqueueShared) {
  ...
  /** Atomic write for this module's own files (journal history, CLI config):
   * temp file + rename, so a crash never truncates them. The profile patch
   * writes go through patch-utils writePatch (shared rolling backup). */
  ...
  const ensureGenericProvider = (backend, { throwWhenUnavailable = false } = {}) => {
    // Remount even when the id is already registered: cliUpsertGeneric edits
    // the stored row in place (command/args/env/grace), and the live provider
    // must track the row — an early return here left the first mount's stale
    // command running until the next host boot. Drop is a no-op when unmounted.
    dropGenericProvider(backend.id)
    if (!subprocessReady) {
  ...
  const cliUpsertGeneric = async (body) => {
    ...
    // The read → checks → write → provider-registration sequence rides ONE
    // queued operation. The checks used to run outside the queue, so two
    // concurrent upserts both passed against the same pre-write snapshot and
    // wrote two rows with one providerName; the next boot's reconcile then
    // threw on the duplicate and silently dropped every backend after it.
    await enqueue(async () => {
      const existing = readGenericCliBackends()
      const previous = existing.find(item => item.id === id)
      // The same executable name from a different directory derives the same
      // id: without an explicit backendId the second upsert used to replace
      // the first (and its stored references) in silence.
      if (previous !== undefined && providedId === undefined && previous.command !== effective.command) {
        throw new Error(`通用 CLI 后端 "${id}" 已指向 ${previous.command}；同一可执行名会派生出相同 id，请显式指定 backendId 以共存`)
      }
  ...
  const cliRemoveGeneric = async (backendId) => {
    await enqueue(() => {
      ...
      // Registration removal rides the same queued op as the file write, so
      // an interleaved upsert can never observe the row gone while the
      // provider is still registered (or the reverse).
  ...
  const service = {
    ...
    async runtimeList() {
      ...
      try {
        ...
        return { agents: [], error: 'agents 注册表不可用：' + String(error) }
      }
      if (!agents || typeof agents.list !== 'function') {
        return { agents: [], error: 'agents 注册表未挂载（运行中页签无法感知子智能体）' }
      }
      const running = []
      for (const agent of agents.list()) {
        const header = agent?.session?.header
        if (agent?.status !== 'running' || header?.origin !== 'subagent' || typeof header.parentSession !== 'string') continue
        const events = sessionEventsSnapshot(agent.session)
        ...
        running.push({
          ...
          // Activity signal for the run panel: how much the child has actually
          // done (log volume), not just that it exists.
    ...
    async runtimeInterrupt(childId, parentSessionId) {
      ...
      const descriptor = [...sessionEventsSnapshot(agent.session)]
    ...
    /**
     * Deliver one human-authored follow-up message to a live continuable
     * child, riding the subagent runtime's browser prompt remote
     * (`ctx.subagents.prompt` — the same face the official client uses from
     * the child session composer, addressed here from the parent's admin
     * panel instead). The exact live parent authorizes the delivery; queue
     * mode targets the child's next turn, steer mode the nearest step.
     * @param payload - { childId, parentSessionId, text, delivery?,
     *   clientTimeZone? } untrusted RPC payload.
     * @returns { ok, messageId, delivery } with the accepted inbox identity.
     */
    async runtimePrompt(payload) {
      ...
      const clientTimeZone = typeof body.clientTimeZone === 'string' && body.clientTimeZone.trim() !== '' ? body.clientTimeZone.trim() : undefined
      if (childId === '' || parentSessionId === '') {
        throw new Error('缺少运行中子智能体的会话标识')
      }
      if (text === '') {
        throw new Error('续接消息不能为空')
      }
      if (text.length > RUNTIME_PROMPT_MAX_CHARS) {
        throw new Error(`续接消息过长（${String(text.length)} > ${String(RUNTIME_PROMPT_MAX_CHARS)} 字符）`)
      }
      const agents = ctx.get?.('agents') ?? ctx.agents
      const agent = agents?.get?.(childId)
      const header = agent?.session?.header
      if (agent?.status !== 'running' || header?.origin !== 'subagent' || header.parentSession !== parentSessionId) {
        throw new Error('该子智能体已结束，或不属于指定父会话；请刷新列表')
      }
      const descriptor = [...sessionEventsSnapshot(agent.session)]
        .reverse().find(event => event?.type === 'subagent/descriptor')?.data
      if (descriptor?.mode !== 'continuable') {
        throw new Error('该一次性子智能体不支持续接')
      }
      if (!ctx.subagents || typeof ctx.subagents.prompt !== 'function') {
        throw new Error('当前 dsh 的 subagents 服务不支持 prompt（需 dsh ≥ 0.1.5-rc.2）')
      }
      // Same payload contract as the browser control surface (subagent.prompt):
      // client-minted request identity, continuable mode discriminator, text
      // content parts (images are the official composer's path, not the panel's).
      const request = {
      ...
      // Caller cancellation cannot cross the panel hop: the call owns its own
      // never-aborted signal and the runtime's admission is awaited as-is.
      const receipt = await ctx.subagents.prompt(request, new AbortController().signal)
      return { ok: true, messageId: typeof receipt?.messageId === 'string' ? receipt.messageId : null, delivery }
    },
    ...
    async upsert(entry) {
      ...
      const warnings = enqueue(() => {
        ...
        const validationWarnings = validateEntryInput(payload, {
          ...
          existing,
          allIds,
          modelSelectionAvailable: ctx.get('subagentModelSelection') !== undefined,
        })
    ...
    async cliUpsert(payload) {
      ...
      await enqueue(async () => {
        ...
        // providerName uniqueness — the generic path had this guard, the
        // builtin path did not: a name another mounted backend (or a base
        // bundle provider) already owns saved fine here and then made the
        // next dsh boot die in registerProvider's DUPLICATE_PROVIDER.
        const effectiveProviderName = effective.providerName
        const otherBuiltinNames = [...mountedRows.entries()]
          .filter(([rowId]) => rowId !== backend.id)
          .map(([, rowConfig]) => rowConfig.providerName)
          .filter(candidate => typeof candidate === 'string' && candidate !== '')
        const otherGenericNames = readGenericCliBackends().map(item => item.providerName)
        if ([...otherBuiltinNames, ...otherGenericNames].includes(effectiveProviderName)) {
          throw new Error(`providerName "${effectiveProviderName}" 已被其他后端占用；请换一个名字`)
        }
        // Base-bundle provider names are never a backend's to take (the row's
        // own default name stays legal — the codex/claude-code rows own theirs).
        const baseReservedNames = ['spawn', 'fork', 'subagent', 'subagent_fork', 'run_code']
        if (baseReservedNames.includes(effectiveProviderName)
          && effectiveProviderName !== backend.defaultConfig.providerName) {
          throw new Error(`providerName "${effectiveProviderName}" 是 dsh 基础 bundle 的保留名；请换一个名字`)
        }
        let liveNames = []
        try { liveNames = ctx.subagents.list() } catch { /* leave empty */ }
        const claimedByThisMount = previous !== undefined && previous.providerName === effectiveProviderName
        if (!claimedByThisMount && liveNames.includes(effectiveProviderName)) {
          throw new Error(`providerName "${effectiveProviderName}" 已在运行中注册，请换一个名字`)
        }
    ...
    // Serialized on the shared queue: two concurrent installs used to run
    // their `npm install -g` steps on top of each other.
    await enqueue(async () => {
  ...
  ctx.effect(() => { ctx.provide(SERVICE_KEY, service) }, 'plugin-admin/subagents: provide')
...
export function subagentInvocations() {
  return [
    ...
    {
      id: `${DESCRIPTOR_PACKAGE}/subagent/runtimePrompt`,
      service: SERVICE_KEY,
      namespace: NAMESPACE,
      method: 'runtimePrompt',
      invocation: { kind: 'direct' },
