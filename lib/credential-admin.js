 ...
 * @param {string[]|(() => string[])} declaredRefs - credential refs the
 ...
 *   namespaces after this mount still surface.
...
export function applyCredentialAdmin(ctx, declaredRefs = []) {
  ...
  async function discoverRefs() {
    ...
    const declared = typeof declaredRefs === 'function' ? declaredRefs() : declaredRefs
    const set = new Set(Array.isArray(declared) ? declared : [])
  ...
  const service = {
    ...
    async list() {
      ...
      // Resolve the declared set fresh so rows can distinguish refs the
      // running composition actually consumes from record keys surfaced by
      // listRecords (the former get `declared: true`).
      const declaredList = typeof declaredRefs === 'function' ? declaredRefs() : declaredRefs
      const declaredSet = new Set(Array.isArray(declaredList) ? declaredList : [])
      const rows = []
      for (const ref of refs) {
        try {
          const info = await svc.describe(ref)
          // `declared` overrides the row spread: credentialRow's extra-based
          // path relies on describe() carrying the ref back, which the host
          // does not guarantee.
          rows.push({ ref, ...credentialRow(info), declared: declaredSet.has(ref) })
        } catch {
          rows.push({ ref, configured: false, source: null, writable: false, declared: declaredSet.has(ref), error: 'describe 失败' })
        }
      }
