...
/**
 * Reference default per-hook timeout: 10 minutes, DELIBERATELY the same
 * value as dsh's own hook protocol (`DEFAULT_HOOK_TIMEOUT_MS` in
 * @deepseek-ai/dsh-hook-protocol runner.ts) — the Claude-Code-format
 * hooks.json this module executes is the same content the stock bridge
 * runs, so both must time hooks alike. Per-hook `timeout` in the project
 * config overrides it. Consequence, by design: a config with several hung
 * hooks can hold a turn for minutes (hooks run serially; the turn's abort
 * signal cancels the chain) — that is the stock semantic, not a bug.
 */
...
/**
 * Read cap for a repo-controlled hooks config (`.agents/hooks.json` or the
 * hooks key of `.agents/settings.json`). The parse is synchronous and runs at
 * session start and on every mtime change, so a multi-megabyte file from a
 * cloned repository must not block the host loop.
 */
  ...
  if (exitCode === 0 && trimmedOut.startsWith('{')) {
    ...
    if (parsed !== undefined) {
      ...
      if (hso !== undefined) {
        ...
        // The discriminator is REQUIRED to apply event-scoped fields: dsh's
        // codec drops the block when the name is missing or different, so a
        // malformed payload must not decide anything here either (the same
        // hooks content otherwise yielded different outcomes in the two
        // bridges).
        if (claimed !== eventName) return output
...
function parseHooksConfig(root, projectDir) {
  ...
  // Events the project bridge does not consume (dsh's own bridge runs
  // SubagentStart/SubagentStop too) are reported instead of silently
  // dropped: a config that only configures those would otherwise look
  // like it applied.
  for (const event of Object.keys(hooksMap)) {
    if (HOOK_EVENTS.includes(event)) continue
    if (!/^[A-Z][A-Za-z]*$/.test(event)) continue
    skipped.push({ event, type: 'unsupported-event' })
  }
...
function resolveProjectHooks(projectRoot, cache, warn) {
  ...
  // A repo-controlled config is untrusted input: a multi-megabyte file would
  // block the host loop on the synchronous read + JSON.parse + sha256 at
  // session start (and again on every mtime change). Refuse it once per
  // mtime — the failure is cached like a parse error — instead of paying
  // that cost on every trigger.
  if (sizeBytes > PROJECT_CONFIG_MAX_BYTES) {
    warn(`project-hooks: ${activePath} is ${sizeBytes} bytes (limit ${PROJECT_CONFIG_MAX_BYTES}) — refusing to parse it; project hooks disabled`)
    cache.set(projectRoot, { path: activePath, mtimeMs, groupsByEvent: {}, fingerprint: null })
    return { groupsByEvent: {}, fingerprint: null }
  }
  let groupsByEvent = {}
  ...
  try {
    ...
    for (const row of parsed.skipped) {
      warn(row.type === 'unsupported-event'
        ? `project-hooks: event ${row.event} is not supported by the project bridge in ${activePath} — its hooks are ignored`
        : `project-hooks: skipping unsupported "${row.type}" hook on ${row.event} in ${activePath} (only command hooks run)`)
    }
...
/**
 * Resolve one session's persisted log path for the hook payload's
 * `transcript_path` field. The abstract `sessionPersistence` service does not
 * expose a path API; today only the JSONL backend carries a runtime `locate`
 * method (declared `private` on its implementation). The host may also use a
 * different backend or rename the private seam later, so the helper degrades
 * to '' instead of throwing inside a waterfall listener.
 * @param ctx - plugin context.
 * @param header - the session header to locate.
 * @returns the absolute log path or '' when unavailable.
 */
function transcriptPathFor(ctx, header) {
  try {
    const service = ctx.get('sessionPersistence')
    if (service === undefined || service === null) return ''
    if (typeof service.locate !== 'function') return ''
    return service.locate(header)?.path ?? ''
  } catch {
    return ''
  }
}

/** A plugin-admin-sourced user message (the shape `agent.inject` / enter decisions take).
 * dsh 0.1.7 (V4 sessions) RETIRED the shared `kind: 'plugin'` wrapper — native
 * admission throws `SessionFormatError` on it — so this module owns its kind,
 * the same producer-owned pattern the schedule package rides (`kind: 'schedule'`). */
function pluginMessage(text) {
  return {
    id: randomUUID(),
    role: 'user',
    content: [{ type: 'text', text }],
    source: { kind: 'plugin-admin', plugin: PLUGIN_SOURCE_NAME },
  }
}
...
export function applyProjectHooks(ctx, options = {}) {
  ...
  async function runSessionStartHooks(agent, source) {
    const merged = await runEvent(
      ...
      source,
      {
        ...
        transcript_path: transcriptPathFor(ctx, agent?.session?.header),
  ...
  //
  // dsh 0.1.6 renamed `agent/session-start` to `agent/created` (same announce
  // step, same payload + `source`). Subscribe to both, de-duplicated per
  // agent, so SessionStart hooks run exactly once on either build.
  const startedAgents = new WeakSet()
  const onAgentStart = ({ agent, source }) => {
    if (agent === null || typeof agent !== 'object' || startedAgents.has(agent)) return
  ...
  ctx.on('agent/session-start', onAgentStart)
  ctx.on('agent/created', onAgentStart)
  ...
  ctx.on('agent/pre-step', async ({ agent, messages, signal }, next) => {
    // 空批次（如后台 compaction / projection 刷新触发的 step）没有用户提交，
    // 镜像官方桥 hooks-claude-code 的同款守卫：直接放行，不跑 hook 也不注入。
  ...
  ctx.on('tools/pre-execute', async (exec, next) => {
    const merged = await runEvent(
      ...
      exec.name,
      {
        ...
        transcript_path: transcriptPathFor(ctx, exec.agent?.session?.header),
 ...
 * `.agents` trees anywhere on disk. Ancestors stay allowed ONLY when they are
 * a plausible project root (contain `.git`) — the upward `.git` walk needs to
 * resolve a project root above a workspace directory (monorepo checkouts),
 * but a blanket ancestor allowance would let `E:\`-style paths pass the fence
 * for every workspace at once. Comparison is case-insensitive on Windows.
...
function cwdInKnownWorkspaces(registry, cwd) {
  ...
  return workspaces.some((ws) => {
    ...
    return root.startsWith(target + sep) && existsSync(join(target, '.git'))
  })
}
...
export function applyProjectAdmin(ctx) {
  const service = {
    ...
    async list(cwdArg) {
      ...
      if (hooksSource !== null) {
        try {
          const sourcePath = join(agentsDir, hooksSource)
          // Same untrusted-input cap the runtime parse uses: a huge config
          // must not block the panel's read either.
          const sourceSize = statSync(sourcePath).size
          if (sourceSize > PROJECT_CONFIG_MAX_BYTES) {
            hooksError = `配置 ${sourceSize} 字节超过 ${PROJECT_CONFIG_MAX_BYTES} 上限，未解析（项目钩子按不可用处理）`
          } else {
            ...
            // Unsupported / non-command entries are surfaced instead of
            // silently vanishing from the panel's view of the file.
            for (const row of parsed.skipped) {
              hooks.push({
                event: row.event,
                matcher: '',
                command: '',
                timeoutSec: null,
                error: row.type === 'unsupported-event'
                  ? '项目桥不支持该事件（其钩子被忽略）'
                  : `仅支持 command 类型钩子（${row.type} 被忽略）`,
              })
            }
          }
        } catch (error) {
