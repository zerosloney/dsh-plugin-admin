...
import { canonicalWatchPath } from './patch-utils.js'
...
/**
 * Read cap for one repo-controlled command markdown file. The scan is
 * synchronous and runs on session start and watcher events, so a
 * multi-megabyte file from a cloned repository must not block the host.
 */
const PROJECT_COMMAND_MAX_BYTES = 1024 * 1024
...
export function parseCommandFrontmatter(raw) {
  ...
  // Find the closing delimiter: a line that is EXACTLY `---` (modulo a
  // trailing \r). `indexOf('\n---')` alone would also stop at `----` or
  // `---x` lines, truncating the frontmatter early.
  let closing = -1
...
export function scanCommandsDir(dir) {
  ...
  for (const fileName of names) {
    ...
    try {
      ...
      // The command file is repo-controlled input: a huge one would block
      // the host loop on the synchronous read (this scan runs on session
      // start and on watcher events). Refuse it loudly instead.
      const size = statSync(filePath).size
      if (size > PROJECT_COMMAND_MAX_BYTES) {
        errors.push({ name, error: `文件超过 ${PROJECT_COMMAND_MAX_BYTES} 字节上限（${size}），已跳过` })
        continue
      }
...
export function applyProjectAgents(ctx, options = {}) {
  ...
  function ensureWatcher(projectRoot, dir) {
    ...
    let watcher
    try {
      // persistent:false + watcher.unref: the watcher must never keep the
      // dsh host process alive on its own; unref additionally covers the
      // deleted-directory state where Windows keeps firing events.
      watcher = watch(canonicalWatchPath(dir), { persistent: false })
      watcher.unref?.()
    } catch (error) {
      warn(`project-agents: watching ${dir} failed: ${String(error)}`)
      return
    }
    // The debounce timer lives on the stored state object so the unload
    // cascade can clear the CURRENT pending timer — capturing the closure
    // variable at set() time would freeze its pre-change null, and the
    // teardown's clearTimeout would never fire.
    const state = { watcher, timer: null }
    watcher.on('change', () => {
    ...
    watcher.on('error', () => {
      const registered = watchers.get(projectRoot)
      if (registered !== undefined && registered.watcher === watcher) watchers.delete(projectRoot)
      try {
        watcher.close()
      } catch {
  ...
  // dsh 0.1.6 renamed `agent/session-start` to `agent/created` — the same
  // `agents.announce()` step, the same payload plus a `source` discriminator.
  // Subscribe to both and de-duplicate per agent so the project commands mount
  // exactly once whichever edge the running build emits.
  const startedAgents = new WeakSet()
  const onAgentStart = ({ agent }) => {
