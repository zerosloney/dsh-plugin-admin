...
export function applyProjectAgents(ctx, options = {}) {
  ...
  function ensureWatcher(projectRoot, dir) {
    ...
    try {
      ...
      // TEMP-DIAG (removed after CI diagnosis): the Windows runner crashes in
      // libuv fs-event.c when the watched path diverges textually from the path
      // the kernel reports in events. Print the given path and its canonical form.
      let diagReal = null
      try { diagReal = realpathSync.native(dir) } catch (error) { diagReal = 'ERR ' + String(error) }
      console.error('[diag] tmpdir=' + JSON.stringify(process.env.TEMP) + ' watchDir=' + JSON.stringify(dir) + ' realpath=' + JSON.stringify(diagReal))
