 ...
 * skill). This module reads the skill registry itself
...
export function applySkillsAdmin(ctx) {
  ...
  const scopeKeyFor = async (sessionId, preset) => {
    ...
    if (presets === null) return { ok: true, scope: undefined }
    try {
      return { ok: true, scope: await presets.standingKeyFor(preset) }
    } catch (error) {
  ...
  const service = {
     ...
     *
     * @param request - `{ sessionIds?: string[] }`; the panel sends every
     * session it knows so the union covers each distinct (cwd, preset) scope.
     * Omitted / empty reads the global layer alone.
     * @returns {{
    ...
    async list(request) {
      ...
      const ids = []
      for (const value of Array.isArray(req.sessionIds) ? req.sessionIds : []) {
      ...
      // 2. Group the requested sessions by (cwd, preset) so N sessions sharing
      // one workspace cost ONE registry read.
