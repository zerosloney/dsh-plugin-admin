 ...
 *   `ctx.agentPresets.acquireScope` (dsh 0.1.7+; the revision lease is held
 *   only for the duration of the read), falling back to
 *   `ctx.agentPresets.standingKeyFor` on older hosts. Either route ensures the
 *   preset's standing mount without starting an agent, a session, or a turn.
 *   This is the layer that matters in a WEB deployment: `@deepseek-ai/dsh-web-app`
 *   disables the host-plane `skill-filesystem` row (presets own local
 *   discovery), so the global read alone legitimately reports ZERO skills
 *   while the user directories live in a preset layer;
 ...
 * `agentPresets.acquireScope` (a revision lease: compose the standing mount,
 * read, then dispose) or, on hosts without it, `agentPresets.standingKeyFor` —
 * either way no agent, session, or turn starts; this is the supported route
...
export function applySkillsAdmin(ctx) {
   ...
   * running, otherwise the recorded preset's standing scope. The caller MUST
   * call `release()` after its registry read completes (a no-op where the
   * route has no lease). A preset that no longer resolves is REPORTED
   * (`ok: false`) rather than silently degrading to a shallower layer — the
   * caller surfaces it as a scope failure.
   *
   * dsh 0.1.7 replaced `standingKeyFor` with `acquireScope`, which hands back
   * a revision lease (`{ key, [Symbol.asyncDispose] }`): the mount is pinned
   * only while the lease is held, so the read must happen between acquire and
   * dispose or the generation may be collected mid-read.
   *
   * @param {string} sessionId
   * @param {string|null} preset
   * @returns {Promise<{ ok: true, scope: unknown, release: () => Promise<void> } | { ok: false, message: string }>}
   */
  const scopeKeyFor = async (sessionId, preset) => {
    ...
    if (agents !== null) {
      try {
        ...
        if (live !== undefined && live !== null) return { ok: true, scope: live, release: async () => {} }
      } catch {
        /* fall through to the preset key */
      }
    }
    if (preset === null) return { ok: true, scope: undefined, release: async () => {} }
    let presets
    try {
    ...
    if (presets === undefined || presets === null) {
      // No preset plane at all (a CLI bundle): the cwd-only read IS the whole
      // story, so degrade exactly as before instead of reporting a fault.
      return { ok: true, scope: undefined, release: async () => {} }
    }
    if (typeof presets.acquireScope !== 'function' && typeof presets.standingKeyFor !== 'function') {
      // The service is mounted but exposes neither route — a host revision the
      // plugin cannot see preset layers through. Fail loud: silently dropping
      // to the cwd layer would hide a whole layer behind a "complete" roster.
      return { ok: false, message: 'agentPresets 服务既无 acquireScope 也无 standingKeyFor（无法解析预设作用域）' }
    }
    try {
      if (typeof presets.acquireScope === 'function') {
        const lease = await presets.acquireScope(preset)
        return { ok: true, scope: lease.key, release: async () => {
          try {
            const dispose = lease !== null && typeof lease === 'object' ? lease[Symbol.asyncDispose] : undefined
            if (typeof dispose === 'function') await dispose.call(lease)
          } catch {
            /* A failed release must not fail the read that already succeeded. */
          }
        } }
      }
      return { ok: true, scope: await presets.standingKeyFor(preset), release: async () => {} }
    } catch (error) {
  ...
  const readPresetRoster = async () => {
    ...
    // dsh 0.1.7 dropped `selectionPolicy()`; the default id is now a public
    // `defaultId` getter. Read it defensively — an absent getter only loses
    // the "default first" marker, never the roster itself.
    let defaultId = null
    try {
      defaultId = typeof presets.defaultId === 'string' && presets.defaultId !== '' ? presets.defaultId : null
    } catch {
      defaultId = null
    }
    if (defaultId === null && typeof presets.selectionPolicy === 'function') {
      const policy = presets.selectionPolicy()
      defaultId = policy !== null && typeof policy === 'object' && typeof policy.defaultId === 'string' ? policy.defaultId : null
    }
    const rows = []
  ...
  /**
   * A preset's standing scope with its release. Ensures the preset's standing
   * mount (the composition is composed; no agent, session, or turn starts)
   * and hands back the key a registry read passes as `scope` TOGETHER with the
   * `release()` that disposes the underlying revision lease when one exists —
   * the caller MUST invoke it in a finally once its read is done.
   * @param {string} id - preset id.
   * @returns {Promise<{ scope: unknown, release: () => Promise<void> }>}
   */
  const standingScopeFor = async (id) => {
    const presets = serviceOf('agentPresets', 'acquireScope') ?? serviceOf('agentPresets', 'standingKeyFor')
    if (presets === null) throw new Error('agentPresets 服务不可用（无法解析预设作用域）')
    if (typeof presets.acquireScope === 'function') {
      const lease = await presets.acquireScope(id)
      return { scope: lease.key, release: async () => {
        try {
          const dispose = lease !== null && typeof lease === 'object' ? lease[Symbol.asyncDispose] : undefined
          if (typeof dispose === 'function') await dispose.call(lease)
        } catch {
    ...
    return { scope: await presets.standingKeyFor(id), release: async () => {} }
  }
  ...
  const readPresetScope = async (registrySvc, map, scopes, preset) => {
    ...
    if (preset.broken !== null) {
      ...
      return { complete: true, label }
    }
    try {
      const { scope, release } = await standingScopeFor(preset.id)
      try {
        const result = await readRegistry(registrySvc, { scope })
        coverage.count = mergeCatalog(map, result.skills, label)
        return { complete: result.complete, label }
      } finally {
        await release()
      }
    } catch (error) {
  ...
  const service = {
    ...
    async list(sessionIds) {
      ...
      const sessions = []
      const warnings = []
      // Layers whose registry read came back with `complete: false`. The
      // aggregate boolean alone cannot tell a user WHICH layer is short (the
      // panel used to print a hard-coded provider name), so the labels ride the
      // warning channel the panel already renders.
      const partialLayers = []
      let complete = true
      ...
      try {
        ...
        if (!result.complete) {
          complete = false
          partialLayers.push(GLOBAL_LABEL)
        }
        ...
        // 抛错 = 全局层一个技能都没读到，聚合 complete 必须诚实翻 false。
        complete = false
      }
      ...
      if (roster.message !== '') {
        warnings.push(roster.message)
        complete = false   // 预设清单读取失败 = 所有预设层缺失
      }
      const presetRows = roster.rows.slice(0, MAX_PRESETS)
      if (roster.rows.length > presetRows.length) {
        warnings.push(`仅读取前 ${MAX_PRESETS} 个预设（共 ${roster.rows.length} 个）`)
        complete = false   // 截断的预设层没有读，名册不完整
      }
      for (const preset of presetRows) {
        const read = await readPresetScope(registrySvc, map, scopes, preset)
        if (!read.complete) {
          complete = false
          partialLayers.push(read.label)
        }
      }
      ...
      for (const bucket of bucketList) {
        ...
        try {
          ...
          if (!resolved.ok) {
            // 该作用域整体没读（预设解析失败会静默丢一层），complete 诚实翻 false。
            complete = false
          ...
          try {
            ...
            // The revision lease (0.1.7 acquireScope) pins the preset mount
            // only for the read — release it even when the read throws.
            await resolved.release()
          }
        } catch (error) {
          // One unusable scope must not sink the roster: it is reported, and
          // the panel renders it beside the skills that did resolve. The read
          // THREW (no partial result to inspect), so the roster is incomplete.
          complete = false
      ...
      if (ids.length > capped.length) {
        warnings.push(`仅解析前 ${MAX_SESSIONS} 个会话（共 ${ids.length} 个）`)
        complete = false   // 截断会话的 (cwd, preset) 作用域没有解析
      }
      if (buckets.size > bucketList.length) {
        warnings.push(`仅读取前 ${MAX_SCOPES} 个 (cwd, preset) 作用域`)
        complete = false   // 截断作用域的技能层没有读
      }
      if (partialLayers.length > 0) {
        // Name the layers: `complete: false` on its own reads as a dead end
        // (the panel used to blame a hard-coded provider name), and the failing
        // layer is the one a reader has to act on.
        warnings.push('以下技能层的源未报告完整发现（registry 的 complete=false，可能少列技能）：' + partialLayers.join('、'))
      }
