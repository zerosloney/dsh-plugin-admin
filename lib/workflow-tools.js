...
export function applyWorkflowTools(ctx, deps) {
  ...
  const dispose = tools.register({
    ...
    description: [
      ...
      '',
      'The script receives a facade: agent(prompt, opts?) delegates one subagent call (returns null on failure so the rest continues), parallel(thunks) runs subagent thunks concurrently behind a semaphore, pipeline(items, ...stages) streams items through stages, phase(title) / log(msg) / report(key, value) record progress, ask(question) blocks the run until the user or you answer it (use it for judgment calls a subagent should not make alone), shell(cmd) runs one host shell command (throws on failure). Top-level return value becomes the run result. Scripts are transpiled and sandboxed: no require/import/fs/network — orchestration only, heavy work goes through agent() and shell().',
      '',
      ...
      '',
      'Actions: create (start a new background run), amend (swap the script, keep cached steps), resume (continue a stopped/errored run), stop (cancel), list (runs), get (one run: script / result / log), answer (reply to a run blocked in ask()), save (store a script in the library: global or project scope), run_saved (start a library workflow by name), list_saved, delete_saved, eval (transpile and dry-run a snippet synchronously — agent() is stubbed, so it costs no subagent calls and only checks syntax and control flow).',
      '',
    ...
    parameters: {
      ...
      properties: {
        ...
        scope: { type: 'string', enum: ['global', 'project'], description: 'Library scope: global ($DSH_HOME) or project (<cwd>/.dsh/workflows, travels with the repo). Defaults to global.' },
    ...
    async execute(args, exec) {
      ...
      switch (action) {
        ...
        case 'eval': {
          ...
          try {
            const value = await evalSnippet(args.script, { args: args.args || {}, timeoutMs: args.timeoutMs })
        ...
        case 'delete_saved': {
          ...
          try {
            const scope = args.scope === 'project' ? 'project' : 'global'
            return json({ ok: library.deleteSaved(args.name, scope, scope === 'project' ? workspacePath : undefined) })
          } catch (err) {
            return json({ ok: false, error: messageOf(err) })
          }
        }
