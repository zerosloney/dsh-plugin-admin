...
export function applyWorkflowTools(ctx, deps) {
  ...
  const dispose = tools.register({
    ...
    description: [
      ...
      '',
      'The script receives a facade: agent(prompt, opts?) delegates one subagent call (returns null on failure so the rest continues; opts are provider / model / schema — schema makes that step return structured data), parallel(thunks) runs subagent thunks concurrently behind a semaphore, pipeline(items, ...stages) streams items through stages, phase(title) / log(msg) / report(key, value) record progress, ask(question) blocks the run until the user or you answer it (use it for judgment calls a subagent should not make alone), shell(cmd, opts?) runs one host shell command with opts workdir / timeoutMs and returns { exitCode, stdout, stderr, timedOut } — a non-zero exit code is data, not an exception (it throws only when the host shell is unavailable or the run is aborted). Top-level return value becomes the run result and must be a JSON value (circular refs / BigInt mark the run errored). Scripts run in an isolated Node vm realm: require/import/fs/network/process globals are unreachable — orchestration only, heavy work goes through agent() and shell(). The realm blocks accidental host access; it is not a hard security boundary.',
      '',
      'Runs are durable: a stopped or errored run can be resumed from its own journal (finished steps are served from cache and cost no subagent calls — the step fingerprint covers call site, kind and prompt only, so changing agent opts or args alone still hits the cached step), and amend() rewrites the script while reusing every cached step it still matches. Runs survive in $DSH_HOME/workflows/runs/ and show in the 工作流 settings panel.',
      '',
