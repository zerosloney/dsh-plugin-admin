...
import { ensureProfileDependency, harnessLockstepVersion, dshHome, profileDirOf, topLevelBlocks } from './patch-utils.js'
...
/**
 * Bounded in-memory replay dedup for `x-webhook-delivery` ids. Retrying
 * providers (GitHub retries reuse the same delivery id) must not re-execute
 * a rule's action; a redelivered id is acknowledged without re-running.
 * intentional-simple: insertion-eviction Map, process-lifetime only — a
 * delivery replayed after 512 newer deliveries or a dsh restart re-executes.
 * Upgrade path: persist seen ids to a JSONL sidecar if cross-restart
 * idempotency ever matters.
 */
 ...
 * re-executing the rule's action. Delete-before-set keeps Map insertion
 * order tracking recency so the cap evicts the least-recently-seen id.
 * @param {string} ruleId - the rule the delivery addressed.
 * @param {string} deliveryId - the caller-supplied idempotency key.
 * @returns {boolean} true when newly claimed (execute), false on replay.
 */
function makeDeliveryClaimer() {
  const seen = new Map()
  return (ruleId, deliveryId) => {
    const key = `${ruleId}\0${deliveryId}`
    if (seen.has(key)) return false
    seen.delete(key)
    while (seen.size >= DELIVERY_DEDUP_CAP) {
...
export function applyWebhookAdmin(ctx, options = {}) {
  ...
  /** Replay dedup for x-webhook-delivery ids (per-mount lifecycle). */
  const claimDelivery = makeDeliveryClaimer()
    ...
    const disp = runtime.register({
      ...
      run: async (delivery, signal) => {
        try {
          ...
          if (!claimDelivery(rule.id, delivery.deliveryId)) {
  ...
  async function runtimeInstall() {
    ...
    const dep = await options.enqueue(async () => {
      ...
      if (!hasWebhookRuntimeRow(existing)) {
        ...
        let next
        if (placeholder !== -1) {
          next = [...lines.slice(0, placeholder), ...row.trimEnd().split('\n'), ...lines.slice(placeholder + 1)].join('\n')
          if (!next.endsWith('\n')) next += '\n'
        } else {
        ...
        writeFileSync(patchTemp, next, 'utf8')
...
function createHttpHandler(ctx, storagePath, history, claimDelivery) {
  return async (req, res) => {
    try {
      ...
      if (!claimDelivery(rule.id, deliveryId)) {
