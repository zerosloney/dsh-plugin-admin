 ...
 */
/**
 * Inline flow-config body: `{ ... }` 后面允许行尾注释（`config: {a: 1} # 备注`）。
 * 注释若不豁免，解析处会把 flow map 误判成 block（键读成空），重写处会把兄弟
 * 键全部丢掉——四处调用共享这一条正则。
 * @param {string} inline - 已 trim 的 `config:` 值。
 */
const INLINE_FLOW_CONFIG = /^\{(.*)\}\s*(?:#.*)?$/
...
function parseWebRow(lines) {
  ...
  for (let i = 0; i < block.length; i++) {
    ...
    if (/^\s*config:\s*/.test(line)) {
      ...
      const inlineMatch = INLINE_FLOW_CONFIG.exec(inline)
      if (inlineMatch !== null) {
...
function mutateWebRow(lines, span, providerId) {
  ...
  if (configInline) {
    ...
    const inline = next[configIndex].replace(/^\s*config:\s*/, '').trim()
    const inlineMatch = INLINE_FLOW_CONFIG.exec(inline)
    const kept = []
    if (inlineMatch !== null) {
      for (const [key, value] of inlineMapEntries(inlineMatch[1])) {
...
function parseEntryConfig(entryLines) {
  ...
  if (found.inline !== '') {
    const inlineMatch = INLINE_FLOW_CONFIG.exec(found.inline)
