...
export function applyWebSearchAdmin(ctx, options = {}) {
  ...
  const service = {
    ...
    async uninstall(providerId) {
      ...
      return await enqueue(async () => {
        // 1. Strip the cordis row (insert-shaped entry or legacy bare row).
        const { lines, patchPath } = readPatchLines(dir)
        const next = removeMountRow(lines, meta.cordisId)
        if (next !== null) writePatch(patchPath, next)
        // 2. Never leave the `web` row pointing at a provider that is about to
        //    disappear: a dangling `searchProvider` makes every search fail with
        //    WEB_PROVIDER_CONFIGURED_MISSING after the restart, and the panel's
        //    radio is disabled for the removed id so the user cannot see why.
        const afterStrip = next === null ? lines : next
        const active = parseWebRow(afterStrip)
        if (active !== null && active.searchProvider === meta.id) {
          const span = findRowSpan(afterStrip, 'web')
          const repaired = span === null
 ...
 * Tolerates either an inline block (`config: { searchProvider: 'x' }`)
 * or block scalars (`config:\n  searchProvider: x`).
 * @param {string[]} lines
 * @returns {{ searchProvider?: string, fetchProvider?: string }|null}
 */
function parseWebRow(lines) {
  ...
  for (let i = 0; i < block.length; i++) {
    ...
    if (inConfig) {
      const inlineMatch = /^\s*config:\s*\{(.*)\}\s*$/.exec(line)
      if (inlineMatch !== null) {
        // Inline JSON-like block — pick the keys we need from one line.
        ...
        for (const part of body.split(',')) {
          ...
          if (kv.length === 2) {
            ...
            if (k === 'searchProvider') searchProvider = v
            if (k === 'fetchProvider') fetchProvider = v
          }
        }
        inConfig = false
        continue
...
function mutateWebRow(lines, span, providerId) {
  ...
  if (configInline) {
    ...
    const head = next[configIndex].match(/^\s*config:/) ? next[configIndex].match(/^\s*/)[0] : '  '
    ...
    if (inlineMatch !== null) {
      for (const part of inlineMatch[1].split(',')) {
        ...
        if (key === '' || key === 'searchProvider') continue
        kept.push(`${head}  ${key}: ${kv[1].trim()}`)
      }
    }
    const splice = [
      `${head}config:`,
