...
export async function probeMcpServer(cfg, callRequest) {
  ...
  // Same trust boundary as the tools/call guard below: a caller that hands
  // over no config at all must get the documented { ok: false } shape, not a
  // TypeError escaping the catch (which re-reads cfg.transport).
  if (cfg === null || typeof cfg !== 'object') {
    return jsonSafe({
      ok: false,
      transport: '',
      ms: ms(),
      error: 'invalid MCP config: an object with a transport is required',
    })
  }
...
export function scrubbedProbeEnv(extra) {
  ...
  // Deliberate trade-off, kept in step with the real launch: proxy variables
  // are what give the server its egress route, and a proxy URL with embedded
  // credentials (http://user:pass@host) therefore rides along — the probe
  // hands the same ambient facts the real dsh-mcp-client launch would.
...
/**
 * Escape one token for a Windows cmd.exe /c command line, using cross-spawn's
 * verbatim-argv quoting rules: double the backslash runs that precede a quote
 * (a lone `\` before `\"` would be read as escaping the BACKSLASH, leaving the
 * quote active), escape each quote as `\"`, and double trailing backslashes
 * (they would otherwise escape the closing quote the wrapper adds).
 */
...
function probeMcpHttp(cfg, callRequest) {
  return new Promise((resolve) => {
    ...
    // Every step of the sequence (initialize, ping, tools/list, tools/call)
    // rides that same budget: a cold-start server that is slow to answer the
    // handshake or the listing must not be failed at the short probe timeout
    // while the tool-call budget is still open. The outer timer below still
    // bounds the whole probe.
    const stepTimeoutMs = budget
    const timer = setTimeout(() => {
    ...
    const doProbe = async () => {
      try {
        ...
        const abortTimer = setTimeout(() => controller.abort(), stepTimeoutMs)
        let response
        try {
        ...
        if (contentType.includes('text/event-stream')) {
          ...
          while (true) {
            ...
            for (const frame of frames) {
              // SSE concatenates every `data:` line of a frame with '\n'; a
              // server that wraps one JSON payload across several of them
              // must not be read as only its last fragment.
              const dataLines = []
        ...
        try {
          ...
          if (pingOk) {
            const listed = await httpToolList(cfg.url, followInit, stepTimeoutMs)
            toolList = listed.tools
            toolListError = listed.error
          }
        } catch {
          pingOk = false
        }
        ...
        const outcome = {
          ...
          pingOk,
          // Only present when the listing failed — the panel would otherwise
          // read an empty roster as "this server offers no tools".
          ...(toolListError === null ? {} : { toolListError }),
        }
        if (callRequest) {
          if (toolListError !== null) {
            fail(`could not list this server's tools: ${toolListError}`)
            return
          }
...
/**
 * Read one fetch response body as text under a hard length cap — the same
 * bound the SSE reader loops enforce on their accumulation. A plain-JSON
 * reply is fully buffered before parsing, so without the cap a misbehaving
 * server could balloon host memory on a non-SSE answer while every other
 * read path stays bounded.
 * @param response - the fetch Response to read.
 * @param maxBytes - cap on the accumulated text length.
 * @returns the body text.
 * @throws {Error} when the body exceeds the cap.
 */
export async function readBoundedText(response, maxBytes) {
  const reader = response.body !== null && typeof response.body === 'object' && typeof response.body.getReader === 'function'
    ? response.body.getReader()
    : null
  if (reader === null) {
    // No streamable body (unusual fetch implementations): read whole, then
    // enforce the cap on the result — the check still fails loud, it just
    // cannot bound the buffering.
    const text = await response.text()
    if (text.length > maxBytes) throw new Error('server response exceeded ' + maxBytes + ' bytes')
    return text
  }
  const decoder = new TextDecoder()
  let acc = ''
  while (true) {
    ...
    if (acc.length > maxBytes) {
      try { reader.cancel() } catch { /* stream already gone */ }
      throw new Error('server response exceeded ' + maxBytes + ' bytes')
    }
  }
  ...
  try {
    ...
    if (contentType.includes('text/event-stream')) {
      ...
      while (true) {
        ...
        acc = frames.pop()
        for (const frame of frames) {
          // Every `data:` line of a frame belongs to the same payload (SSE
          // joins them with '\n'); keeping only the last one would drop a
          // multi-line JSON answer on the floor.
          const dataLines = []
          for (const line of frame.split('\n')) {
            if (line.startsWith('data:')) dataLines.push(line.slice(5).trim())
          }
      ...
      // Same bound as every other read path (see readBoundedText): a
      // plain-JSON reply is fully buffered before parsing, so the cap is
      // what keeps a misbehaving server from ballooning host memory.
      message = JSON.parse(await readBoundedText(response, MCP_PROBE_MAX_RESPONSE_BYTES))
    }
    if (message === null) {
 ...
 * both plain-JSON and SSE responses. Best-effort: a transport failure comes
 * back as `{ tools: [], error }` rather than a throw, so the caller can tell
 * "this server offers no tools" apart from "the listing itself failed".
 * @param url - MCP endpoint URL.
 * @param init - base request init (headers).
 * @param timeoutMs - per-request budget (the probe's own step budget).
 * @returns {{ tools: object[], error: string|null }} the raw tool objects
 * (name-carrying), so callers can derive both the name list and each tool's
 * inputSchema-required params.
 */
async function httpToolList(url, init, timeoutMs = MCP_PROBE_HTTP_TIMEOUT_MS) {
  try {
    const response = await fetch(url, {
      ...init,
    ...
    if (!response.ok) return { tools: [], error: `HTTP ${response.status} ${response.statusText}` }
    const contentType = (response.headers.get('content-type') || '').toLowerCase()
    let tools = []
    if (contentType.includes('text/event-stream')) {
      const reader = response.body.getReader()
      ...
      while (!matched) {
        ...
        acc = frames.pop()
        for (const frame of frames) {
          // Same per-frame join as httpToolCall: SSE spreads one payload
          // across consecutive `data:` lines.
          const dataLines = []
          for (const line of frame.split('\n')) {
            if (line.startsWith('data:')) dataLines.push(line.slice(5).trim())
          }
      ...
      const text = await readBoundedText(response, MCP_PROBE_MAX_RESPONSE_BYTES)
      const message = JSON.parse(text)
      if (Array.isArray(message.result?.tools)) tools = message.result.tools
    }
    return {
      tools: tools.filter(tool => tool !== null && typeof tool === 'object' && typeof tool.name === 'string'),
      error: null,
    }
  } catch (error) {
    return {
