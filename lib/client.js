...
'[data-dsh-admin-section] .search-icon, [data-dsh-sa-section] .search-icon { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); font-size: 12px; opacity: 0.7; pointer-events: none; z-index: 1; }',
  ...
  '[data-dsh-admin-section] .filter-bar { display: flex; gap: 4px; align-items: center; flex-wrap: wrap; padding: 0 20px 0 10px; }',
  ...
  '[data-dsh-admin-section] .pill { font-size: 11px; padding: 3px 9px; border-radius: 6px; border: 1px solid var(--dsw-alias-border-l2, rgba(200,200,210,0.45)); background: transparent; cursor: pointer; color: var(--dsw-alias-label-secondary, #555); transition: all 0.12s; font-family: inherit; }',
  ...
  '[data-dsh-admin-section] .group-header { cursor: pointer; user-select: none; }',
  '[data-dsh-admin-section] .group-header:hover { border-color: var(--dsw-alias-border-l1, rgba(160,160,180,0.6)); }',
  ...
  '[data-dsh-admin-section] .group-collapsed-note { font-size: 11px; color: var(--dsw-alias-label-secondary, #61666b); font-weight: 400; }',
  ...
  // 有余量时向右长到 360px 以容纳长项目名，真挤不下才整组换行并右对齐。
  '[data-dsh-admin-section] .usage-filter { display: flex; flex-wrap: nowrap; align-items: stretch; flex: 1 1 200px; min-width: 170px; max-width: 360px; margin-left: auto; height: 30px; border: 1px solid var(--dsw-alias-border-l2, rgba(200,200,210,0.5)); border-radius: 8px; overflow: hidden; background: var(--dsw-alias-bg-base, transparent); }',
  ...
  '[data-dsh-admin-section] .usage-filter .usage-project-select { flex: 1 1 auto; width: auto; min-width: 0; max-width: 100%; height: 100%; border: none; border-radius: 0; background: transparent; padding: 0 10px; text-overflow: ellipsis; }',
  ...
  '[data-dsh-admin-section] .card-header, [data-dsh-sa-section] .card-header { display: flex; align-items: center; gap: 8px; min-width: 0; }',
  '[data-dsh-admin-section] .card-title, [data-dsh-sa-section] .card-title { font-size: 13px; font-weight: 600; color: var(--dsw-alias-label-primary); display: flex; align-items: center; gap: 7px; min-width: 0; flex: 1; }',
...
function setupArchivedSessionsMerge(call, refreshSessions, onFallback) {
  ...
  function sync() {
    ...
    var archivedRow = navRowByLabel(nav, '已归档会话', 'Archived sessions')
    if (archivedRow === null) {
  ...
  function runBulkDelete() {
    ...
    var step = function () {
      ...
      if (done >= plan.length) {
        ...
        if (failures > 0) {
          patchSession({ error: '批量删除完成，' + String(failures) + ' 个会话删除失败（详见交付历史/宿主日志）' })
        }
  ...
  return createElement('div', { className: 'card mcp-playground', key: 'mcp-playground' },
    ...
    createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px' } },
      ...
      createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center' } },
        ...
        resultChildren.length > 0 && !pg.busy ? createElement('span', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, '结果见下方') : null),
  ...
  if (view.mcpConfirmRemove === entry.id) {
    rowChildren.push(createElement('div', { className: 'confirm-bar', key: 'confirm-remove' },
      createElement('span', { className: 'confirm-text', key: 'text' },
        '⚠️ 确定从 cordis.patch.yml 移除该 MCP 服务器配置？整块配置（含 headers 中的密钥行）会被删除。'),
...
function UsageDashboardSection(props) {
  ...
  var sweepCadence = sweepMinutes >= 1 ? sweepMinutes + ' 分钟' : Math.round(usage.snapshotIntervalMs / 1000) + ' 秒'
  var sweepHint = sweepOn
    ? '每 ' + sweepCadence + '自动快照会话用量到台账'
      + (usage.lastSnapshotAt !== null ? '；最近一次 ' + new Date(usage.lastSnapshotAt).toLocaleTimeString() : '')
      + (usage.storagePath ? '（' + usage.storagePath + '）' : '')
    : '后台自动快照已关闭（config.usageSnapshotIntervalMs = 0）——只有打开本页时才会记录用量'

  return createElement('div', { 'data-dsh-admin-section': '' },
    createElement('div', { className: 'toolbar usage-head' },
      createElement('span', { className: 'title' }, '📊 用量仪表盘'),
      createElement('span', { className: 'usage-sub' }, 'VibeUsage 姿态 · 本地聚合'),
  ...
  return createElement('div', { className: 'usage-dash', key: 'usage-dash' },
    createElement('div', { className: 'usage-toolbar' },
      createElement('div', { className: 'usage-group' },
        createElement('span', { className: 'usage-group-label' }, '⏱ 日期'),
...
function buildGroupHeader(g, groupKey, collapsed, view, groupUi) {
  ...
  return createElement('div', {
    ...
    key: 'header-' + groupKey,
    title: collapsed ? '点击展开该目录的会话' : '点击折叠该目录的会话',
    ...
    createElement('span', { className: 'group-title', key: 'title' },
      (isUngrouped ? '📂 ' : '📁 ') + (g.title || '未命名')
    ),
    createElement('span', { className: 'group-count', key: 'count' }, String(g.sessions.length) + ' 个'),
    collapsed ? createElement('span', { className: 'group-collapsed-note', key: 'note' }, '已折叠') : null,
    g.path !== null
    ...
    createElement('button', {
      type: 'button',
      key: 'btn-bulk-group',
      className: 'btn xs danger-solid',
      title: '删除该目录下的全部 ' + String(g.sessions.length) + ' 个会话（在线会话会先关停）',
...
function renderSessionsView(view, patch, reload, act, onExport, pinnedIds, togglePinned, runFulltext, loadHealth, enableSearch, groupUi) {
  ...
  elements.push(createElement('div', { className: 'filter-bar', key: 'filters' },
    ...
    (groupKeys.length > 0 || filtered.length > 0) ? createElement('div', { className: 'filter-actions', key: 'actions' },
      ...
      }, allCollapsed ? '▾ 全部展开' : '▴ 全部折叠') : null,
      filtered.length > 0 ? createElement('button', {
        type: 'button', key: 'btn-bulk-all', className: 'btn xs danger-solid',
        title: '删除当前筛选下的全部 ' + String(filtered.length) + ' 个会话（在线会话会先关停）',
        disabled: view.bulkBusy,
        onClick: function () { groupUi.requestBulk('all', null) },
      }, '🗑 删除当前 (' + String(filtered.length) + ')') : null
    ) : null
  ))
  ...
  if (view.bulk !== null && view.bulk !== undefined) {
    ...
    elements.push(createElement('div', { className: 'bulk-bar', key: 'bulk-bar' },
      createElement('span', { className: 'bulk-text', key: 't' },
        view.bulkBusy
          ? '删除中 ' + String(view.bulkDone) + ' / ' + String(bulk.plan.length) + '…'
          : '确认删除 ' + bulkScope + '的 ' + String(bulk.plan.length) + ' 个会话？此操作不可撤销'
      ),
      ...
      if (followupOpen) {
        cardExtra.push(createElement('div', { className: 'card followup-editor', key: 'followup', style: { margin: '0 12px 10px 12px', padding: '8px' } },
          ...
          createElement('div', { key: 'acts', style: { display: 'flex', gap: '6px', alignItems: 'center' } },
            ...
            createElement('span', { key: 'hint', style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, 'Ctrl+Enter 发送 · Esc 关闭'),
          ),
        ))
      }
    ...
    importing !== null
      ? createElement('div', { className: 'card', key: 'import-panel' },
        ...
        createElement('div', { style: { display: 'flex', gap: '8px', marginTop: '8px', alignItems: 'center' } },
          ...
          createElement('span', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, '兼容「⬇ 导出」的文件内容；同名命令一律跳过，绝不静默覆盖本地修改。')))
      : null,
...
function ChHooksTab(props) {
  ...
  if (bridgePathMismatch) {
    bridgeText = '⚠️ 桥挂载行的 configPath 指向 '
      + (bridgeConfigPath === null ? '（未声明）' : bridgeConfigPath)
      + '，与本面板管理的 ' + ((data && data.hooksPath) || 'hooks.json') + ' 不一致——在此保存的钩子不会生效。点「⚡ 修复桥指向」把它改到本面板文件，或手工编辑 profile 的 cordis.patch.yml。'
 ...
 var codexText
 if (codexMounted) {
 codexText = 'Codex 钩子桥（hooks-codex）已挂载：消费 ' + codexHooksPath + ' 里的 Codex 格式钩子（5 个钩点：SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop），与上面的 Claude-Code 格式互不影响。本页只做安装 / 卸载 —— 钩子内容手工编辑该文件。'
 } else if (codexInstalled && codexRowPresent) {
 codexText = 'Codex 钩子桥已安装并写入 profile（钩子文件 ' + codexHooksPath + '）：重启 dsh 后挂载生效。'
 } else {
 codexText = '当前未安装 Codex 钩子桥（' + codexPackage + '）：Codex 格式钩子写入 ' + codexHooksPath + '，安装并挂载后才会执行。'
 }
 ...
 */
 ...
 : codexHooksPath
 setNote('Codex 钩子桥已安装并写入 profile（钩子文件 ' + path + '），重启 dsh 后挂载生效。')
 } else {
 setNote('Codex 钩子桥已卸载：patch 行与依赖包均已移除（重启 dsh 后完全生效）。')
 }
 load()
 }, function (e) {
 setError((verb === 'codexBridgeInstall' ? '安装失败：' : '卸载失败：') + messageOf(e))
 setBusy(false)
 })
 }
  ...
  return createElement('div', null,
    ...
    createElement('div', {
      ...
      (bridgeMissing || bridgePathMismatch)
        ...
        }, busy ? '安装中…' : (bridgePathMismatch ? '⚡ 修复桥指向' : '⚡ 安装并挂载 hooks 桥'))
        : null,
...
function CronSection(props) {
  ...
  function reload() {
    ...
    Promise.all([
      ...
      if (!cronRes.ok) {
        patch({ busy: false, error: '加载失败：' + messageOf(cronRes.error) })
        return
      }
      var v = cronRes.value || {}
  ...
  function toggleTask(id, enabled) {
    callRemote('cronAdmin/toggle', { id: id, enabled: enabled }).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ error: '切换失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ error: '切换失败：' + messageOf(err) })
    })
  }
  ...
  function runNow(id) {
    ...
    callRemote('cronAdmin/runNow', { id: id }).then(function (result) {
      ...
      if (result.ok && v.ok) {
        ...
        patch({ error: '立即触发失败：' + (messageOf(v.error || result.error) || '未知错误') })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busy: false, error: '立即触发失败：' + messageOf(err) })
    })
  }
  ...
  return createElement('div', { 'data-dsh-admin-section': '' },
    CronRender(state, cronActions, CRON_PRESETS))
}
  ...
  for (let i = 0; i < state.workspaces.length; i++) {
    ...
    var cardChildren = [
      ...
      createElement('div', { className: 'card-sub', key: 'sub' },
        ...
        ws.createdAt ? createElement('span', { className: 'card-sub-item', style: { marginLeft: '12px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, '创建于 ' + ws.createdAt.slice(0, 10)) : null,
        ws.updatedAt ? createElement('span', { className: 'card-sub-item', style: { marginLeft: '12px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, '更新于 ' + ws.updatedAt.slice(0, 10)) : null,
      ),
    ]
...
function WebSearchSection(props) {
  ...
  function loadConfig(id) {
    ...
    call('webSearchAdmin/config', { providerId: id }).then(function (result) {
      ...
      if (!result.ok) {
        patch({ configBusy: false, configError: '读取配置失败：' + messageOf(result.error) })
        return
      }
      ...
      if (!alive.current || configRequest.current !== id) return
      patch({ configBusy: false, configError: '读取配置失败：' + messageOf(err) })
    })
  }
  ...
  function saveProviderConfig(id) {
    ...
    call('webSearchAdmin/saveConfig', {
      ...
      if (!alive.current) return
      if (!result.ok) {
        patch({ configBusy: false, configError: '保存失败：' + messageOf(result.error) })
        return
      }
      ...
      patch({
        ...
        configNote: saved.restartRequired === true
          ? '✅ 已写入 ' + (saved.source === 'settings' ? 'settings' : 'cordis.patch.yml') + ' — 重启 dsh 后生效'
          : '✅ 已保存（即时生效）',
      })
      loadConfig(id)
    }, function (err) {
      if (!alive.current) return
      patch({ configBusy: false, configError: '保存失败：' + messageOf(err) })
    })
  }
  ...
  elements.push(createElement('div', { className: 'hint', key: 'restart-banner',
    ...
    '🔁 切换 provider / 安装 / 卸载 后需**重启 dsh** 生效；⚙ 配置里带 settings 命名空间的 provider 保存后即时生效，写 cordis 行的需重启',
  ))
  if (state.busy) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'busy' },
      createElement('span', { className: 'spinner', key: 'sp' }), '加载中…'))
  ...
  for (let i = 0; i < state.providers.length; i++) {
    ...
    actionChildren.push(createElement('button', {
      ...
      title: '编辑该 provider 的 Config（Endpoint / 模型 / API Key …）',
      onClick: function () { toggleConfig(p.id) },
    }, state.configOpen === p.id ? '⚙ 收起配置' : '⚙ 配置'))
...
function renderProviderConfig(view, ui) {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if (field.kind === 'enum') {
      var options = [createElement('option', { key: '__inherit', value: '' },
        '继承默认' + (field.default === null || field.default === undefined ? '' : '（' + field.default + '）'))]
      ...
      var placeholder = field.kind === 'secret'
        ? (field.set === true ? '已配置 — 留空不修改' : '未配置 — 留空则回退到凭据 / 环境变量')
        : (field.default === null || field.default === undefined ? '留空 = 继承 provider 默认值' : '继承默认：' + field.default)
      ...
      createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
        ...
        field.kind === 'secret'
          ? createElement('button', {
            ...
            title: field.set === true ? '删除已存的密钥值' : '当前没有可删除的密钥值',
            onClick: function () { ui.toggleClear(field.key) },
          }, ui.cleared[field.key] === true ? '↺ 撤销' : '清除')
          : null,
        field.set === true ? createElement('span', { className: 'tag live', key: 'set' }, '已设置') : null,
      ),
  ...
  return createElement('div', {
    ...
    createElement('div', { className: 'hint', key: 'storage', style: { fontSize: '11px' } },
      (view.source === 'settings' ? '🗄 存储：dsh settings 命名空间 `' : '🗄 存储：cordis.patch.yml 行 `') + view.namespace + '`'
      + (view.restartRequired === true ? ' · 保存后需重启 dsh 生效' : ' · 保存后即时生效')),
    ...
    rows.length > 0
      ...
      : createElement('div', { className: 'empty', key: 'norows' }, '该 provider 没有可配置字段'),
    ...
    return createElement('div', { key: 'config' },
      state.configBusy
        ? createElement('div', { className: 'busy-banner', key: 'busy' },
          createElement('span', { className: 'spinner', key: 'sp' }), '读取 ' + provider.label + ' 配置…')
        : createElement('div', { className: 'error', key: 'err' }, state.configError !== '' ? state.configError : '配置不可用'))
  }
...
function SkillsSection(props) {
  ...
  function loadRoster() {
    ...
    var sessionIds = call('sessionAdmin/list', {}).then(function (result) {
      if (!(result && result.ok)) {
        return { ids: [], note: '会话列表不可用（' + messageOf(result && result.error) + '）— 仅显示全局技能' }
      }
      ...
      return { ids: [], note: '会话列表不可用（' + messageOf(err) + '）— 仅显示全局技能' }
    })
  ...
  function copySlashName(name) {
    ...
    copyTextSilently('/' + name).then(function () {
      ...
      if (!alive.current) return
      showToast('error', '❌ 复制失败：' + messageOf(err) + '（可手动复制 /' + name + '）')
    })
  }
  ...
  if (!state.available) {
    return createElement('div', { 'data-dsh-admin-section': '' },
      createElement('div', { className: 'group-header', key: 'unavail' },
        ...
        createElement('span', { className: 'group-count', key: 'c' }, '本部署未挂载 @deepseek-ai/dsh-skill（技能注册表不可用）'),
      ),
    )
  }
  ...
  if (state.busy) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'busy' },
      createElement('span', { className: 'spinner', key: 'sp' }), '加载技能清单…'))
  }
  if (state.error !== '') {
    elements.push(createElement('div', { className: 'error', key: 'err' }, state.error))
  }
  ...
  if (!state.complete) {
    elements.push(createElement('div', { className: 'hint', key: 'incomplete', style: { fontSize: '11px' } },
      '⚠ 部分技能源本次发现未完成（dsh-skill 的 complete=false）— 清单可能少列若干技能，刷新可重试'))
  }
  ...
  // card says.
  elements.push(createElement('div', { className: 'toolbar', key: 'toolbar' },
    createElement('button', { type: 'button', className: 'btn', key: 'refresh', disabled: state.busy, onClick: reload }, '⟳ 刷新'),
    createElement('div', { className: 'search-wrap', key: 'search' },
      createElement('span', { className: 'search-icon', key: 'icon' }, '🔎'),
      createElement('input', {
        className: 'input', key: 'needle', placeholder: '按名称 / 描述 / 路径过滤…',
        value: state.needle, 'aria-label': '过滤技能',
        onChange: function (e) { patch({ needle: e.target.value }) },
      }),
    ),
  ))

  // Roster summary + the scope coverage panel.
  ...
  function unresolvedSessionText(session) {
    return '⚠ 会话 ' + session.sessionId + ' 的作用域未能解析：' + session.message
  }
  elements.push(createElement('div', { className: 'hint', key: 'summary' },
    '📌 共 ' + state.skills.length + ' 个技能（全局 + ' + presetScopes.length + ' 个预设 + ' + sessionScopes.length + ' 个会话作用域）· 当前匹配 ' + filtered.length + ' 个 — 仅枚举（不加载正文、不修改 SKILL.md）'))
  elements.push(createElement('div', { key: 'scope-toggle', style: { fontSize: '11px' } },
    ...
    }, state.scopePanel ? '▾ 收起作用域明细' : '▸ 查看作用域明细（' + state.scopes.length + ' 个）'),
  ))
  if (state.scopePanel) {
    var scopeChildren = []
    for (var pi = 0; pi < state.scopes.length; pi++) {
      ...
      scopeChildren.push(createElement('div', { key: 'scope-' + pi, className: 'card-sub', style: { fontSize: '11px' } },
        ...
        scopeRow.error !== null && scopeRow.error !== undefined && scopeRow.error !== ''
          ...
          : createElement('span', { style: { marginLeft: '8px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, scopeRow.count + ' 个技能' + (scopeSessions > 0 ? ' · ' + scopeSessions + ' 个会话' : '')),
      ))
    }
    ...
    elements.push(createElement('div', { key: 'scope-panel', className: 'card' },
      createElement('div', { className: 'card-header', key: 'h' },
        createElement('span', { className: 'card-title-text', key: 't' }, '作用域覆盖'),
      ),
  ...
  for (let ci = 0; ci < filtered.length; ci++) {
    ...
    var headerChildren = [
      ...
      skill.modelInvocable
        ? createElement('span', { className: 'tag plugin', key: 'model', title: '模型可主动调用（skill 工具）' }, '🤖 模型可调用')
        : null,
      skill.userInvocable
        ? createElement('span', { className: 'tag', key: 'user', title: '人类可在 composer 用 /name 调用' }, '👤 人类可调用')
        : null,
      !skill.modelInvocable && !skill.userInvocable
        ? createElement('span', { className: 'tag archived', key: 'none', title: '该技能当前不暴露给任何调用方' }, '未暴露')
        : null,
    ...
    if (skill.whenToUse) {
      subChildren.push(createElement('div', { key: 'whe', className: 'card-sub', style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)' } }, '何时用：' + skill.whenToUse))
    }
    if (skill.path) {
    ...
    subChildren.push(createElement('div', { key: 'scopes', className: 'card-sub', style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)' } },
      '可见于：' + seenIn.map(function (label) { return scopeShortByLabel[label] || label }).join('、')
  ...
  if (filtered.length === 0 && state.skills.length > 0) {
    cardNodes.push(createElement('div', { className: 'empty', key: 'empty-filter' }, '没有匹配当前筛选条件的技能'))
  }
  ...
  if (state.skills.length === 0 && !state.busy && state.error === '') {
    elements.push(createElement('div', { className: 'empty', key: 'empty-skills' },
      '没有发现任何技能（全局层、预设作用域与会话作用域都未命中技能源）'))
    ...
    if (globalScope !== null) {
      ...
      emptyDiagnose.push('全局层：' + globalScope.count + ' 个技能'
        + (globalScope.count === 0 && presetScopes.length > 0 ? '（本部署由预设挂载本地技能，全局层为空属正常）' : ''))
    }
    ...
    if (presetScopes.length > 0) {
      emptyDiagnose.push('预设作用域：' + presetScopes.length + ' 个' + (failedPresetCount > 0 ? '（' + failedPresetCount + ' 个解析失败，展开上方"查看作用域明细"看原因）' : ''))
    }
    ...
    if (sessionScopeCount > 0) {
      emptyDiagnose.push('会话作用域：' + sessionScopeCount + ' 个' + (failedScopeCount > 0 ? '（' + failedScopeCount + ' 个解析失败，展开上方"查看作用域明细"看原因）' : ''))
    } else if (state.sessions.length === 0) {
      emptyDiagnose.push('会话作用域：无（dsh 当前没有已知会话）')
    }
    if (state.warnings.length > 0) emptyDiagnose.push('警告：' + state.warnings.length + ' 条')
    if (emptyDiagnose.length > 0) {
      elements.push(createElement('div', { className: 'hint', key: 'empty-diagnose', style: { fontSize: '11px' } },
        '诊断：' + emptyDiagnose.join(' · ')))
    }
  }
...
function CronRender(view, actions, cronPresets) {
  var elements = []
  ...
  if (!view.schedulerActive) {
    elements.push(createElement('div', { className: 'update-strip checking', key: 'sched-banner' },
      createElement('span', null, '调度器未运行（headless 部署或插件加载失败）——任务可编辑，但不会自动触发'),
    ))
  }

  // Toolbar
  elements.push(createElement('div', { className: 'toolbar', key: 'toolbar' },
    createElement('button', { type: 'button', className: 'btn primary', onClick: function () { actions.openEditor(null) } }, '+ 新建任务'),
    createElement('button', { type: 'button', className: 'btn', onClick: function () { actions.reload() } }, '⟳ 刷新'),
  ))

  if (view.editorOpen) {
    elements.push(renderCronEditor(view, actions, cronPresets))
  } else {
    if (view.tasks.length === 0) {
      elements.push(createElement('div', { className: 'empty', key: 'empty' },
        '还没有定时任务。新建一个任务，dsh 运行期间会按 cron 表达式自动触发。'))
    }
      ...
      elements.push(createElement('div', { className: 'card', key: 'task-' + task.id },
        createElement('div', { className: 'card-header' },
          createElement('span', { className: 'card-title' },
            ...
            task.enabled ? null : createElement('span', { className: 'tag', style: { background: 'rgba(239,68,68,0.1)', color: '#ef4444' } }, '已停用'),
            task.nextRun === null
              ? createElement('span', { className: 'tag', style: { background: 'rgba(234,179,8,0.12)', color: '#eab308' } }, '无可触发时刻')
              : null,
          ),
          createElement('span', { className: 'card-actions' },
            createElement('label', { style: { display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', marginRight: '4px' }, title: '启用/停用' },
              createElement('input', {
                type: 'checkbox',
            ...
            createElement('button', { type: 'button', className: 'btn sm', onClick: function (e) { e.stopPropagation(); actions.openEditor(task) } }, '编辑'),
            createElement('button', { type: 'button', className: 'btn sm', onClick: function (e) { e.stopPropagation(); actions.runNow(task.id) }, title: '立即触发一次（会真实注入消息/新建会话）' }, '▶ 立即触发'),
            createElement('button', {
              type: 'button', className: 'btn sm danger',
              onClick: function (e) { e.stopPropagation(); actions.patch({ confirmId: view.confirmId === task.id ? null : task.id }) },
            }, view.confirmId === task.id ? '✕' : '删除'),
          ),
        ),
        view.confirmId === task.id
          ? createElement('div', { className: 'confirm-bar', key: 'confirm' },
            createElement('span', { className: 'confirm-text' }, '确定删除任务「', task.id, '」？'),
            createElement('span', { className: 'confirm-actions' },
              createElement('button', { type: 'button', className: 'btn sm', onClick: function () { actions.patch({ confirmId: null }) } }, '取消'),
              createElement('button', { type: 'button', className: 'btn sm danger-solid', onClick: function () { actions.deleteTask(task.id) } }, '删除'),
            ))
          : null,
        createElement('div', { className: 'card-sub' },
          createElement('span', { className: 'card-sub-item' },
        ...
        createElement('div', { className: 'card-sub', style: { fontSize: '11px' } },
          createElement('span', { className: 'card-sub-item', style: { color: 'var(--dsw-alias-label-secondary, #61666b)' } },
            ...
            createElement('span', { style: { fontWeight: 600 }, title: task.nextRunISO ? formatLocal(task.nextRunISO) : '' },
              task.nextRun === null ? '—（表达式在可搜索范围内无匹配日期）'
                : formatCountdown(nextIn) + ' 后 (' + formatLocal(task.nextRun) + ')'),
          ),
          task.action.mode === 'steer' && task.targetOnline === false
            ? createElement('span', { className: 'card-sub-item', style: { color: '#eab308' }, title: '目标会话不在内存中，触发时注入会失败' }, '⚠ 目标会话离线')
            : null,
        ),
  ...
  if (view.history.length > 0) {
    elements.push(createElement('div', { style: { marginTop: '8px', fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)' }, key: 'hist-head' },
      '最近 ' + view.history.length + ' 次触发（本次运行期间）'))
    for (var hi = 0; hi < view.history.length; hi++) {
      var h = view.history[hi]
      var ok = h.ok !== false
      elements.push(createElement('div', { className: 'card', key: 'hist-' + hi, style: { padding: '8px 12px', fontSize: '12px' } },
  ...
  if (view.storagePath) {
    elements.push(createElement('div', { className: 'footer', key: 'footer' },
      createElement('span', { className: 'path', title: view.storagePath }, '📁 ' + view.storagePath),
      createElement('span', null, '定时任务（宿主级）'),
    ))
  }
...
function renderCronEditor(view, actions, cronPresets) {
  var d = view.draft
  if (!d) return null
  var presets = cronPresets || []
  return createElement('div', { className: 'card mcp-editor', key: 'editor' },
    createElement('div', { style: { display: 'flex', gap: '10px', alignItems: 'baseline' } },
      createElement('label', { style: { fontSize: '12px', fontWeight: 600, flex: 'none' } }, 'ID'),
      createElement('input', { className: 'input', value: d.id, disabled: !d.isNew, onChange: function (e) { actions.patchDraft({ id: e.target.value }) }, placeholder: '任务标识（英文字母开头，无空格）' }),
    ),
    createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center' } },
      createElement('label', null,
        createElement('input', { type: 'checkbox', checked: d.enabled, onChange: function (e) { actions.patchDraft({ enabled: e.target.checked }) } }), ' 启用',
    ...
    createElement('div', null,
      createElement('label', { style: { fontSize: '12px', fontWeight: 600 } }, 'cron 表达式（分 时 日 月 周，本地时区）'),
      createElement('div', { style: { display: 'flex', gap: '8px', marginTop: '4px' } },
        ...
        createElement('select', {
          ...
          onChange: function (e) { if (e.target.value) actions.patchDraft({ cron: e.target.value }) },
          value: '',
        },
          [createElement('option', { key: 'ph', value: '' }, '常用预设…')]
            .concat(presets.map(function (p, i) {
              return createElement('option', { key: 'preset-' + i, value: p.value }, p.label + '  ' + p.value)
            })),
        ),
      ),
      createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-secondary, #61666b)', marginTop: '2px' } },
        '支持 *, 逗号列表, 短横范围, 斜杠步长；周接受 0-7 与 SUN-SAT；进程重启期间到期的任务不补投，恢复后重算下次时刻。'),
    ),
    createElement('div', null,
      createElement('label', { style: { fontSize: '12px', fontWeight: 600 } }, '动作模式'),
      createElement('div', { style: { display: 'flex', gap: '6px', marginTop: '4px' } },
        createElement('button', { type: 'button', className: 'pill' + (d.actionMode === 'steer' ? ' active' : ''), onClick: function () { actions.patchDraft({ actionMode: 'steer' }) } }, '推送到既有会话'),
        createElement('button', { type: 'button', className: 'pill' + (d.actionMode === 'create' ? ' active' : ''), onClick: function () { actions.patchDraft({ actionMode: 'create' }) } }, '新建会话'),
      ),
