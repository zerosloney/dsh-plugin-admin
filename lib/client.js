...
function setupMenuInjection(call, refreshSessions) {
  ...
  function injectItems(menuEl) {
    ...
    if (isSession) {
      ...
      // Delete the session (never live). The item arms on the first click
      // and only fires on a second click within 4s (confirmText below) —
      ...
      appendMenuItem(viewport, '删除会话', 'danger', function () {
        resolveSessionExact(function (match) {
          if (match.live) { showToast('error', '❌ 删除会话失败：会话在线（仍挂载于 dsh host 内存），重启 dsh 后可删除'); return }
 ...
 * this process) — NOT that a turn is running. Such sessions keep their agent
 * alive, resist physical deletion (their log would resurrect on the next
 * flush), and become deletable "ended" sessions after a dsh restart.
 */
var LIVE_HINT = '会话在线：仍挂载于 dsh host 内存（本进程内创建或打开过的会话保持在线，不代表正在运行）；重启 dsh 后变为已结束，即可删除'
