...
// and falls back to IconSettingsOutline16), so this plugin's ten pages
...
var SETTINGS_NAV_ICONS = {
  ...
  '存储与凭据': '<ellipse cx="8" cy="5" rx="4.6" ry="2"/><ellipse cx="8" cy="10.4" rx="4.6" ry="2"/>',
    ...
    call('credentialAdmin/list', {}).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({
          ...
          refs: v.refs || [],
          scan: v.scan && v.scan.reason === 'scan-failed' ? v.scan : null,
        })
      } else {
...
function renderCredentialSection(view, actions) {
  ...
  elements.push(createElement('div', { className: 'toolbar', key: 'head-credentials' },
    createElement('span', { className: 'title' }, '凭据'),
    createElement('span', { className: 'count' }, view.refs ? String(view.refs.length) : '—'),
  ))
  ...
  var refs = view.refs || []
  // A failed scan and an empty roster both render zero rows but mean opposite
  // things: the first is a host fault that would otherwise masquerade as a
  // confidently empty page, so it is reported ahead of the empty state.
  var scanFailed = view.scan && view.scan.reason === 'scan-failed'
  if (scanFailed && view.available !== false) {
    elements.push(createElement('div', { key: 'scan-failed', className: 'update-strip warn' },
      '⚠ 凭据引用扫描失败：' + (view.scan.message !== '' ? view.scan.message : 'settings.describe() 未返回描述符')
      + ' —— 引用清单可能不完整（已挂载提供方的 credential-ref 字段读不到）。'))
  }
  if (refs.length === 0 && view.available !== false && !view.busy && !view.error && !scanFailed) {
    // A blank page reads as "broken"; say what an empty roster means. Refs come
    // from the mounted providers' own settings schemas, so an empty roster is a
    // fact about this composition, not a failed load.
    elements.push(createElement('div', { key: 'empty', className: 'empty' },
      '当前 composition 未声明任何凭据引用 —— 已挂载的模型提供方都没有 apiKeyEnv 这类 credential-ref 字段。'))
  }
...
function StorageSection(props) {
  ...
  // ---------- Render ----------
  var elements = []
  elements.push(createElement('div', { className: 'toolbar', key: 'head-storage' },
    createElement('span', { className: 'title' }, '存储后端'),
  ))
  ...
  for (let i = 0; i < state.backends.length; i++) {
    // Block-scoped on purpose (same shape WorkspacesSection documents): a
    // `var` binding would be shared by every handler in the loop, so each
    // radio would act on the LAST backend.
...
/**
 * 存储与凭据 settings page: the storage-backend block (picker + session
 * format version) followed by the credential block. The two blocks keep
 * their own state, mount and RPC namespace — this component only stacks
 * them inside one nav entry, each with its own heading strip.
 * @param props - { call }.
 * @returns the page element.
 */
function StorageCredentialsSection(props) {
  var call = props.call
  return createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '14px' } },
    createElement(StorageSection, { call: call, key: 'storage' }),
    createElement(CredentialSection, { call: call, key: 'credentials' }),
  )
}
...
function SkillsSection(props) {
  ...
  function copySlashName(name) {
    ...
    // non-secure contexts and older engines; a bare navigator.clipboard
    // branch used to make the click a silent no-op there.
  ...
  for (let si = 0; si < state.skills.length; si++) {
    // Block-scoped on purpose: a shared `var` binding made copy / open act
    // on the LAST skill in the list.
    const skill = state.skills[si]
    var headerChildren = [
...
function apply(ctx) {
  ...
  // Storage backend swap + credential refs (order 32, one page). JSON ↔
  // SQLite picker + session format panel, then the credential rows. Two
  // unrelated admin surfaces that were two nav entries until they were
  // merged — the storage page is where "where does this deployment keep its
  // state" lives, and `.credentials.yaml` is the other half of that answer.
  ctx.slots.inject('settings.section', function () {
    return ctx.slots.register({
      name: 'settings.section',
      id: 'storage-admin',
