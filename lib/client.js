...
var I18N_EN = {
  ...
  "⚠ 部分技能源本次发现未完成（宿主注册表 complete=false）— 清单可能少列若干技能，具体层见上方警告": "⚠ Some skill sources did not finish discovery this round (host registry complete=false) — the roster may be missing a few skills; see the warnings above for which layer",
...
// contracts. The dsh module loader reads only `apply` / `inject`, so the extra
// keys are inert at runtime (this half exports no `Config`).
