 ...
 *
 * Slot contributions (all assembled with the platform-shared React,
 * module-table seed word, same instance the shell renders with):
 * - 扩展插件 / 技能 / MCP服务器 / 子智能体 / 命令 / 钩子: tabs inside the
 *   shell-owned 插件 section (`settings.plugins.tab`);
 * - 自动化 (定时任务 / Webhook / 工作流) and Web 与会话 (历史会话 /
 *   Web 搜索): standalone `settings.section` pages with internal tabs.
...
var I18N_EN = {
  ...
  "三步上手：① 点「从模板开始」里的任意卡片 → ② 按需改名称和参数 → ③ 点「🚀 启动」，运行卡片实时显示每一步进度与结果。": "Three steps: ① click any template card below → ② tweak the name and args → ③ press 🚀 Start and watch each step live.",
  ...
  "暂无运行。三步上手：点上方模板卡片 → 按需改参数 → 点「🚀 启动」。也可以从「工作库」启动已保存的脚本。": "No runs yet. Three steps: click a template above → tweak the args → press 🚀 Start. Saved scripts live in the Library tab.",
...
function renderInventoryPresetsTable(presets) {
  ...
  for (var i = 0; i < presets.length; i++) {
    ...
    var headerChildren = [
      ...
      // dsh 0.1.7 dropped the trust field from the inventory: with trust null
      // the badge is omitted rather than guessing (mis)system for every preset.
      preset.trust === 'user'
...
function apply(ctx) {
  ...
  // into one nav entry, v1.25.0) / 用量仪表盘 are standalone pages, and the
  ...
  // the Web 与会话 entry as its default tab (v1.25.0 — it used to be
  ...
  // Web 与会话 (order 27 — v1.25.0 merged the standalone 历史会话 fallback
...
// SessionsSection mounts at runtime through the Web 与会话 tab, and
// WorkspacesSection registers nowhere (retired — dsh covers workspaces
// natively), so the harness mounts both directly to keep exercising the panel
// contracts. The dsh module loader reads only `apply` / `inject` / `Config`,
// so the extra keys are inert at runtime.
