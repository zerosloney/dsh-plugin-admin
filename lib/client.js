...
/**
 * Merge this plugin's workspace manager and session-history panels into dsh's
 * OWN 已归档会话 settings page (`dsh-client-ui-settings-unarchive-sessions`,
 * section id `archived-sessions`) instead of registering two more nav rows.
 *
 ...
 * into, so the panels keep their `[data-dsh-admin-section]` scoped CSS and
 * internal scrolling unchanged, and everything scrolls together with the
 * official archived list above them.
 *
 ...
 * so the merged panels would have no home. When the settings dialog's nav
 * carries no 已归档会话 row, {@link onFallback} registers the two sections the
 * plugin used to own — the panels stay reachable on every dsh version.
 *
 ...
 *
 * @param {Function} call - the /api RPC gateway call the panels drive.
 ...
 * official 已归档会话 page; registers the two standalone sections instead.
 * @returns a disposer disconnecting the observer and unmounting the panels.
 */
function setupArchivedSessionsMerge(call, refreshSessions, onFallback) {
  ...
  function sync() {
    ...
    root.render(React.createElement('div', { className: 'dsh-admin-archived-merge' },
      ...
      React.createElement(WorkspacesSection, { call: call }),
    ))
  }
