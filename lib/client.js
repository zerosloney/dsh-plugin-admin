...
// and falls back to IconSettingsOutline16), so this plugin's thirteen pages
// would all read as the same icon. The slot contract carries no icon field,
// so — same posture as the sidebar menu injection — we re-paint the nav row
// glyphs in the DOM: one distinct 16×16 outline icon per page, sharing the
// official stroke language (currentColor, 1.3 stroke, round caps/joins) so
// they sit naturally next to the stock icons.
//
// The nav row exposes only the label span (no section id / data attribute),
// so the label IS the key — which is why this plugin must not register a
// label the shell already uses: the official `ui-agent-preset` page is
// labelled 'Agent 预设' too, and a shared key would repaint that official
// row as well. Our editor is therefore labelled '预设编辑'.
//
// Keys are listed in nav order (settings.section order, then session history).
var SETTINGS_NAV_ICONS = {
  '工作区': '<path d="M2 4.4A1.4 1.4 0 0 1 3.4 3h2.4l1.4 1.7h5.4A1.4 1.4 0 0 1 14 6.1v5.5A1.4 1.4 0 0 1 12.6 13H3.4A1.4 1.4 0 0 1 2 11.6z"/>',
  ...
  '存储': '<ellipse cx="8" cy="5" rx="4.6" ry="2"/><ellipse cx="8" cy="10.4" rx="4.6" ry="2"/>',
  '预设编辑': '<path d="M3 5.4h10M3 10.6h10"/><circle cx="6.2" cy="5.4" r="1.6" fill="currentColor" stroke="none"/><circle cx="9.8" cy="10.6" r="1.6" fill="currentColor" stroke="none"/>',
  '🪝 Codex 钩子桥': '<path d="M3.2 12.4V8.6a4.8 4.8 0 0 1 9.6 0v3.8"/><path d="M1.4 12.4h3.6M11 12.4h3.6"/>',
  ...
  function reloadInventory() {
    ...
    callRemote('pluginInventoryAdmin/list', {}).then(function (result) {
      ...
      if (result.ok) {
        ...
        patchPlugin({ inventoryLoading: false, inventoryError: '加载 Loader 状态失败：' + messageOf(result.error) })
      }
    }, function (failure) {
      if (!alive.current) return
      patchPlugin({ inventoryLoading: false, inventoryError: '调用失败：' + messageOf(failure) })
    })
  }
  ...
  function upgradeAllPlugins() {
    ...
    var runNext = function () {
      ...
      if (index >= targets.length) {
        ...
        bulkNoteRef.current = '✅ 批量更新完成：' + (targets.length - failedTotal) + ' 个已更新' + (failedTotal > 0
          ? '，' + failedTotal + ' 个失败：' + failures.map(function (row) {
      ...
      if (result.ok) {
        ...
        patchPlugin({
          ...
          note: value.state === 'present'
            ? (disabled ? '该插件已处于停用状态' : '该插件已处于启用状态')
            : (disabled
              ? '已写入 ' + rows + ' 行停用标记 — 重启 dsh 后该插件不再挂载；点「启用」可恢复'
              : '已移除停用标记 — 重启 dsh 后插件恢复挂载'),
          profileDir: value.profileDir || '',
          plugins: value.plugins || [],
        })
        return
      }
      var failMessage = (disabled ? '停用' : '启用') + '失败：' + messageOf(result.error)
      patchPlugin({ busy: false, error: failMessage })
      showToast('error', failMessage)
      reloadPlugins(true)
    }, function (failure) {
...
function SessionsSection(props) {
  ...
  function enableSearch() {
    ...
    callRemote('overlayAdmin/searchEnable', {}).then(function (result) {
      ...
      if (result.ok) {
        ...
        patchSession({ searchEnableState: null, error: '启用全文检索失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (alive.current) patchSession({ searchEnableState: null, error: '启用全文检索失败：' + messageOf(err) })
    })
  }
        ...
        try {
          ...
          showToast(
            ...
            result.value.truncated === true
              ? '⚠️ 已导出（会话事件超出上限，文件已截断）'
              : '✅ 已导出 ' + (result.value.messages || 0) + ' 条消息',
          )
        } catch (e) {
  ...
  return createElement('div', { 'data-dsh-admin-section': '' },
    renderSessionsView(sView, patchSession, reloadSessions, actSession, exportSession, pinnedIds, togglePinned, runFulltext, loadHealth, enableSearch))
}
...
function renderInventoryPanel(view, patch, reloadInventory) {
  ...
  if (inventory === null) {
    if (view.inventoryError !== '') {
      return createElement('div', { className: 'group-header', key: 'inventory', style: { marginTop: '12px', borderTop: '1px dashed var(--dsw-alias-border-l1, #e5e7eb)', paddingTop: '10px' } },
        createElement('span', { className: 'group-title', key: 't' }, '📡 Loader 运行时'),
    ...
    return createElement('div', { className: 'group-header', key: 'inventory', style: { marginTop: '12px', borderTop: '1px dashed var(--dsw-alias-border-l1, #e5e7eb)', paddingTop: '10px' } },
      createElement('span', { className: 'group-title', key: 't' }, '📡 Loader 运行时'),
      createElement('span', { className: 'group-count', key: 'c' }, view.inventoryLoading ? '加载中…' : '等待首次快照')
    )
  }
    ...
    return createElement('div', { className: 'group-header', key: 'inventory', style: { marginTop: '12px', borderTop: '1px dashed var(--dsw-alias-border-l1, #e5e7eb)', paddingTop: '10px' } },
      ...
      callFailed
        ? createElement('span', { className: 'group-count', key: 'c', style: { color: 'var(--dsw-alias-fg-error, #b91c1c)' } },
          '⚠ 读取 Loader 快照失败' + (typeof inventory.error === 'string' && inventory.error !== '' ? '：' + inventory.error : ''))
        : createElement('span', { className: 'group-count', key: 'c' }, '本部署未挂载 dsh-host-plugin-inventory'),
      callFailed
        ? createElement('button', {
          type: 'button', key: 'btn-retry-inventory', className: 'btn sm',
          onClick: function () { reloadInventory() },
        ...
        : createElement('span', { className: 'hint', key: 'hint', style: { marginLeft: 'auto', fontSize: '11px', color: 'var(--dsw-alias-fg-muted, #6b7280)' } },
          'web-app 编辑器已自带此服务；CLI 模式可装一个 web 配置以查看 Fiber phase')
    )
  }
  ...
  var failedChip = createElement('span', {
    ...
    key: 'failed',
    title: '有 ' + counts.failed + ' 个 Loader 项加载失败，详见展开表',
  }, '⚠ 失败 ' + counts.failed)
  var summaryChildren = [
    createElement('span', { key: 'active', style: { marginRight: '8px' } }, '🟢 ' + counts.active + ' 已激活'),
    createElement('span', { key: 'none', style: { marginRight: '8px' } }, '⚪ ' + counts.none + ' 未挂载'),
    createElement('span', { key: 'loading', style: { marginRight: '8px' } }, '⏳ ' + counts.loading + ' 加载中'),
    createElement('span', { key: 'pending', style: { marginRight: '8px' } }, '⏳ ' + counts.pending + ' 等待挂载'),
    createElement('span', { key: 'unloading', style: { marginRight: '8px' } }, '🟡 ' + counts.unloading + ' 卸载中'),
    failedChip,
    presets.length > 0 ? createElement('span', { key: 'presets', style: { marginLeft: '4px' } }, '· ' + presets.length + ' 个 Agent 预设') : null,
  ]
  ...
  var panelChildren = [
    createElement('div', {
      ...
      createElement('span', { className: 'group-title', key: 't' }, '📡 Loader 运行时 (' + totalEntries + ' 项)'),
  ...
  if (expanded) {
    ...
    panelChildren.push(createElement('div', { key: 'body', style: { borderLeft: '2px solid var(--dsw-alias-border-l1, #e5e7eb)', marginLeft: '8px', padding: '8px 12px' } },
      ...
      createElement('div', { key: 'footer', style: { marginTop: '6px', fontSize: '11px', color: 'var(--dsw-alias-fg-muted, #6b7280)' } },
        '📌 此为运行时快照 — 反映当前 dsh 进程的 Loader 状态（重启后由 profile patch 重新装配）。',
  ...
  for (var i = 0; i < ranked.length; i++) {
    ...
    rows.push(createElement('div', {
      ...
      entry.enabled
        ...
        : createElement('span', { className: 'tag update-error', key: 'en', title: '有效 enablement = false（包含被祖先组停用）' }, 'disabled'),
      renderFiberPhaseBadge(entry.fiberPhase),
    ))
  }
  if (rows.length === 0) {
    rows.push(createElement('div', { key: 'empty', style: { padding: '8px', color: 'var(--dsw-alias-fg-muted, #6b7280)' } }, '暂无 Loader 项（结构组行已跳过）'))
  }
...
function renderFiberPhaseBadge(phase) {
  switch (phase) {
    case 'active':
      return createElement('span', { className: 'tag plugin', key: 'ph', title: 'Fiber phase: active（根 Fiber 已激活）' }, '🟢 active')
    case 'failed':
      return createElement('span', { className: 'tag update-error', key: 'ph', title: 'Fiber phase: failed（apply 抛错，需修复）' }, '🔴 failed')
    case 'pending':
      return createElement('span', { className: 'tag version', key: 'ph', title: 'Fiber phase: pending（等待挂载）' }, '⏳ pending')
    case 'loading':
      return createElement('span', { className: 'tag version', key: 'ph', title: 'Fiber phase: loading（正在读取/apply）' }, '⏳ loading')
    case 'unloading':
      return createElement('span', { className: 'tag update-error', key: 'ph', title: 'Fiber phase: unloading（正在拆解）' }, '🟡 unloading')
    default:
      return createElement('span', { className: 'tag', key: 'ph', title: '无 root Fiber（结构组行 / 已移除 / 从未挂载）' }, '⚪ unmounted')
  }
}
...
function renderInventoryPresetsTable(presets) {
  ...
  for (var i = 0; i < presets.length; i++) {
    ...
    var headerChildren = [
      ...
      preset.trust === 'user'
        ? createElement('span', { className: 'tag plugin', key: 'trust' }, '用户预设')
        : createElement('span', { className: 'tag', key: 'trust' }, '系统预设'),
      preset.isDefault === true
        ? createElement('span', { className: 'tag update', key: 'default' }, '默认')
        : null,
    ]
    ...
    sections.push(createElement('div', { key: 'preset-' + preset.id, style: { marginTop: '8px' } },
      createElement('div', { key: 'hdr', className: 'group-header', style: { padding: '4px 0' } },
        headerChildren,
        createElement('span', { className: 'group-count', key: 'count', style: { marginLeft: 'auto' } }, rowCount + ' 行'),
      ),
      typeof preset.broken === 'string' && preset.broken !== ''
        ? createElement('div', { key: 'broken', className: 'error', style: { marginTop: '4px' } }, '⚠ 预设文件解析失败：' + preset.broken)
...
function renderPresetRowsTable(rows) {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    var enabledBadge = row.enabled === true
      ...
      : (row.enabled === 'conditional'
        ? createElement('span', { className: 'tag version', key: 'en', title: '含 !!js 条件表达式 — 需 Loader 上下文才能判定' }, 'conditional')
  ...
  if (rendered.length === 0) {
    rendered.push(createElement('div', { key: 'empty', style: { padding: '8px', color: 'var(--dsw-alias-fg-muted, #6b7280)' } }, '此预设暂无组成行'))
  }
...
function renderPluginCard(plugin, view, remove, patch, upgrade, setEnabled) {
  ...
  if (plugin.disabled) {
    titleChildren.push(createElement('span', {
      ...
      title: 'profile patch 中已写入该插件的停用行 — 重启 dsh 后不再挂载；点「启用」恢复',
    }, '⏸ 已停用'))
  }
    ...
    if (plugin.disablable && typeof setEnabled === 'function') {
      actionChildren.push(createElement('button', {
        ...
        title: plugin.disabled
          ? '移除 profile patch 中的停用行（重启 dsh 后恢复挂载）'
          : '写入停用行到 profile patch（不卸载、保留配置；重启 dsh 后不再挂载）',
...
function UsageDashboardSection(props) {
  var call = props.call
  var kit = sectionState({ open: true, loading: true, rows: [], range: '30d', project: '' })
  var usage = kit.state
    ...
    children.push(h('div', { key: 'disabled-banner', style: { display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '8px', padding: '8px 11px', borderRadius: '8px', border: '1px solid rgba(217,119,6,0.35)', background: 'rgba(217,119,6,0.08)', color: '#b45309', fontSize: '12px', lineHeight: 1.5 } },
      h('span', { key: 't', style: { flex: '1 1 220px' } },
        view.searchEnableState === 'pending'
          ? '✅ 已写入 profile 配置（持久索引 + 首次搜索时打开）— 重启 dsh 后全文检索生效。'
          : '全文检索在此部署中默认关闭（官方 base 配置 openAt: never）。一键写入 profile 覆盖行：索引落在 $DSH_HOME 下、首次搜索时才打开，不拖慢启动；写入后需重启 dsh。'),
      view.searchEnableState === null
        ? h('button', { key: 'go', type: 'button', className: 'btn sm primary', onClick: enableSearch }, '⚡ 一键启用')
...
function renderSessionsView(view, patch, reload, act, onExport, pinnedIds, togglePinned, runFulltext, loadHealth, enableSearch) {
  var elements = []
...
function RuntimeSubagentsPanel(props) {
  ...
  var sendFollowup = function (agent, delivery) {
    ...
    call('subagentAdmin/runtimePrompt', {
      ...
      setToast(delivery === 'steer' ? '已插队投递续接消息' : '已排队投递续接消息')
      setFollowup({ childId: null, text: '', busy: false })
    }).catch(function (error) {
      setToast('续接失败：' + String((error && error.message) || error))
      ...
      if (followupOpen) {
        cardExtra.push(createElement('div', { className: 'card followup-editor', key: 'followup', style: { margin: '0 12px 10px 12px', padding: '8px' } },
          createElement('textarea', {
            ...
            rows: 3,
            autoFocus: true,
            placeholder: '给这个子智能体发一条续接消息（排队到它的下一轮，或插队到当前步）…',
            value: followup.text,
          ...
          createElement('div', { key: 'acts', style: { display: 'flex', gap: '6px', alignItems: 'center' } },
            createElement('button', {
              ...
              title: '排队投递：在子智能体当前回合结束后作为新一轮投递',
              onClick: function () { sendFollowup(agent, 'queue') },
            }, followup.busy ? createElement(Spinner, { key: 'spin' }) : '📨 排队发送'),
            createElement('button', {
              type: 'button', key: 's', className: 'btn sm',
              disabled: followup.busy || followup.text.trim() === '',
              title: '插队投递：在最近的步骤边界插入（正在运行时不等回合结束）',
              onClick: function () { sendFollowup(agent, 'steer') },
            }, '⚡ 插队'),
            createElement('span', { key: 'hint', style: { fontSize: '11px', opacity: 0.6 } }, 'Ctrl+Enter 发送 · Esc 关闭'),
          ),
        ))
      }
          ...
          createElement('div', { className: 'card-actions' },
            isContinuable
              ? createElement('button', {
                ...
                title: '给这个运行中的子智能体发一条续接消息（父会话在线时投递）',
...
var SCHEDULE_CSS_TEXT = [
  ...
  // Capability-absent offer: slim one-time enablement card (schedule runtime off).
  '[data-dsh-admin-schedule] .schedule-offer { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; padding: 8px 14px; }',
  '[data-dsh-admin-schedule] .schedule-offer .offer-text { flex: 1 1 220px; font-size: 12px; line-height: 1.5; color: var(--dsw-alias-label-secondary, #555); }',
  '[data-dsh-admin-schedule] .schedule-offer .offer-text[data-done="true"] { color: var(--dsw-static-amber-500, #d97706); font-weight: 600; }',
  '[data-dsh-admin-schedule] .schedule-offer .offer-error { flex: 1 1 100%; font-size: 11px; color: #b91c1c; word-break: break-word; }',
].join('\n')
...
// The official web composition mounts ui-schedule disabled and ships no host
// schedule rows (see bundle/web-app/cordis.patch.yml) — the 'schedule'
// projection key is then never registered and useProjection reads undefined
// uniformly (capability absent, projection-store.ts). Offer a one-time
// enablement card when the absence persists past the baseline window; the
// official overlay composition (time-context + schedule + ui-schedule) is
// written by overlayAdmin/scheduleEnable. Dismissal is a localStorage
// preference: the host stays UI-preference-free.
var SEARCH_DISABLED_MARKER = 'SESSION_QUERY_SEARCH_DISABLED'
var SCHEDULE_OFFER_KEY = 'dsh-plugin-admin.scheduleOffer.dismissed'
var SCHEDULE_OFFER_DELAY_MS = 2000

function scheduleOfferDismissed() {
  try { return localStorage.getItem(SCHEDULE_OFFER_KEY) === '1' } catch (e) { return false }
}

function dismissScheduleOffer() {
  try { localStorage.setItem(SCHEDULE_OFFER_KEY, '1') } catch (e) { /* private mode etc. */ }
}
...
/**
 * Render the enablement card: what's missing, the ⚡ action, and the
 * post-enable 'pending restart' state. Same visual language as the dock.
 */
function ScheduleOfferRender(ui) {
  return createElement('section', { 'data-dsh-admin-schedule': '', 'aria-label': '日程未启用' },
    createElement('div', { className: 'schedule-offer' },
      createElement('span', { 'aria-hidden': 'true' }, '⏰'),
      createElement('span', { className: 'offer-text', 'data-done': ui.done ? 'true' : 'false' },
        ui.done
          ? '✅ 已写入官方 Schedule overlay（time-context + schedule + ui-schedule）— 重启 dsh 后日程提醒生效。'
          : '日程提醒在官方 web 编成中默认关闭（需要 Schedule overlay）。一键写入 overlay 行即可启用，重启 dsh 生效。'),
      !ui.done
        ? createElement('button', {
          type: 'button',
          ...
          disabled: ui.busy,
          onClick: ui.onEnable,
        }, ui.busy ? createElement('span', { className: 'spinner' }) : '⚡ 一键启用')
        : null,
      !ui.done
        ? createElement('button', {
          type: 'button',
          ...
          'aria-label': '不再提示',
          title: '不再提示',
          onClick: ui.onDismiss,
        }, '✕')
        : null,
      ui.error !== '' ? createElement('span', { className: 'offer-error' }, ui.error) : null,
    ),
  )
}
 ...
 * (the enablement card rides it when the schedule capability is absent).
 * A one-second ticker keeps the relative times visibly alive.
 */
function ScheduleAdminDock(props) {
  ...
  // Capability-absent offer state. undefined (never a value frame) means the
  // host unit is unmounted; the delay dodges the pre-baseline window where
  // even mounted keys still read undefined.
  var offerState = useState(function () {
    return { ready: false, dismissed: scheduleOfferDismissed(), busy: false, done: false, error: '' }
  })
  var offer = offerState[0]
  var setOffer = offerState[1]
  var absent = typeof useProjection === 'function' && projected === undefined

  useEffect(function () {
    if (!absent) {
      setOffer(function (cur) { return cur.ready ? Object.assign({}, cur, { ready: false }) : cur })
      return undefined
    }
    var timer = setTimeout(function () {
      setOffer(function (cur) { return cur.ready ? cur : Object.assign({}, cur, { ready: true }) })
    }, SCHEDULE_OFFER_DELAY_MS)
    return function () { clearTimeout(timer) }
  }, [absent])

  function enableSchedule() {
    if (offer.busy || offer.done) return
    setOffer(function (cur) { return Object.assign({}, cur, { busy: true, error: '' }) })
    call('overlayAdmin/scheduleEnable', {}).then(function (result) {
      if (result.ok) {
        dismissScheduleOffer()
        setOffer(function (cur) { return Object.assign({}, cur, { busy: false, done: true }) })
      } else {
        setOffer(function (cur) { return Object.assign({}, cur, { busy: false, error: '启用失败：' + messageOf(result.error) }) })
      }
    }, function (err) {
      setOffer(function (cur) { return Object.assign({}, cur, { busy: false, error: '启用失败：' + messageOf(err) }) })
    })
  }
  ...
  // Capability absent (runtime off): the one-time enablement card. The card
  // needs the injected call — an old shell without it degrades to silence.
  if (absent && offer.ready && !offer.dismissed && !offer.done && typeof call === 'function') {
    return ScheduleOfferRender({
      busy: offer.busy,
      done: offer.done,
      error: offer.error,
      onEnable: enableSchedule,
      onDismiss: function () { dismissScheduleOffer(); setOffer(function (cur) { return Object.assign({}, cur, { dismissed: true }) }) },
    })
  }
  if (absent && offer.done) {
    return ScheduleOfferRender({ busy: false, done: true, error: '', onEnable: function () {}, onDismiss: function () {} })
  }
  return null
}
  ...
  function testRule(id) {
    ...
    callRemote('webhookAdmin/testRule', { id: id }).then(function (result) {
      ...
      if (result.ok && v.ok) {
        ...
        patch({ error: '触发测试失败：' + (messageOf(v.error || result.error) || '未知错误') })
      }
    }, function (err) {
      if (!alive.current) return
...
function WorkspacesSection(props) {
  ...
  function pickDirectory() {
    call('workspaceAdmin/pickDirectory', {}).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ createError: '目录选择失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ createError: '目录选择失败：' + messageOf(err) })
    })
  }
  ...
  function submitCreate() {
    ...
    if (path === '') {
      patch({ createError: '请提供目录路径（原生选择或手动输入）' })
      return
    }
    ...
    call('workspaceAdmin/create', args).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ createBusy: false, createError: '新建失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ createBusy: false, createError: '新建失败：' + messageOf(err) })
    })
  }
  ...
  function submitRename() {
    ...
    call('workspaceAdmin/rename', { workspaceId: id, title: title }).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ renameBusy: false, renameError: '重命名失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ renameBusy: false, renameError: '重命名失败：' + messageOf(err) })
    })
  }
  ...
  function moveWorkspace(id, beforeId) {
    ...
    call('workspaceAdmin/insertBefore', args).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ busy: false, error: '排序失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busy: false, error: '排序失败：' + messageOf(err) })
    })
  }
  ...
  function confirmDelete() {
    ...
    call('workspaceAdmin/delete', { workspaceId: id }).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ deleteBusy: false, deleteError: '删除失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ deleteBusy: false, deleteError: '删除失败：' + messageOf(err) })
    })
  }
    ...
    call('workspaceAdmin/status', { workspaceId: id }).then(function (result) {
      ...
      if (result.ok) {
        ...
        if (status === 'missing-dir') {
          patch({ error: '⚠ 该工作区的目录当前不存在（可能临时移走）；registry 不会改写记录' })
        } else {
          patch({ error: '' })
        }
      } else {
        patch({ error: '状态检查失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ error: '状态检查失败：' + messageOf(err) })
    })
  }
  ...
  if (!state.available) {
    return createElement('div', { 'data-dsh-admin-section': '' },
      createElement('div', { className: 'group-header', key: 'unavail' },
        createElement('span', { className: 'group-title', key: 't' }, '📁 工作区'),
        createElement('span', { className: 'group-count', key: 'c' }, '本部署未挂载 dsh-workspace（仅 web-app 编成自带）'),
      ),
    )
  }
  ...
  // Toolbar
  var toolbarChildren = [
    createElement('button', { type: 'button', className: 'btn primary', key: 'btn-create', disabled: state.createBusy, onClick: openCreate }, '➕ 新建工作区'),
    createElement('button', { type: 'button', className: 'btn', key: 'btn-refresh', disabled: state.busy, onClick: reload }, '⟳ 刷新'),
  ]
  if (state.archivedSessionIds.length > 0) {
    toolbarChildren.push(createElement('button', {
      ...
      disabled: state.busy,
      title: '一键取消归档全部已归档会话（不删除会话本体）',
      onClick: bulkUnarchive,
    }, '📦 取消全部归档 (' + state.archivedSessionIds.length + ')'))
  }
  ...
  if (state.createOpen) {
    var pickerHint = state.pickerAvailable
      ? (state.pickerBackend ? '原生选择器后端：' + state.pickerBackend : '原生选择器可用')
      : '原生选择器不可用，请手动输入绝对路径'
    elements.push(createElement('div', { className: 'card mcp-editor', key: 'create-form', style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
      createElement('div', { key: 'hdr', style: { display: 'flex', gap: '8px', alignItems: 'baseline' } },
        createElement('span', { style: { fontWeight: 600 } }, '新建工作区'),
        createElement('span', { style: { fontSize: '11px', opacity: 0.65 } }, pickerHint),
      ),
      createElement('div', { key: 'row-path', style: { display: 'flex', gap: '6px' } },
        createElement('button', { type: 'button', className: 'btn sm', key: 'pick', disabled: !state.pickerAvailable, onClick: pickDirectory }, '📁 选择目录'),
        createElement('input', { className: 'input', key: 'path', placeholder: '或手动输入绝对目录路径', value: state.createPath, onChange: function (e) { setCreateField('path', e.target.value) } }),
      ),
      createElement('input', { className: 'input', key: 'title', placeholder: '显示标题（留空则用目录最后一段）', value: state.createTitle, onChange: function (e) { setCreateField('title', e.target.value) } }),
      state.createError !== ''
      ...
      createElement('div', { key: 'actions', style: { display: 'flex', gap: '6px' } },
        createElement('button', { type: 'button', className: 'btn sm primary', key: 'save', disabled: state.createBusy || state.createPath.trim() === '', onClick: submitCreate }, state.createBusy ? '创建中…' : '保存'),
        createElement('button', { type: 'button', className: 'btn sm', key: 'cancel', disabled: state.createBusy, onClick: closeCreate }, '取消'),
      ),
    ))
  }
  ...
  if (state.workspaces.length === 0 && !state.createOpen) {
    elements.push(createElement('div', { className: 'empty', key: 'empty' }, '暂无工作区 — 点「➕ 新建工作区」选定一个目录作为项目根'))
  }
  for (let i = 0; i < state.workspaces.length; i++) {
    ...
    var headerChildren = [
      createElement('span', { className: 'card-title-text', key: 'title', title: ws.workspaceId }, isRenaming
        ? createElement('span', { style: { display: 'inline-flex', gap: '6px' } },
            ...
            createElement('button', { type: 'button', className: 'btn sm primary', key: 'rename-save', disabled: state.renameBusy, onClick: submitRename }, '保存'),
            createElement('button', { type: 'button', className: 'btn sm', key: 'rename-cancel', disabled: state.renameBusy, onClick: cancelRename }, '取消'),
          )
        : ws.title || ws.path),
    ...
    var actionChildren = []
    if (!isRenaming) {
      actionChildren.push(createElement('button', { type: 'button', className: 'btn sm', key: 'btn-rename', onClick: function () { openRename(ws.workspaceId, ws.title) } }, '✎ 重命名'))
      actionChildren.push(createElement('button', { type: 'button', className: 'btn sm', key: 'btn-status', title: '检查目录是否仍存在（不修改记录）', onClick: function () { checkStatus(ws.workspaceId) } }, '🔎 检查状态'))
      ...
      actionChildren.push(createElement('button', { type: 'button', className: 'btn sm danger', key: 'btn-del', onClick: function () { askDelete(ws.workspaceId) } }, isDeleting ? '✕' : '删除'))
    }
    ...
    var cardChildren = [
      ...
      isDeleting
        ? createElement('div', { className: 'confirm-bar', key: 'confirm' },
            createElement('span', { className: 'confirm-text' }, '确定删除工作区「', ws.title || ws.path, '」？（仅删除注册，目录与会话本体保留）'),
            createElement('span', { className: 'confirm-actions' },
              createElement('button', { type: 'button', className: 'btn sm', onClick: cancelDelete, key: 'cancel' }, '取消'),
              createElement('button', { type: 'button', className: 'btn sm danger-solid', disabled: state.deleteBusy, onClick: confirmDelete, key: 'confirm-btn' }, state.deleteBusy ? '删除中…' : '删除'),
            ))
        : null,
      createElement('div', { className: 'card-sub', key: 'sub' },
        createElement('span', { className: 'card-sub-item' }, '📊 ' + (Array.isArray(ws.sessionIds) ? ws.sessionIds.length : 0) + ' 个会话'),
        ws.createdAt ? createElement('span', { className: 'card-sub-item', style: { marginLeft: '12px', opacity: 0.7 } }, '创建于 ' + ws.createdAt.slice(0, 10)) : null,
        ws.updatedAt ? createElement('span', { className: 'card-sub-item', style: { marginLeft: '12px', opacity: 0.7 } }, '更新于 ' + ws.updatedAt.slice(0, 10)) : null,
      ),
    ]
...
/**
 * Storage administration settings section.
 *
 * Surfaces dsh's two storage backends (JSON default + SQLite opt-in)
 * as a radio-card picker, and exposes the installed session format
 * version + auto-migrate chain so users see what's actually on disk.
 *
 * The panel mutates `storage-domain.config.backend` in place — single
 * key change preserves the row's comments and any user-added config.
 * The sqlite row lands on `storages/state.db` by default; the user
 * hand-edits the row afterwards to point at a different path.
 */
function StorageSection(props) {
  ...
  function reload() {
    patch({ busy: true, error: '' })
    call('storageAdmin/list', {}).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        var v = result.value || {}
        patch({
          busy: false,
          backends: Array.isArray(v.backends) ? v.backends : [],
  ...
  function selectBackend(id) {
    ...
    if (cur !== null && cur.backend === id) return
    patch({ busyId: id, error: '' })
    call('storageAdmin/swap', { backendId: id }).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        patch({ busyId: '', active: result.value || { backend: id } })
      } else {
        patch({ busyId: '', error: '切换失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busyId: '', error: '切换失败：' + messageOf(err) })
    })
  }

  function installBackend(id) {
    patch({ busyId: id, error: '' })
    call('storageAdmin/install', { backendId: id }).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        patch({ busyId: '' })
        reload()
      } else {
        patch({ busyId: '', error: '安装失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busyId: '', error: '安装失败：' + messageOf(err) })
    })
  }
  ...
  elements.push(createElement('div', { className: 'hint', key: 'restart-banner',
    style: { background: 'var(--dsw-alias-bg-elevated, rgba(245,245,250,0.5))', padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--dsw-alias-border-subtle, rgba(200,200,210,200,200,0.4))' },
  },
    '🔁 切换 / 安装 storage backend 后需**重启 dsh**（默认 sqlite 数据库落在 `~/.dsh/storages/state.db`，可在 patch 里改 `path`）',
  ))
  if (state.busy) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'busy' },
      createElement('span', { className: 'spinner', key: 'sp' }), '加载中…'))
  ...
  // Backend cards
  var activeId = state.active === null ? null : state.active.backend
  for (var i = 0; i < state.backends.length; i++) {
    var b = state.backends[i]
    // The live selection, not the list() snapshot: after a successful swap the
    // snapshot still says `json`, and a controlled radio reading it snapped
    // the user's choice back right after it was written.
    var isActive = activeId === b.id
    var isInstalled = b.installed
    var isBundled = b.bundled
    var isBusy = state.busyId === b.id
    ...
    var radioChildren = [
      createElement('label', { key: 'radio-row', style: { display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' } },
        createElement('input', {
          ...
          checked: isActive,
          disabled: isBusy,
          onChange: function () { selectBackend(b.id) },
          key: 'radio',
        }),
        createElement('span', { style: { fontWeight: 600 }, key: 'name' }, b.label),
        isBundled
          ? createElement('span', { className: 'tag', key: 'bundled', title: 'dsh 默认 backend，无法从此面板卸载' }, '🛡 dsh 默认')
          : null,
        isInstalled
          ? createElement('span', { className: 'tag plugin', key: 'installed' }, '✓ 已装')
          : createElement('span', { className: 'tag', key: 'not-installed' }, '📦 未装'),
      ),
    ]
    var subChildren = [
      createElement('div', { key: 'pkg', className: 'card-sub', style: { fontFamily: 'monospace', fontSize: '11px' } }, b.packageName),
      createElement('div', { key: 'desc', className: 'card-sub' }, b.description),
    ]
    var actionChildren = []
    if (!isInstalled && !isBundled) {
      actionChildren.push(createElement('button', {
        type: 'button', className: 'btn sm primary', key: 'install',
        disabled: isBusy, onClick: function () { installBackend(b.id) },
      }, isBusy ? '安装中…' : '📥 安装'))
    }
  ...
  if (state.format !== null) {
    var f = state.format
    var chainText = f.autoMigrateChain.join(' → ')
    elements.push(createElement('div', { className: 'card', key: 'format',
      style: { background: 'var(--dsw-alias-bg-elevated, rgba(245,245,250,0.5))' } },
      createElement('div', { className: 'card-header', key: 'h' },
        createElement('span', { className: 'card-title', key: 'title-row' },
          createElement('span', { style: { fontWeight: 600 } }, '📐 会话格式版本'),
          createElement('span', { className: 'tag plugin', key: 'current', style: { marginLeft: '6px' } }, '当前 v' + f.installedVersion),
        ),
      ),
      createElement('div', { className: 'card-sub', key: 'chain', style: { fontFamily: 'monospace', fontSize: '12px' } }, '迁移链：' + chainText),
      createElement('div', { className: 'card-sub', key: 'hint',
        style: { fontSize: '11px', opacity: 0.7, marginTop: '4px' } },
        '💡 旧版本会话在**下次被访问时**自动通过 `session-format-catalog` 升级；panel 不提供手动触发（迁移是 dsh 启动加载期的稳定路径）',
      ),
    ))
  }

  if (state.backends.length === 0 && !state.busy) {
    elements.push(createElement('div', { className: 'empty', key: 'empty' }, '加载 backend 列表失败 — 检查 dsh 是否在运行'))
  }
 ...
 *
 * Restart dsh for any change to take effect — the panel surfaces this
 * at the top as a one-line banner.
 */
function WebSearchSection(props) {
  ...
  function uninstallProvider(id) {
    ...
    call('webSearchAdmin/uninstall', { providerId: id }).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({ busyId: '', error: '卸载失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busyId: '', error: '卸载失败：' + messageOf(err) })
    })
  }
  ...
  var elements = []
  elements.push(createElement('div', { className: 'hint', key: 'restart-banner',
    style: { background: 'var(--dsw-alias-bg-elevated, rgba(245,245,250,0.5))', padding: '8px 12px', borderRadius: '8px', border: '1px solid var(--dsw-alias-border-subtle, rgba(200,200,210,0.4))' } },
    '🔁 切换 provider / 安装 / 卸载 后需**重启 dsh** 生效（关闭 dsh 进程后重新运行即可）',
  ))
  if (state.busy) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'busy' },
      createElement('span', { className: 'spinner', key: 'sp' }), '加载中…'))
  ...
  for (var i = 0; i < state.providers.length; i++) {
    ...
    // Live selection, not the list() snapshot (same hazard as storageAdmin).
    ...
    var radioChildren = [
      createElement('label', { key: 'radio-row', style: { display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' } },
        ...
        isBundled
          ? createElement('span', { className: 'tag', key: 'bundled', title: 'dsh 默认 provider，无法从此面板卸载' }, '🛡 dsh 默认')
          : null,
        createElement('span', { className: envClass, key: 'env', title: 'provider 通过此环境变量读取 API key（launch env, 不存 plugin）' }, '🔑 ' + p.envVar),
      ),
    ]
    var subChildren = [
      ...
      isInstalled
        ? null
        : createElement('div', { key: 'not-installed', className: 'card-sub', style: { color: 'var(--dsw-alias-fg-warning, #b45309)' } },
          '📦 此 provider 未安装 — 需要点「📥 安装」按钮调 `pnpm add` 把 npm 包加进 profile'),
    ]
    ...
    elements.push(createElement('div', {
      ...
      subChildren.some(Boolean) ? createElement('div', { key: 'body' }, subChildren) : null,
    ))
  }

  if (state.providers.length === 0 && !state.busy) {
    elements.push(createElement('div', { className: 'empty', key: 'empty' }, '加载 provider 列表失败 — 检查 dsh 是否在运行'))
  }
 ...
 *
 * Wraps the host's `ctx.sessionSkillCatalog` (`@deepseek-ai/dsh-api-session-controller`)
 * with a settings page that lists one Session's user-invocable skills.
 * The catalog is Session-addressed (cwd + agent preset scope select the
 * visible set), so the panel needs a Session identity to read against —
 * we pre-populate from `sessionAdmin/list()` and let the user switch via
 * a dropdown. The web-app bundle mounts the catalog by default; CLI /
 * headless deployments render a quiet "skills unavailable" hint.
 *
 * Out of scope for v1: skill authoring. SKILL.md files are managed on
 * the filesystem (the `projectAdmin/` sub-agent's `.agents/skills/`
 * inspection shows their filesystem presence); the panel only enumerates
 * the catalog. The path field, when present, opens the SKILL.md directory
 * via `fsAdmin/reveal`.
 */
function SkillsSection(props) {
  ...
  var kit = sectionState({
    ...
    sessionsBusy: false,
    sessionsError: '',
    copiedName: '', // transient confirmation of a copy click
  })
  var state = kit.state
  var setState = kit.set
  var alive = kit.alive
  // Identity of the newest skills request: a slow answer for a session the
  // user already switched away from must not overwrite the current list
  // (the dropdown would show session B while the cards show A's skills).
  var skillsRequest = useRef('')
  ...
  function reloadSkills() {
    var sid = state.selectedSessionId
    if (sid === '') {
      skillsRequest.current = ''
      patch({ skills: [], error: '' })
      return
    }
    skillsRequest.current = sid
    patch({ busy: true, error: '' })
    call('skillsAdmin/list', { sessionId: sid }).then(function (result) {
      if (!alive.current) return
      // A late answer for a session the user already switched away from must
      // not replace the current list (the dropdown would name session B while
      // the cards showed A's skills).
      if (skillsRequest.current !== sid) return
      if (result.ok) {
        var v = result.value || {}
        patch({
          busy: false,
        ...
        patch({ busy: false, error: '加载技能失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      if (skillsRequest.current !== sid) return
      patch({ busy: false, error: '加载技能失败：' + messageOf(err) })
    })
  }

  function reloadSessions() {
    patch({ sessionsBusy: true, sessionsError: '' })
    call('sessionAdmin/list', {}).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        var v = result.value || {}
        var sessions = Array.isArray(v.sessions) ? v.sessions : []
        // Pick the first non-archived session; fall back to the very
        // first session (so the panel always has something to read).
        var pick = ''
        for (var i = 0; i < sessions.length; i++) {
          if (!sessions[i].archived) { pick = sessions[i].sessionId || sessions[i].id || ''; break }
        }
        if (pick === '' && sessions.length > 0) {
          pick = sessions[0].sessionId || sessions[0].id || ''
        }
        patch({ sessionsBusy: false, sessions: sessions, selectedSessionId: pick })
      } else {
        patch({ sessionsBusy: false, sessionsError: '加载会话失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ sessionsBusy: false, sessionsError: '加载会话失败：' + messageOf(err) })
    })
  }

  kit.mount(reloadSessions)

  // Re-fetch skills whenever the selected session changes (and we have
  // a session to read against). The dependency-on-state closure pattern
  // below intentionally mirrors what SessionsSection does so a future
  // reader recognizes the shape.
  var lastFetchedSession = useRef('')
  useEffect(function () {
    if (!alive.current) return
    if (state.selectedSessionId === '') return
    if (lastFetchedSession.current === state.selectedSessionId) return
    lastFetchedSession.current = state.selectedSessionId
    reloadSkills()
  }, [state.selectedSessionId])

  function selectSession(id) {
    if (id === state.selectedSessionId) return
    patch({ selectedSessionId: id, skills: [], error: '', copiedName: '' })
  }
  ...
  function copySlashName(name) {
    ...
    if (typeof navigator !== 'undefined' && navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(function () {
        ...
        // Silent fallback: do nothing — the panel still shows the
        // `/<name>` text in the title attribute so users can copy it.
      })
    }
  }
  ...
  function openPath(path) {
    if (!path) return
    call('fsAdmin/reveal', { path: path }).then(function () { /* best-effort */ }, function () { /* best-effort */ })
  }
  ...
  if (!state.available) {
    return createElement('div', { 'data-dsh-admin-section': '' },
      createElement('div', { className: 'group-header', key: 'unavail' },
        createElement('span', { className: 'group-title', key: 't' }, '📚 技能'),
        createElement('span', { className: 'group-count', key: 'c' }, '本部署未挂载 dsh-session-skill-catalog（仅 web-app 编成自带）'),
      ),
    )
  }

  var elements = []
  if (state.sessionsBusy) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'sessions-busy' },
      createElement('span', { className: 'spinner', key: 'sp' }), '加载会话列表…'))
  }
  if (state.sessionsError !== '') {
    elements.push(createElement('div', { className: 'error', key: 'sessions-err' }, state.sessionsError))
  }
  if (state.busy) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'busy' },
      createElement('span', { className: 'spinner', key: 'sp' }), '加载技能…'))
  }
  if (state.error !== '') {
    elements.push(createElement('div', { className: 'error', key: 'err' }, state.error))
  }
  ...
  // Toolbar
  var toolbarChildren = [
    createElement('button', { type: 'button', className: 'btn', key: 'btn-refresh', disabled: state.busy || state.sessionsBusy || state.selectedSessionId === '', onClick: reloadSkills }, '⟳ 刷新'),
  ]
  elements.push(createElement('div', { className: 'toolbar', key: 'toolbar' }, toolbarChildren))
  ...
  if (state.sessions.length > 0) {
    var sessionOptions = state.sessions.map(function (s) {
      ...
      var title = s.title || s.summary || id
      var archived = s.archived === true
      // README: an archived session is labelled but NOT selectable (its skills
      // are historical). Marking it disabled is what makes the label honest —
      // a selectable "(已归档)" option let the user read a stale roster.
      return createElement('option', { key: id, value: id, disabled: archived }, title + (archived ? ' (已归档)' : ''))
    })
    elements.push(createElement('div', { className: 'toolbar', key: 'session-picker', style: { alignItems: 'center' } },
      createElement('span', { key: 'lbl', style: { fontSize: '12px', fontWeight: 600 } }, '关联会话：'),
      createElement('select', {
        key: 'select',
        className: 'input',
        style: { flex: 1 },
        value: state.selectedSessionId,
        disabled: state.busy || state.sessionsBusy,
        onChange: function (e) { selectSession(e.target.value) },
      }, sessionOptions),
      createElement('span', { key: 'cnt', style: { fontSize: '11px', opacity: 0.65 } }, String(state.sessions.length) + ' 个会话'),
    ))
  } else if (!state.sessionsBusy && state.sessionsError === '') {
    elements.push(createElement('div', { className: 'empty', key: 'no-sessions' }, '暂无会话 — 先在 dsh 跑至少一个会话后再查看技能'))
  }

  // Skills list
  if (state.selectedSessionId !== '' && state.skills.length === 0 && !state.busy && state.error === '') {
    elements.push(createElement('div', { className: 'empty', key: 'empty-skills' },
      '该会话暂无可用技能（cwd 或 agent preset 没有命中任何 skill 源）'))
  }
  for (var si = 0; si < state.skills.length; si++) {
    var skill = state.skills[si]
    var headerChildren = [
      createElement('span', { className: 'card-title-text', key: 'name', title: '/' + skill.name }, '/' + skill.name),
      skill.modelInvocable
        ? createElement('span', { className: 'tag plugin', key: 'model', title: '该技能同时暴露给模型（model 可以主动调用）' }, '🤖 模型可调用')
        : createElement('span', { className: 'tag', key: 'user', title: '仅人类可在 composer 用 /name 调用' }, '👤 仅人类'),
      state.copiedName === skill.name
        ? createElement('span', { className: 'tag live', key: 'copied', style: { marginLeft: '4px' } }, '✓ 已复制')
        : null,
    ]
    var actionChildren = [
      createElement('button', {
        type: 'button', className: 'btn sm', key: 'btn-copy',
        title: '复制 /' + skill.name + ' 到剪贴板（在 composer 直接粘贴即可调用）',
    ...
    if (skill.path) {
      actionChildren.push(createElement('button', {
        ...
        title: '在系统资源管理器中打开 SKILL.md 所在目录（' + skill.path + '）',
    ...
    if (skill.whenToUse) {
      subChildren.push(createElement('div', { key: 'whe', className: 'card-sub', style: { fontSize: '11px', opacity: 0.7 } }, '何时用：' + skill.whenToUse))
    }
    if (skill.path) {
      subChildren.push(createElement('div', { key: 'path', className: 'card-sub', style: { fontSize: '11px', opacity: 0.6, fontFamily: 'monospace' } }, skill.path))
    }
    elements.push(createElement('div', { className: 'card', key: 'skill-' + skill.name },
      createElement('div', { className: 'card-header', key: 'h' },
        createElement('span', { className: 'card-title', key: 'title-row' }, headerChildren),
  ...
  if (state.selectedSessionId !== '' && state.skills.length > 0) {
    elements.push(createElement('div', { key: 'hint', className: 'hint', style: { marginTop: '6px', fontSize: '11px' } },
      '📌 共 ' + state.skills.length + ' 个技能可见 — 仅枚举（' + '不加载正文、不修改 SKILL.md' + '）'))
  }
...
function WebhookRender(view, actions) {
  ...
  if (view.storagePath) {
    elements.push(createElement('div', { className: 'footer', key: 'footer' },
      ...
      createElement('span', null, 'Webhook 触发'),
    ))
  }
      ...
      createElement('div', { style: { flex: 1 } },
        createElement('div', { style: { display: 'flex', gap: '6px' } },
          createElement('input', {
            ...
            placeholder: '共享密钥（必填；编辑时留空表示保持不变）',
          }),
          createElement('button', { type: 'button', className: 'btn sm', onClick: function () { actions.toggleSecret() } }, view.showSecret ? '隐藏' : '显示'),
        ),
      ),
      createElement('div', { style: { flex: 1 } },
...
/**
 * Agent Preset administration settings section (v1.16.0).
 *
 * Lets the user author/edit/delete USER presets under
 * `<dshHome>/.agent-presets/<id>/`. Shipped presets (`cordis` / `minimal`
 * / `ptc` / `standard`) appear read-only with a 🛡 dsh 默认预设 badge
 * and refuse mutation through the panel. Each user preset carries a
 * name / description / order metadata record + one YAML composition
 * (the dsh plugin-row list the host's `AgentPresets` service consumes).
 *
 * UX flow:
 *   - **🎛 默认预设** radio cards at the top pick the active id (writes
 *     a single `config.default:` key to the `agent-presets` cordis row,
 *     preserving siblings).
 *   - **卡片列表**：each preset gets a card with metadata + actions.
 *     Shipped cards show 🛡 + 「🟢 当前默认」+ no destructive buttons.
 *     User cards show ✎ 编辑 + 🗑 删除 (with a confirm second-step).
 *   - **编辑表单**：modal-style form with metadata fields + a textarea
 *     for the composition. Save calls `agentPresetsAdmin/upsert` which
 *     re-validates shape (every entry carries `name:`) before writing
 *     both files atomically.
 *   - **`+ 新建预设`** button opens the same form with empty state.
 *
 * Restart dsh after toggling default — the host's `AgentPresets`
 * settings service reads the patch once per boot. Upserts compose at
 * the next session, not necessarily mid-session.
 */
function AgentPresetsEditorSection(props) {
  var call = props.call
  var kit = sectionState({
    presets: [],
    defaultId: '',
    busy: false,
    error: '',
    editing: null, // null | { kind: 'new', id, metadata, compositionText }
    confirmingDeleteId: '',
  })
  var state = kit.state
  var setState = kit.set
  ...
  function reload() {
    patch({ busy: true, error: '' })
    call('agentPresetsAdmin/list', {}).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        var v = result.value || {}
        patch({
          presets: Array.isArray(v.presets) ? v.presets : [],
          defaultId: typeof v.defaultId === 'string' ? v.defaultId : '',
          busy: false,
        })
      } else {
        patch({ busy: false, error: '加载失败：' + messageOf(result.error) })
  ...
  function setDefault(id) {
    if (id === state.defaultId) return
    patch({ busy: true, error: '' })
    call('agentPresetsAdmin/setDefault', { id: id }).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        patch({ busy: false, defaultId: id })
        reload()
      } else {
        patch({ busy: false, error: '设置默认失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busy: false, error: '设置默认失败：' + messageOf(err) })
    })
  }
  ...
  function startCreate() {
    patch({
      editing: {
        kind: 'new',
        id: '',
        metadata: { name: '', description: '', order: 0 },
        compositionText: '- name: \'@deepseek-ai/dsh-your-plugin\'\n',
      },
      error: '',
    })
  }
  ...
  function startEdit(preset) {
    ...
    call('agentPresetsAdmin/getDocument', { id: preset.id }).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        var v = result.value || {}
        patch({
          editing: {
            kind: 'edit',
            id: preset.id,
            metadata: v.metadata || { name: preset.id, description: '', order: 0 },
            compositionText: (v.composition && v.composition.raw) || '',
          },
          error: '',
        })
      } else {
        patch({ error: '读取失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ error: '读取失败：' + messageOf(err) })
    })
  }
  ...
  function saveDraft() {
    ...
    var id = (editing.id || '').trim()
    if (id === '') {
      patch({ error: '请填写 preset id（kebab-case，如 my-team）' })
      return
    }
    if (!/^[a-z0-9][a-z0-9-]*$/.test(id)) {
      patch({ error: 'preset id 必须匹配 ^[a-z0-9][a-z0-9-]*$' })
      return
    }
    patch({ busy: true, error: '' })
    var args = {
      ...
      payload: {
        ...
        compositionText: editing.compositionText,
      },
    }
    call('agentPresetsAdmin/upsert', args).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        patch({ busy: false, editing: null })
        reload()
      } else {
        patch({ busy: false, error: '保存失败：' + messageOf(result.error) })
      }
  ...
  function cancelEdit() {
    patch({ editing: null, error: '' })
  }

  function confirmDelete(id) {
    patch({ confirmingDeleteId: id, error: '' })
  }

  function runDelete(id) {
    if (state.confirmingDeleteId !== id) return
    patch({ busy: true, error: '' })
    call('agentPresetsAdmin/delete', { id: id }).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        patch({ busy: false, confirmingDeleteId: '' })
        reload()
      } else {
        patch({ busy: false, confirmingDeleteId: '', error: '删除失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busy: false, confirmingDeleteId: '', error: '删除失败：' + messageOf(err) })
    })
  }

  function startEditSetField(field, value) {
    var editing = state.editing
    if (editing === null) return
    if (field === 'id') {
      patch({ editing: Object.assign({}, editing, { id: value }) })
    } else {
      var nextMeta = Object.assign({}, editing.metadata)
      nextMeta[field] = field === 'order' ? Number(value) : value
      patch({ editing: Object.assign({}, editing, { metadata: nextMeta }) })
    }
  }

  function startEditSetComposition(value) {
    var editing = state.editing
    if (editing === null) return
    patch({ editing: Object.assign({}, editing, { compositionText: value }) })
  }
  ...
  elements.push(createElement('div', { className: 'hint', key: 'restart-banner',
    ...
    '🔁 切换默认预设 / 保存新增预设后需**重启 dsh** —— AgentPresets 在启动期从 patch 读取默认值，保存即生效，下次启动即用',
  ))
  elements.push(createElement('div', { key: 'role-hint', className: 'card-sub', style: { fontSize: '12px', opacity: 0.7 } },
    '📝 这里只管理 **用户预设** —— 在 <dshHome>/.agent-presets/ 下。dsh 官方预设（cordis / minimal / ptc / standard）随仓库打包，不可编辑不可删除。',
  ))
  if (state.error !== '') {
    elements.push(createElement('div', { className: 'error', key: 'err' }, state.error))
  }

  // Default preset selector — radio cards spanning the user roster (the
  // shipped ones still render but as disabled since they're immutable,
  // except when they are already the active default).
  elements.push(createElement('div', { className: 'card', key: 'default-card' },
    createElement('div', { className: 'card-header', key: 'h' },
      createElement('span', { className: 'card-title', key: 't' },
        createElement('span', { style: { fontWeight: 600 } }, '🎛 默认预设'),
      ),
      createElement('span', { className: 'card-sub', key: 'cur', style: { marginLeft: '8px' } },
        state.defaultId === '' ? '当前：未设置' : '当前：' + state.defaultId,
      ),
    ),
    state.presets.length === 0
      ? createElement('div', { className: 'empty', key: 'e' }, '加载中…')
      : createElement('div', { key: 'radios', style: { display: 'flex', flexWrap: 'wrap', gap: '6px', padding: '8px 0' } },
          state.presets.map(function (p) {
            var isActive = state.defaultId === p.id
            var shipped = p.trust === 'shipped'
            return createElement('label', {
              key: 'r-' + p.id,
              title: shipped ? 'dsh 默认预设，可选作默认' : '',
              style: {
                display: 'flex', alignItems: 'center', gap: '6px',
                padding: '4px 10px',
                border: '1px solid var(--dsw-alias-border-subtle, rgba(200,200,210,200,200,0.4))',
                borderRadius: '999px',
                cursor: 'pointer',
                background: isActive ? 'rgba(120,120,200,0.1)' : 'transparent',
              },
            },
              createElement('input', {
                type: 'radio',
                name: 'agent-presets-default',
                value: p.id,
                checked: isActive,
                disabled: state.busy,
                onChange: function () { setDefault(p.id) },
                key: 'r',
              }),
              createElement('span', { key: 'name' }, p.name || p.id),
              shipped ? createElement('span', { key: 'b', className: 'tag', style: { fontSize: '10px' } }, '🛡 默认') : null,
            )
          }),
        ),
  ))
  ...
  // Preset list
  elements.push(createElement('div', { className: 'section-header', key: 'list-h',
    style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px' } },
    createElement('span', { style: { fontWeight: 600 }, key: 't' }, '📚 预设列表'),
    createElement('button', { type: 'button', className: 'btn primary sm', key: 'new',
      onClick: startCreate, disabled: state.busy,
    }, '➕ 新建预设'),
  ))

  elements.push(createElement('div', { key: 'list' },
    state.presets.map(function (p) {
      var isActive = p.isDefault === true
      var shipped = p.trust === 'shipped'
      var broken = p.broken === true
      var confirming = state.confirmingDeleteId === p.id
      return createElement('div', { className: 'card', key: 'p-' + p.id,
        style: { borderColor: isActive ? 'var(--dsw-alias-border-strong, rgba(120,120,200,0.6))' : undefined } },
        createElement('div', { className: 'card-header', key: 'h' },
          createElement('span', { className: 'card-title', key: 't' },
            createElement('span', { style: { fontWeight: 600 }, key: 'n' },
              p.name + createElement('span', { key: 'path', style: { opacity: 0.55, fontWeight: 400, fontSize: '11px', marginLeft: '6px' } }, ' (' + p.id + ')'),
            ),
            isActive ? createElement('span', { className: 'tag plugin', key: 'a', style: { marginLeft: '6px' } }, '🟢 当前默认') : null,
            shipped ? createElement('span', { className: 'tag', key: 'b', style: { marginLeft: '4px' }, title: 'dsh 默认预设，不可编辑' }, '🛡 dsh 默认') : null,
            broken ? createElement('span', { className: 'tag', key: 'k', style: { marginLeft: '4px' }, title: '目录缺失' }, '⚠ 目录缺失') : null,
          ),
          createElement('span', { className: 'card-actions', key: 'a' },
            p.editable && !broken ? createElement('button', {
              type: 'button', className: 'btn sm', key: 'e',
              disabled: state.busy || confirming,
              onClick: function () { startEdit(p) },
            }, '✎ 编辑') : null,
            p.editable && !broken ? (confirming
              ? createElement('span', { key: 'conf', style: { display: 'flex', gap: '4px' } },
                  createElement('button', {
                    type: 'button', className: 'btn sm danger', key: 'yes',
                    disabled: state.busy, onClick: function () { runDelete(p.id) },
                  }, '确认删除'),
                  createElement('button', {
                    type: 'button', className: 'btn sm', key: 'no',
                    disabled: state.busy, onClick: function () { patch({ confirmingDeleteId: '' }) },
                  }, '取消'),
                )
              : createElement('button', {
                type: 'button', className: 'btn sm danger', key: 'd',
                disabled: state.busy, onClick: function () { confirmDelete(p.id) },
                title: '删除用户预设目录',
              }, '🗑 删除')) : null,
          ),
        ),
        p.description !== ''
          ? createElement('div', { className: 'card-sub', key: 'd' }, p.description)
          : null,
      )
    }),
  ))

  // Edit form (modal-ish card at the bottom, replaces inline until the
  // host gives the plugin a real modal portal).
  if (state.editing !== null) {
    var editing = state.editing
    var isNew = editing.kind === 'new'
    var draftField = function (key, label, value, onChange, mono) {
      return createElement('label', { key: 'f-' + key, className: 'card-sub', style: { display: 'block', marginTop: '6px' } },
        createElement('span', { key: 'l', style: { display: 'block', fontWeight: 600, fontSize: '12px', opacity: 0.75 } }, label),
        createElement('input', {
          type: 'text',
          key: 'i',
          value: value,
          onChange: function (e) { onChange(e.target.value) },
          style: { width: '100%', fontFamily: mono === true ? 'monospace' : 'inherit' },
        }),
      )
    }
    var fields = []
    fields.push(createElement('div', { key: 'h', style: { fontWeight: 600, marginBottom: '6px' } },
      isNew ? '➕ 新建预设' : '✎ 编辑 ' + editing.id))
    if (isNew) {
      fields.push(draftField('id', 'preset id（kebab-case，例如 my-team）', editing.id,
        function (v) { startEditSetField('id', v) }, true))
    } else {
      fields.push(createElement('div', { key: 'id-readonly', className: 'card-sub' },
        'preset id：' + editing.id + createElement('span', { key: 'r', style: { opacity: 0.6, fontSize: '11px', marginLeft: '6px' } }, '（不可修改，目录名是 id）')))
    }
    fields.push(draftField('name', '名称（preset.yml 的 name）', editing.metadata.name || '',
      function (v) { startEditSetField('name', v) }, false))
    fields.push(draftField('description', '描述（preset.yml 的 description）', editing.metadata.description || '',
      function (v) { startEditSetField('description', v) }, false))
    fields.push(draftField('order', '排序（order，数字；自定义预设默认 0）', String(editing.metadata.order ?? 0),
      function (v) { startEditSetField('order', v) }, false))
    fields.push(createElement('label', { key: 'comp', style: { display: 'block', marginTop: '6px' } },
      createElement('span', { key: 'l', style: { display: 'block', fontWeight: 600, fontSize: '12px', opacity: 0.75, marginBottom: '2px' } },
        'composition（agent.cordis.yml —— 至少一条 `- name: <plugin>` 行；保留原始 !!js / 注释）',
      ),
      createElement('textarea', {
        key: 'ta',
        value: editing.compositionText,
        onChange: function (e) { startEditSetComposition(e.target.value) },
        rows: 18,
        style: {
          width: '100%',
    ...
    fields.push(createElement('div', { key: 'actions', style: { display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '8px' } },
      createElement('button', { type: 'button', className: 'btn', key: 'cancel',
        disabled: state.busy, onClick: cancelEdit,
      }, '取消'),
      createElement('button', { type: 'button', className: 'btn primary', key: 'save',
        disabled: state.busy, onClick: saveDraft,
      }, isNew ? '创建' : '保存'),
    ))
    elements.push(createElement('div', { className: 'card', key: 'edit-card',
      style: { marginTop: '12px', background: 'var(--dsw-alias-bg-elevated, rgba(245,245,250,0.5))' } },
      fields,
    ))
  }

  if (state.busy && state.presets.length === 0) {
    elements.push(createElement('div', { className: 'busy-banner', key: 'b' },
      createElement('span', { className: 'spinner' }), '加载中…'))
  }
  return createElement('div', { 'data-dsh-admin-section': '' }, elements)
}
...
/**
 * Codex hooks bridge settings section (v1.16.0).
 *
 * One-click install / uninstall of `@deepseek-ai/dsh-hooks-codex`, the
 * Codex-format sibling of the Claude-Code-format `hooks/bridgeInstall`
 * already exposed by the 命令与钩子 section. The two bridges compose
 * side-by-side (distinct row ids `hooks-claude-code` vs `hooks-codex`)
 * and each gets its own hooks.json file (Codex-format by default at
 * `<dshHome>/hooks.codex.json`, distinct from `<dshHome>/hooks.json`).
 *
 * v1 scope: install + remove of the bridge package + cordis row. The
 * panel does NOT yet ship a Codex-format hook editor — users hand-edit
 * `hooks.codex.json` for now. The bridge consumes any well-formed Codex
 * hooks config.
 *
 * Like the existing Claude-Code bridge, the panel needs to read the
 * `commandHookAdmin/codexBridgeInstall` / `codexBridgeRemove` results to
 * surface post-install state. The result shape includes `codexHooksPath`
 * so the panel can show the user where to author Codex-format hooks.
 */
function HooksCodexBridgeSection(props) {
  var call = props.call
  var kit = sectionState({
    installed: false,
    mounted: false,
  ...
  function reload() {
    patch({ busy: true, error: '' })
    // `listHooks()` reports the Codex bridge's own state — `codexBridgeInstalled`,
    // `codexBridgeMounted` and `codexHooksPath` (command-hook-admin.js) — so the
    // section mirrors the host instead of guessing from the install button.
    call('commandHookAdmin/listHooks', {}).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({
          busy: false,
          installed: v.codexBridgeInstalled === true,
          mounted: v.codexBridgeMounted === true,
          codexHooksPath: typeof v.codexHooksPath === 'string'
            ? v.codexHooksPath
            : '',
        })
      } else {
  ...
  function install() {
    patch({ busy: true, error: '' })
    call('commandHookAdmin/codexBridgeInstall', {}).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        var v = result.value || {}
        patch({
          ...
          codexHooksPath: typeof v.codexHooksPath === 'string'
            ? v.codexHooksPath
            : state.codexHooksPath,
          output: typeof v.output === 'string' ? v.output : state.output,
        })
        reload()
      } else {
        patch({ busy: false, error: '安装失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busy: false, error: '安装失败：' + messageOf(err) })
  ...
  function runRemove() {
    patch({ busy: true, error: '', confirming: false })
    call('commandHookAdmin/codexBridgeRemove', {}).then(function (result) {
      if (!alive.current) return
      if (result.ok) {
        var v = result.value || {}
        patch({
        ...
        patch({ busy: false, error: '卸载失败：' + messageOf(result.error) })
      }
    }, function (err) {
      if (!alive.current) return
      patch({ busy: false, error: '卸载失败：' + messageOf(err) })
    })
  }
  ...
  elements.push(createElement('div', { className: 'hint', key: 'restart-banner',
    ...
    '🔁 Codex 钩子桥安装 / 卸载 后需**重启 dsh** —— patch 重排在启动期生效',
  ))
  if (state.error !== '') {
    elements.push(createElement('div', { className: 'error', key: 'err' }, state.error))
  }

  var status = state.installed && state.mounted
    ? '✅ 已挂载'
    : state.installed
      ? '📦 已装未挂载'
      : '⚪ 未安装'
  var cardChildren = [
    createElement('div', { className: 'card-header', key: 'h' },
      createElement('span', { className: 'card-title', key: 't' },
        createElement('span', { style: { fontWeight: 600 }, key: 'n' }, '🪝 Codex 钩子桥'),
        createElement('span', { className: 'tag plugin', key: 's', style: { marginLeft: '6px' } }, status),
      ),
      createElement('span', { className: 'card-actions', key: 'a' },
        !state.installed
          ? createElement('button', {
            type: 'button', className: 'btn sm primary', key: 'install',
            disabled: state.busy, onClick: install,
            title: 'pnpm add @deepseek-ai/dsh-hooks-codex 并追加 hooks-codex 行',
          }, state.busy ? '安装中…' : '📥 安装')
          : (state.confirming
            ? createElement('span', { key: 'conf', style: { display: 'flex', gap: '4px' } },
                createElement('button', {
                  type: 'button', className: 'btn sm danger', key: 'yes',
                  disabled: state.busy, onClick: runRemove,
                }, '确认卸载'),
                createElement('button', {
                  type: 'button', className: 'btn sm', key: 'no',
                  disabled: state.busy, onClick: function () { patch({ confirming: false }) },
                }, '取消'),
              )
            : createElement('button', {
              type: 'button', className: 'btn sm danger', key: 'remove',
              disabled: state.busy, onClick: function () { patch({ confirming: true }) },
              title: '卸载 hooks-codex 包与 patch 行（Claude Code 桥不受影响）',
            }, '🗑 卸载')),
      ),
    ),
    createElement('div', { className: 'card-sub', key: 'pkg', style: { fontFamily: 'monospace', fontSize: '11px' } },
      '@deepseek-ai/dsh-hooks-codex  ·  row id = hooks-codex'),
    createElement('div', { className: 'card-sub', key: 'h2', style: { marginTop: '4px' } },
      '📝 Codex 钩子文件：' +
      (state.codexHooksPath !== '' ? state.codexHooksPath : '安装后默认落在 <dshHome>/hooks.codex.json'),
    ),
    createElement('div', { className: 'card-sub', key: 'h3', style: { fontSize: '11px', opacity: 0.75, marginTop: '4px' } },
      '💡 与 Claude-Code 格式不同的 5 个 hook 点（SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop）。该面板 v1 不提供 Codex 格式的钩子编辑器 —— 用户自己手工编辑 hooks.codex.json；桥只消费 Codex 格式配置文件。'),
  ]
  elements.push(createElement('div', { className: 'card', key: 'codex-bridge' }, cardChildren))
  return createElement('div', { 'data-dsh-admin-section': '' }, elements)
}
...
function apply(ctx) {
  ...
  // Workspace admin (order 22 — between Agent Presets and MCP). The web-app
  // bundle mounts `@deepseek-ai/dsh-workspace`; CLI / headless deployments
  // resolve to { available: false } and the panel renders a quiet hint.
  ctx.slots.inject('settings.section', function () {
    return ctx.slots.register({
      name: 'settings.section',
      id: 'workspaces',
  ...
  // Skills administration (order 23 — adjacent to workspaces). Reads one
  ...
  // Storage backend swap (order 31). JSON ↔ SQLite picker + session format
  // version panel. The shipped `json` backend is the default; `sqlite`
  // is opt-in and the panel handles the install + cordis-row append.
  ctx.slots.inject('settings.section', function () {
    return ctx.slots.register({
      name: 'settings.section',
      id: 'storage-admin',
  ...
  // Agent preset editor (order 32). Author/edit/delete USER Agent
  // presets under `<dshHome>/.agent-presets/`. Shipped presets are
  // read-only. Labelled '预设编辑', NOT 'Agent 预设': the official
  // ui-agent-preset page already registers that label, and the nav-icon
  // injection can only key off the label text — a shared label would
  // repaint the official row's own icon.
  ctx.slots.inject('settings.section', function () {
    return ctx.slots.register({
      name: 'settings.section',
      id: 'agent-presets-admin',
      order: 33,
      label: '预设编辑',
      inject: function () { return { call: call } },
    }, AgentPresetsEditorSection)
  })
  // Codex hooks bridge (order 33). One-click install of
  // @deepseek-ai/dsh-hooks-codex + its cordis row. v1 scope is install/
  // remove only — the Codex-format hook file itself is hand-edited.
  ctx.slots.inject('settings.section', function () {
    return ctx.slots.register({
      name: 'settings.section',
      id: 'hooks-codex-bridge',
      order: 34,
      label: '🪝 Codex 钩子桥',
      inject: function () { return { call: call } },
    }, HooksCodexBridgeSection)
  })
  ctx.slots.inject('settings.section', function () {
    return ctx.slots.register({
      name: 'settings.section',
