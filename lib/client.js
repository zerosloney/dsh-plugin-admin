...
function setupMenuInjection(call, refreshSessions) {
  ...
  function injectItems(menuEl) {
    ...
    if (isSession) {
      ...
      // Resolve the session by title lazily at click time (fresh RPC) so
      // the match is always current. Shared by copy-id and delete.
      function resolveSession(cb) {
        call('sessionAdmin/list', {}).then(function (listResult) {
          var sessions = (listResult && listResult.ok && listResult.value && listResult.value.sessions) || []
          var match = null
          // Normalize a title for fuzzy comparison: trim, collapse spaces,
          // drop trailing ellipsis and truncation artifacts.
          function norm(v) {
      ...
      appendMenuItem(viewport, '复制会话 ID', 'normal', function () {
        resolveSession(function (session) {
          copyTextToClipboard(session.id)
        })
      })

      // Delete the session (never live).
      appendMenuItem(viewport, '删除会话', 'danger', function () {
        resolveSession(function (match) {
          if (match.live) { showToast('error', '❌ 删除会话失败：会话正在进行中'); return }
          call('sessionAdmin/deleteSession', { sessionId: match.id }).then(function (result) {
            if (result && result.ok) {
  ...
  function appendMenuItem(viewport, label, kind, onClick) {
  ...
  function actSession(method, sessionId) {
    ...
    callRemote('sessionAdmin/' + method, { sessionId: sessionId }).then(function (result) {
      ...
      if (result.ok) {
        ...
        if (method === 'deleteSession' && props.refreshSessions) props.refreshSessions()
...
function McpSection(props) {
  ...
  return createElement('div', { 'data-dsh-admin-section': '' },
    renderMcpSection(mView, patchDraft, reloadMcp, openMcpEditor, closeMcpEditor, saveMcpDraft, removeMcpEntry))
}
...
function renderMcpSection(view, patchDraft, reloadMcp, openMcpEditor, closeMcpEditor, saveMcpDraft, removeMcpEntry) {
  var children = []
  ...
  for (var i = 0; i < view.mcpEntries.length; i++) {
    ...
    rows.push(createElement('div', { className: 'card', key: 'mcp-' + entry.id },
      createElement('div', { className: 'card-header', key: 'h' },
        ...
        createElement('div', { className: 'card-actions', key: 'a' },
          createElement('button', {
            type: 'button',
            className: 'btn sm',
            disabled: view.mcpBusy || entry.config === null || entry.config === undefined,
  ...
  if (view.mcpEditorOpen && view.mcpDraft !== null) {
    ...
    var editor = createElement('div', { className: 'card', key: 'mcp-editor' },
      createElement('div', { className: 'card-header', key: 'h' },
        createElement('span', { className: 'card-title-text', key: 't' }, d.id !== '' ? '编辑 ' + d.id : '添加 MCP 服务器'),
      ),
 ...
 * that is both live and in the archived set is shown as 进行中); otherwise
...
function renderSessionCard(session, view, act, patch) {
  ...
  var dotTitle = session.live ? '进行中' : session.archived ? '已归档' : '已结束'
  ...
  var titleChildren = [
    ...
    createElement('div', { className: 'card-title', key: 'title' },
      ...
      session.live ? createElement('span', { className: 'tag live', key: 'tag-live' }, '进行中')
  ...
  if (isConfirming) {
    cardChildren.push(createElement('div', { className: 'confirm-bar', key: 'confirm' },
      createElement('span', { className: 'confirm-text', key: 'text' }, '⚠️ 确定永久删除该会话记录及日志文件？'),
      createElement('div', { className: 'confirm-actions', key: 'actions' },
        createElement('button', {
          type: 'button',
          key: 'btn-confirm',
          className: 'btn danger-solid sm',
          disabled: view.busy,
          onClick: function () { act('deleteSession', session.id) },
        }, '确认删除'),
        createElement('button', {
          type: 'button',
          key: 'btn-cancel',
