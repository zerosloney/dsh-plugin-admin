...
function SkillsSection(props) {
  ...
  var summaryText = '📌 共 ' + state.skills.length + ' 个技能'
    + (filtered.length === state.skills.length ? '' : ' · 当前匹配 ' + filtered.length + ' 个')
  if (rosterIncomplete) {
    var incompleteCause = ''
    if (failedSessions.length > 0) {
      incompleteCause = '⚠ ' + failedSessions.length + ' 个会话作用域未能解析'
    } else if (failedScopeTotal > 0) {
      incompleteCause = '⚠ ' + failedScopeTotal + ' 个作用域读取失败'
    } else if (!state.complete) {
      incompleteCause = '⚠ 部分技能源发现未完成'
    } else {
      incompleteCause = '未命中任何技能源'
    }
    summaryText = '📌 共 ' + state.skills.length + ' 个技能（全局 + ' + presetScopes.length + ' 个预设 + '
      + sessionScopes.length + ' 个会话作用域）· 当前匹配 ' + filtered.length + ' 个 — ' + incompleteCause
  }
...
function describeSchedule(mode, time, dow) {
  var names = { '0': '周日', '1': '周一', '2': '周二', '3': '周三', '4': '周四', '5': '周五', '6': '周六' }
  var parts = String(time || '09:00').split(':')
  if (mode === 'hourly') return '每小时第 ' + parts[1] + ' 分'
  if (mode === 'weekly') return '每周 ' + (names[dow] || dow) + ' ' + time
  return '每天 ' + time
}
...
function renderCronEditor(view, actions) {
  ...
  return createElement('div', { className: 'card mcp-editor', key: 'editor' },
    ...
    createElement('div', null,
      createElement('label', { style: { fontSize: '12px', fontWeight: 600 } }, '调度（本地时区 ' + tzLabel() + '）'),
      createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', marginTop: '4px' } },
        createElement('select', {
          ...
          [{ v: 'hourly', t: '每小时' }, { v: 'daily', t: '每天' }, { v: 'weekly', t: '每周' }, { v: 'custom', t: '自定义' }].map(function (o) {
        ...
        d.schedMode === 'weekly'
          ? createElement('select', {
            ...
            [['1', '周一'], ['2', '周二'], ['3', '周三'], ['4', '周四'], ['5', '周五'], ['6', '周六'], ['0', '周日']].map(function (o) {
              return createElement('option', { key: o[0], value: o[0] }, o[1])
            }),
          )
          : null,
        d.schedMode !== 'custom'
          ? createElement('span', { style: { fontSize: '12px', color: secondary } }, '于')
          : null,
        d.schedMode === 'hourly'
          ? createElement('select', {
            className: 'input', style: schedSelectStyle,
      ...
      createElement('div', {
        ...
        d.schedMode === 'custom'
          ? '5 位 cron（分 时 日 月 周）：支持 *, 逗号列表, 短横范围, 斜杠步长；周接受 0-7 与 SUN-SAT。'
          : describeSchedule(d.schedMode, d.schedTime, d.schedDow) + ' · ' + d.cron + ' · 进程重启期间到期的任务不补投。'),
    ),
    createElement('div', null,
