...
function TodoAdminRender(list, stats, collapsed, toggle, activeRef) {
  var children = []
  if (!collapsed) {
    var items = []
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      // The FIRST in-progress row carries the scroll anchor so a list longer
      // than the viewport keeps the current step visible.
      var liProps = { key: i, className: 'todo-item', 'data-status': status }
      if (status === 'in_progress' && !activeAttached && activeRef) {
        liProps.ref = activeRef
        activeAttached = true
      }
