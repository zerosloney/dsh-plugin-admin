...
function HooksCodexBridgeSection(props) {
  ...
  function reload() {
    patch({ busy: true, error: '' })
    // `listHooks()` is the only surface that exposes `bridgeInstalled` for
    // the Claude-Code bridge; the Codex bridge has no equivalent read —
    // we probe `codexBridgeRemove` would-be state by re-installing the
    // idempotency (no-op when present). Instead, derive `installed` from
    // `listHooks().codexBridgeInstalled`. listHooks does not yet expose
    // that flag; we instead probe the Cordis registry for the presence
    // of `hooks-codex` plugins via pluginInventory — but to keep the
    // contract simple, this section just renders buttons that *attempt*
    // install/uninstall; an absent bridge → pnpm add + first-mount; a
    // present bridge → uninstall removes both row + package.
    call('commandHookAdmin/listHooks', {}).then(function (result) {
      ...
      if (result.ok) {
        ...
        patch({
          ...
          // listHooks does not surface codexHooksPath yet; derive a
          // best-effort default (host recomputes it on install).
