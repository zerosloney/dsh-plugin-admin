...
var CSS_TEXT = [
  ...
  '[data-dsh-admin-section] .toolbar { display: flex; gap: 8px; align-items: center; padding: 10px 20px 10px 10px; border: 1px solid var(--dsw-alias-border-subtle, rgba(200,200,210,0.4)); border-radius: 12px; background: var(--dsw-alias-bg-elevated, var(--dsw-alias-bg-base, transparent)); box-shadow: 0 1px 2px rgba(0,0,0,0.02); }',
  ...
  '[data-dsh-admin-section] .session-search .input { height: 32px; padding: 0 12px 0 30px; font-size: 13px; border-radius: 8px; border-color: var(--dsw-alias-border-subtle, rgba(200,200,210,0.5)); }',
  ...
  '[data-dsh-admin-section] .btn.xs { height: 22px; padding: 0 8px; font-size: 11px; border-radius: 6px; gap: 4px; }',
  ...
  '[data-dsh-admin-section] .list { display: flex; flex-direction: column; gap: 10px; flex: 1 1 auto; min-height: 0; max-height: 560px; overflow-y: auto; padding: 2px 4px 2px 2px; scrollbar-width: thin; scrollbar-color: var(--dsw-alias-border-subtle, rgba(200,200,210,0.4)) transparent; scrollbar-gutter: stable; }',
  ...
  '[data-dsh-admin-section] .group-caret { flex: none; width: 12px; font-size: 10px; color: var(--dsw-alias-label-tertiary, #888); transition: transform 0.15s ease; }',
  '[data-dsh-admin-section] .group-header.collapsed { margin-bottom: 8px; }',
  '[data-dsh-admin-section] .group-collapsed-note { font-size: 11px; color: var(--dsw-alias-label-tertiary, #888); font-weight: 400; }',
...
function renderProviderConfig(view, ui) {
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    rows.push(createElement('div', { key: 'field-' + field.key, className: 'card-sub', style: { display: 'flex', flexDirection: 'column', gap: '2px' } },
      ...
      field.hint ? createElement('div', { style: { fontSize: '11px', opacity: 0.65 } }, field.hint) : null,
    ))
  }
  return createElement('div', {
    key: 'config',
    style: { borderTop: '1px solid var(--dsw-alias-border-subtle, rgba(200,200,210,0.4))', marginTop: '8px', paddingTop: '8px', display: 'flex', flexDirection: 'column', gap: '6px' },
  },
    ...
    view.note ? createElement('div', { className: 'hint', key: 'srcc', style: { fontSize: '11px', opacity: 0.7 } }, view.note) : null,
    rows.length > 0
 ...
 * skills, runtime registrations) with ONE read per distinct
 * (cwd, agent-preset) scope taken from `sessionAdmin/list` — so a skill
 * only one workspace sees still appears, with its source label, its
 * SKILL.md path, and whether the model, the human, or both can invoke it.
 *
 ...
 * exists to fix. Filters (text / source / scope) narrow the roster without
 * another host round-trip; the scope panel shows coverage and the
 * sessions whose scope could not be resolved.
 *
...
function SkillsSection(props) {
  ...
  // Filter option domains, derived from the roster itself.
  var sourceOptions = []
  var scopeOptions = []
  for (var si = 0; si < state.skills.length; si++) {
    var skillSources = Array.isArray(state.skills[si].sources) ? state.skills[si].sources : [state.skills[si].source]
    for (var sj = 0; sj < skillSources.length; sj++) {
      if (skillSources[sj] && sourceOptions.indexOf(skillSources[sj]) === -1) sourceOptions.push(skillSources[sj])
    }
    var skillScopes = Array.isArray(state.skills[si].scopes) ? state.skills[si].scopes : []
    for (var sk = 0; sk < skillScopes.length; sk++) {
      if (skillScopes[sk] && scopeOptions.indexOf(skillScopes[sk]) === -1) scopeOptions.push(skillScopes[sk])
    }
  }
  sourceOptions.sort()
  scopeOptions.sort()
  // Display shortening comes from the host (`short`), so the panel never
  // re-parses a label string; filter VALUES stay the full labels.
  ...
  var filtered = state.skills.filter(function (skill) {
    var sources = Array.isArray(skill.sources) ? skill.sources : [skill.source]
    if (state.source !== 'all' && sources.indexOf(state.source) === -1) return false
    var scopes = Array.isArray(skill.scopes) ? skill.scopes : []
    if (state.scope !== 'all' && scopes.indexOf(state.scope) === -1) return false
    if (needle === '') return true
    var haystack = [skill.name, skill.description, skill.whenToUse, skill.path, skill.url].join(' ').toLowerCase()
    return haystack.indexOf(needle) !== -1
  })

  // Toolbar: refresh + text filter + source filter + scope filter.
  var sourceSelect = [createElement('option', { key: 'all', value: 'all' }, '全部来源')]
  for (var so = 0; so < sourceOptions.length; so++) {
    sourceSelect.push(createElement('option', { key: sourceOptions[so], value: sourceOptions[so] }, skillsSourceLabel(sourceOptions[so])))
  }
  var scopeSelect = [createElement('option', { key: 'all', value: 'all' }, '全部作用域')]
  for (var po = 0; po < scopeOptions.length; po++) {
    scopeSelect.push(createElement('option', { key: scopeOptions[po], value: scopeOptions[po], title: scopeOptions[po] }, scopeShortByLabel[scopeOptions[po]] || scopeOptions[po]))
  }
  elements.push(createElement('div', { className: 'toolbar', key: 'toolbar' },
    createElement('button', { type: 'button', className: 'btn', key: 'refresh', disabled: state.busy, onClick: reload }, '⟳ 刷新'),
    createElement('div', { className: 'search-wrap', key: 'search', style: { flex: 1 } },
      createElement('span', { className: 'search-icon', key: 'icon' }, '🔎'),
      createElement('input', {
    ...
    createElement('select', {
      className: 'input', key: 'source', value: state.source, 'aria-label': '按来源过滤',
      style: { maxWidth: '190px' },
      onChange: function (e) { patch({ source: e.target.value }) },
    }, sourceSelect),
    createElement('select', {
      className: 'input', key: 'scope', value: state.scope, 'aria-label': '按作用域过滤',
      style: { maxWidth: '220px' },
      onChange: function (e) { patch({ scope: e.target.value }) },
    }, scopeSelect),
  ))

  // Roster summary + the scope coverage panel.
  var sessionScopes = state.scopes.filter(function (s) { return s.kind !== 'global' })
  ...
  elements.push(createElement('div', { className: 'hint', key: 'summary' },
    '📌 共 ' + state.skills.length + ' 个技能（全局 + ' + sessionScopes.length + ' 个会话作用域）· 当前匹配 ' + filtered.length + ' 个 — 仅枚举（不加载正文、不修改 SKILL.md）'))
    ...
    for (var pi = 0; pi < state.scopes.length; pi++) {
      ...
      scopeChildren.push(createElement('div', { key: 'scope-' + pi, className: 'card-sub', style: { fontSize: '11px' } },
        ...
        scopeRow.error !== null && scopeRow.error !== undefined && scopeRow.error !== ''
          ...
          : createElement('span', { style: { marginLeft: '8px', opacity: 0.7 } }, scopeRow.count + ' 个技能' + (scopeSessions > 0 ? ' · ' + scopeSessions + ' 个会话' : '')),
      ))
    }
  ...
  for (let ci = 0; ci < filtered.length; ci++) {
    ...
    subChildren.push(createElement('div', { key: 'scopes', className: 'card-sub', style: { fontSize: '11px', opacity: 0.75 } },
  ...
  if (state.skills.length === 0 && !state.busy && state.error === '') {
    elements.push(createElement('div', { className: 'empty', key: 'empty-skills' },
      '没有发现任何技能（当前 cwd / 会话作用域与全局目录都未命中技能源）'))
  } else if (filtered.length === 0 && state.skills.length > 0) {
    elements.push(createElement('div', { className: 'empty', key: 'empty-filter' }, '没有匹配当前筛选条件的技能'))
  }
...
function apply(ctx) {
  ...
  // Nine slot surfaces, each parked where the shell already renders its
  // domain. 插件管理 joins the shell-owned 插件 section as its third tab
  // (插件配置 0, 插件列表 10, 扩展插件 20); MCP / 技能 / Web 搜索 / 子智能体 /
  // 命令与钩子 / 用量仪表盘 / Webhook 触发 become standalone settings pages
  // (MCP服务器 takes the 25 slot right after Agent 预设, 子智能体 follows at 26,
  // 命令与钩子 at 27), and the 待办清单 dock rides the composer.
