...
function Picker(props) {
  ...
  function emit(next) { props.onChange(next) }
        ...
        filtered.length === 0
          ...
          : filtered.map(function (o, idx) {
              return createElement('div', {
                ...
                onMouseDown: function (ev) { ev.preventDefault(); pick(o) },
...
function SubagentsPanel(props) {
  ...
  var openCreate = function () {
    setNotice(null)
    setForm({ draft: emptyDraft(), editing: false, saving: false, error: null })
  }
  var openEdit = function (entry) {
    setNotice(null)
    setForm({ draft: draftFromEntry(entry), editing: true, saving: false, error: null })
  }
    ...
    createElement('div', { className: 'form-grid' },
      ...
      createElement('div', { className: 'field' },
        ...
        createElement('select', {
          ...
          onChange: function (event) { onPatch({ provider: event.target.value }) },
        },
          meta.providers.map(function (provider) {
      ...
      createElement('div', { className: 'field' },
        ...
        createElement('select', {
          ...
          createElement('option', { value: 'continuable' }, 'continuable（可持续会话）')
        ),
      ...
      createElement('div', { className: 'field full' },
        ...
        createElement('textarea', {
          className: 'input', value: draft.persona, rows: 4, disabled: form.saving,
          placeholder: '该子智能体的人设/职责说明…支持 {{model}} 与 {{cwd}} 模板变量',
          onChange: function (event) { onPatch({ persona: event.target.value }) },
        }),
        createElement('span', { className: 'field-hint' }, '保存后影子覆盖（shadow）部署级 persona，仅对该子智能体生效')
      ),
      createElement('div', { className: 'fieldset full' },
        createElement('span', { className: 'fieldset-legend' }, '工具约束（toolFilter，留空则不限制；可选项来自 harness 已注册工具与内置工具名录，也可手填未知名）'),
        createElement('div', { className: 'field' },
          createElement('span', { className: 'field-label' }, '仅允许（allow 白名单）'),
          createElement(Picker, {
            multi: true, kind: 'allow', allowCustom: true,
        ...
        createElement('div', { className: 'field' },
          ...
          createElement(Picker, {
            multi: true, kind: 'deny', allowCustom: true,
          ...
          createElement('span', { className: 'field-hint' }, '名单内工具对子智能体不可见；未知工具名保存时会被拒绝')
        )
      ),
      createElement('div', { className: 'fieldset full' },
          ...
          createElement('div', { className: 'field' },
            ...
            createElement('div', { style: { display: 'flex', gap: '8px', alignItems: 'center' } },
              createElement('input', {
                ...
                disabled: form.saving || draft.maxDepthManaged,
              ...
              createElement('label', { className: 'checkbox-row', style: { flex: 'none' } },
                createElement('input', {
                  type: 'checkbox', checked: draft.maxDepthManaged, disabled: form.saving,
    ...
    createElement('div', { className: 'card-header' },
      ...
      createElement('span', { className: 'cli-tags' },
        ...
        createElement('span', { className: 'tag ' + (cli.ok ? 'model' : 'warn') },
          cli.ok ? 'PATH ' + cli.version : 'PATH ✗ ' + backend.cliCommand),
      ),
  ...
  return createElement('div', { className: 'card', key: backend.id },
    createElement('div', { className: 'card-header' },
      ...
      createElement('span', { className: 'cli-tags' },
        ...
        createElement('span', { className: 'tag ' + (cli.ok ? 'model' : 'warn') },
          cli.ok ? 'PATH ' + cli.version : 'PATH ✗ ' + backend.command),
      ),
      createElement('span', { style: { display: 'flex', gap: '5px', flex: 'none' } },
        createElement('button', { type: 'button', className: 'btn sm primary', disabled: busy, onClick: onSave },
          busy ? createElement(Spinner, { key: 'spin' }) : null, '保存配置'),
          ...
          .map(function (item) {
            return createElement('div', { className: 'cli-scan-card', key: item.name },
              ...
              createElement('span', { className: 'cli-tags' },
                ...
                createElement('span', { className: 'tag live' }, 'PATH ' + item.cli.version),
              ),
              createElement('button', {
...
function apply(ctx) {
  ...
  // Four surfaces, each parked where the shell already renders its domain.
  // 插件管理 joins the shell-owned 插件 section as its third tab (插件配置 0,
  // 插件列表 10, 扩展插件 20); MCP / 子智能体 / 历史会话 become standalone
  // settings pages (MCP服务器 takes the 25 slot right after Agent 预设,
  // 子智能体 follows at 26, 历史会话 sorts last). inject() waits on each
