...
import { ensureProfileDependency, harnessLockstepVersion, profileDirOf, topLevelBlocks, yamlScalar } from './patch-utils.js'
...
import { applyCredentialAdmin, credentialInvocations } from './credential-admin.js'
 ...
 * ignoring build metadata (+sha) so `1.0.0+sha` == `1.0.0`.
      ...
      if (message && message.id !== undefined && pending.has(message.id)) {
        ...
        if (method === 'tools/call') {
          finish({
            ok: true,
            serverInfo: child.stdoutInfo || undefined,
            toolCount: typeof child.toolCount === 'number' ? child.toolCount : undefined,
    ...
    service[methodName] = function (...args) {
      ...
      const capture = (handle) => {
        if (handle !== null && typeof handle === 'object'
          ...
          // LRU eviction: drop the oldest entry when the cap is reached so a
          // long-running host cannot leak handles for sessions that ended
          // through the normal loop dispose path.
...
export function apply(ctx, config) {
  ...
  const cfg = config ?? {}
  const PNPM_TIMEOUT_MS = cfg.pnpmTimeoutMs ?? 5 * 60_000
  const UPDATE_CHECK_TIMEOUT_MS = cfg.updateCheckTimeoutMs ?? 8_000
  ...
  const MCP_PROBE_TIMEOUT_MS = cfg.mcpProbeTimeoutMs ?? 10_000
  const MCP_PROBE_HTTP_TIMEOUT_MS = cfg.mcpProbeHttpTimeoutMs ?? 8_000
  const MCP_PROBE_MAX_RESPONSE_BYTES = cfg.mcpProbeMaxResponseBytes ?? 256 * 1024
   ...
   * (cheapest, always freshest), else the persistence replay. Shared by the
   * read paths that want the whole log (export).
  ...
  const sessionService = {
    ...
    async healthReport(sessionId) {
      ...
      const headers = await ctx.sessionPersistence.list()
      if (!headers.some((h) => h && h.id === sessionId)) {
        throw new Error(`session-admin: session '${sessionId}' does not exist`)
      }
      const events = await sessionEventsFor(sessionId)
  ...
  /**
   * Locate the top-level entry blocks in the patch file. Delegates to the
   * shared topLevelBlocks from patch-utils.js.
   */
  /**
