 ...
 *
 * 8. Namespace `skillsAdmin` (see lib/skills-admin.js): the merged, read-only
 ...
 *    the requested sessions (`ctx.sessionQuery.observeSession` +
 *    `agentPresets.standingKeyFor`, without activating a cold Agent). Each
 *    entry carries name / description / whenToUse / source / provider /
 ...
 *
 * 10. Namespace `commandHookAdmin.codexBridgeInstall` /
 ...
 ctx.logger?.warn?.('plugin-admin: workspace registry lacks unarchiveSession() — 取消归档与删除时的归档清理将不可用（dsh 版本过旧）')
  ...
  function foldUsageEvent(session, event) {
    ...
    if (event.type === 'assistant/message') {
      ...
      // checkpoint replaces shadowed history instead of adding a turn.
      const source = event.data?.source
      if (source !== null && typeof source === 'object' && source.kind === 'plugin' && source.plugin === 'compact') return
      row.messageCount += 1
 ...
 * Whether a block is an MCP client entry: its lines contain a `name:` row
  ...
  ctx.effect(() => ctx.typert.register({
    ...
    }), 'plugin-admin: typert descriptors (plugins, sessions, fs, mcp, subagents, commands+hooks, webhooks, overlays, inventory, workspaces, skills, web-search)')
}
