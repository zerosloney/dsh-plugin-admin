...
import { applyCommandHookAdmin, ensureProfileDependency, harnessLockstepVersion } from './command-hook-admin.js'
...
// Git runs on every dock poll tick; the 3s TTL collapses concurrent/rapid
// polls into one subprocess pair per session.
const GIT_STATS_CACHE_TTL_MS = 3_000
...
// derived-summary cache above.
const gitStatsCache = new Map()

/**
 * Run one git command in `cwd`, returning stdout on success or null on any
 * failure (not a repo, no git binary, timeout) — the dock degrades to an
 * empty stats segment instead of surfacing errors.
 */
function runGit(cwd, args) {
  const res = spawnSync('git', args, { cwd, timeout: GIT_TIMEOUT_MS, windowsHide: true, encoding: 'utf8' })
  return res.status === 0 && typeof res.stdout === 'string' ? res.stdout : null
}

/**
 * Parse `git status --porcelain=v1 -z` output into change entries. The -z
 * format is NUL-separated `XY <path>` records; renames carry the ORIGINAL
 * path as an extra NUL field after the new one. The status letter is the
 * most user-meaningful of the two index/worktree columns: untracked (`??`)
 * wins, then A (added), D (deleted), R (renamed), default M (modified).
...
export function parseGitStatusZ(out) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    else if (x === 'R' || y === 'R' || x === 'C' || y === 'C') status = 'R'
 ...
 * @param statusOut - `git status --porcelain=v1 -z` stdout, or null.
 * @param numstatOut - `git diff --numstat HEAD` stdout, or null.
 * @param countLines - optional (path) => positive line count | null for
 *   untracked entries.
 * @returns {{ files: number, added: number, removed: number, changed: Array }}.
 */
export function gitFileStats(statusOut, numstatOut, countLines) {
  ...
  return { files: changed.length, added, removed, changed }
}
...
export function apply(ctx, config) {
  ...
  const sessionService = {
     ...
     *   changed: Array<{ path: string, status: string, added: number,
     *   removed: number }> }}.
     */
    ...
    async computeFileStats(sessionId) {
      ...
      if (cwd === '' || !existsSync(cwd)) {
        return { files: 0, added: 0, removed: 0, changed: [] }
      }
      const statusOut = runGit(cwd, ['status', '--porcelain=v1', '-z', '--untracked-files=all'])
      if (statusOut === null) return { files: 0, added: 0, removed: 0, changed: [] }
      const numstatOut = runGit(cwd, ['diff', '--numstat', 'HEAD'])
      return gitFileStats(statusOut, numstatOut, (path) => countUntrackedLines(cwd, path))
    },
  }
      ...
      if (/^- insert:/.test(lines[block.index])) {
        ...
        for (let i = 1; i <= inner.length; i++) {
          ...
          if (i === inner.length || startsEntry) {
            ...
            if (/^\s*name:\s*['"]?@deepseek-ai\/dsh-mcp-client['"]?\s*$/.test(entryLines.find(l => /^\s*name:/.test(l)) ?? '')) {
  ...
  }), 'plugin-admin: typert descriptors (plugins, sessions, fs, mcp, subagents, commands+hooks)')
}
