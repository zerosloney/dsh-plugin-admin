...
/* Exported for the host-check: the credential-ref discovery the
 * credentialAdmin mount wires in. It is a pure read over `ctx.get('settings')`,
 * so the check can drive it against a fixture schema envelope without mounting
 * the whole plugin. */
export { credentialRefsFor }
...
/** Credential-reference grammar; mirrors the credentials seam's own REF_PATTERN. */
const CREDENTIAL_REF_NAME = /^[A-Za-z_][A-Za-z0-9_]*$/

/**
 * Discover the credential references the running composition consumes, by
 * walking each settings namespace's serialized schema alongside its resolved
 * value and reading every node the schema marks `role('credential-ref')`.
 * Covers the provider packages actually mounted, so the panel only ever offers
 * refs the active model providers will resolve. Called lazily on every
 * credentialAdmin.list(), so namespaces registered after this plugin's mount
 * are still discovered without a remount.
 *
 * Never throws — it reports instead, because the three outcomes mean different
 * things to the panel and only two of them are benign:
 *   - `{ refs, reason: null }` — the scan ran;
 *   - `{ refs: [], reason: 'service-absent' }` — no settings service here
 *     (expected on a bundle without it, not a fault);
 *   - `{ refs: [], reason: 'scan-failed', message }` — `describe()` threw, which
 *     would otherwise be indistinguishable from "nothing is declared": the panel
 *     would show a confident empty roster over a real host fault.
 * @param ctx - host context (needs `settings`).
 * @returns {{ refs: string[], reason: string|null, message?: string }} scan report.
 */
function credentialRefsFor(ctx) {
  ...
  try {
    ...
    return { refs: [], reason: 'scan-failed', message: messageOfError(error) }
  }
  if (settings === undefined || settings === null || typeof settings.describe !== 'function') {
    return { refs: [], reason: 'service-absent' }
  }
  try {
    // `redactSecrets` strips every `role('secret')` value before the walk sees
    // it — this surface only ever needs the reference names beside them, so
    // the secret half must not be materialized here at all.
    const descriptors = settings.describe({ redactSecrets: true })
    for (const descriptor of Array.isArray(descriptors) ? descriptors : []) {
      collectCredentialRefs(descriptor?.schema, descriptor?.value, refs)
    }
  } catch (error) {
    // A namespace whose schema cannot serialize (a recursive schemastery
    // schema blows `toJSON()`'s stack) fails the whole describe() call, so the
    // scan has nothing to report rather than a partial roster.
    return { refs: [], reason: 'scan-failed', message: messageOfError(error) }
  }
  return { refs: [...refs], reason: null }
}

/** One-line message for a thrown value the panel can show verbatim. */
function messageOfError(error) {
  if (error !== null && typeof error === 'object' && typeof error.message === 'string') return error.message
  return String(error)
}

/** Whether a value is a plain data object the walk may index into. */
function isRecordValue(value) {
  return value !== null && typeof value === 'object' && !Array.isArray(value)
}

/**
 * Collect every credential reference one namespace declares, pairing the
 * serialized schema with the resolved value at the same position.
 *
 * Two facts about the wire form drive this walk:
 * - `schema.toJSON()` returns an envelope (`{ uid, refs }`) whose **every child
 *   slot holds a uid number** into the sibling `refs` map, never a nested node.
 *   A child must therefore be resolved through that map before it can be read;
 *   treating the numbers as inert values stops the walk at the root.
 * - The reference in force is a runtime value — `apiKeyEnv` names whichever key
 *   the profile was configured with — so a node's `meta.default` is only the
 *   fallback for a position the resolved value leaves unset.
 *
 * Cycles cannot occur in the wire form (schemastery's `toJSON` cannot serialize
 * a recursive schema at all), but the in-progress set keeps the walk total. A
 * uid may legitimately appear at several positions (one shared profile schema
 * under two dict keys), so it is deliberately not skipped globally.
 * @param schema - the descriptor's serialized schema envelope.
 * @param value - the descriptor's resolved, secret-redacted value.
 * @param refs - sink for discovered reference names.
 */
function collectCredentialRefs(schema, value, refs) {
  const envelope = isRecordValue(schema) ? schema : null
  const nodes = envelope !== null && typeof envelope.uid === 'number' && isRecordValue(envelope.refs)
    ? envelope.refs
    : null
  const onStack = new Set()

  const nodeOf = (child) => (typeof child === 'number' && nodes !== null ? nodes[child] : child)

  const adopt = (meta, nodeValue) => {
    const live = typeof nodeValue === 'string' ? nodeValue.trim() : ''
    const fallback = typeof meta.default === 'string' ? meta.default.trim() : ''
    const ref = live !== '' ? live : fallback
    if (ref !== '' && CREDENTIAL_REF_NAME.test(ref)) refs.add(ref)
  }

  const walk = (child, nodeValue) => {
    if (typeof child === 'number' && onStack.has(child)) return
    const node = nodeOf(child)
    if (!isRecordValue(node)) return
    if (isRecordValue(node.meta) && node.meta.role === 'credential-ref') {
      adopt(node.meta, nodeValue)
      return
    }
    if (typeof child === 'number') onStack.add(child)
    const container = isRecordValue(nodeValue) ? nodeValue : null
    if (node.type === 'object') {
      const properties = isRecordValue(node.dict) ? node.dict : null
      for (const key of Object.keys(properties ?? {})) walk(properties[key], container?.[key])
    } else if (node.type === 'dict') {
      if (container !== null) for (const key of Object.keys(container)) walk(node.inner, container[key])
    } else if (node.type === 'array') {
      if (Array.isArray(nodeValue)) for (const entry of nodeValue) walk(node.inner, entry)
    } else if (node.type === 'tuple') {
      const branches = Array.isArray(node.list) ? node.list : []
      branches.forEach((branch, index) => walk(branch, Array.isArray(nodeValue) ? nodeValue[index] : undefined))
    } else if (node.type === 'union' || node.type === 'intersect') {
      // Position is ambiguous across branches; try each against the same value.
      for (const branch of Array.isArray(node.list) ? node.list : []) walk(branch, nodeValue)
    } else if (node.inner !== undefined) {
      // dict/array/transform/lazy carry their element or wrapped schema here.
      walk(node.inner, nodeValue)
    }
    if (typeof child === 'number') onStack.delete(child)
  }

  walk(nodes !== null ? envelope.uid : envelope, value)
}
