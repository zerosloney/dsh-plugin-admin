 ...
 *    metadata edits.
 *
 * 11. Namespace `agentPresetsAdmin` (see lib/agent-presets-admin.js):
 *    user-authored Agent preset CRUD over
 *    `<dshHome>/.agent-presets/<id>/` (one dir per id; carries
 *    `agent.cordis.yml` + `preset.yml`). The shipped root in the
 *    harness tree is read-only and refuses edits. `list()` exposes the
 *    roster with `trust: 'user' | 'shipped'`, the current default id,
 *    and metadata fields; `getDocument(id)` returns metadata + raw
 *    composition text; `upsert(id, { metadata, compositionText })`
 *    re-validates the composition shape (every entry must carry a
 *    `name:` scalar) and writes both files atomically; `delete(id)`
 *    removes the directory; `setDefault(id)` writes a single
 *    `config.default:` key to the `agent-presets` cordis row (the same
 *    key dsh's AgentPresets service reads at runtime), preserving
 *    sibling keys + comments. Id validation locked to
 *    `/^[a-z0-9][a-z0-9-]*$/` (POSIX-safe, no traversal).
 *
 * 12. Namespace `commandHookAdmin.codexBridgeInstall` /
...
import { applyAgentPresetsAdmin, agentPresetsInvocations } from './agent-presets-admin.js'
 ...
 * Whether a block is an MCP client entry: its lines contain a `name:` row
  ...
  /* ------------------------ Agent Preset Editor ---------------------------- */
  // User-authored Agent preset CRUD over `<dshHome>/.agent-presets/`. The
  // shipped root (`packages/preset/agent-presets/presets/`) is immutable —
  // the editor only targets the user root, refusing any id in the shipped
  // set (`cordis` / `minimal` / `ptc` / `standard`).
  applyAgentPresetsAdmin(ctx)
  ...
  ctx.effect(() => ctx.typert.register({
    ...
    invocations: [
      ...
      ...storageInvocations(),
      // agentPresetsAdmin (user Agent preset CRUD + default selection)
      ...agentPresetsInvocations(),
    ],
  }), 'plugin-admin: typert descriptors (plugins, sessions, fs, mcp, subagents, commands+hooks, webhooks, overlays, inventory, workspaces, skills, web-search, storage, agent-presets)')
}
