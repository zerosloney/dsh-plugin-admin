...
export function apply(ctx, config) {
  ...
  /**
   * One session's events: the persistence replay through the official read
   * seam (`open(id, 'read')` handle + `read(0)` + close). Session exposes no
   * public event property (its synchronous readers are deprecated — dsh note
   * 2026-09-09), so the former live-memory fast path never fired and is gone;
   * the trade is that an online session's unflushed tail appears in exports
   * and health reports only after the backend flushes.
  ...
  async function sessionEventsFor(sessionId) {
    if (ctx.sessionPersistence === undefined) return []
    let handle
    try {
      handle = await ctx.sessionPersistence.open(sessionId, 'read')
      const events = readEventsOf(await handle.read(0))
  ...
  async function sessionEventsBoundedFor(sessionId, cap) {
    ...
    try {
      ...
      // Same stale-entry clearing contract as sessionEventsFor above.
      sessionReadErrors.delete(sessionId)
      return events
    } catch (error) {
      sessionReadErrors.set(sessionId, error instanceof Error && error.message ? error.message : String(error))
  ...
  const sessionService = {
    ...
    async searchSessions(query) {
      ...
      // the cwd basename.
      const page = await sq.searchSessions({ query: query.trim(), limit: SESSION_SEARCH_LIMIT })
      const workspaces = ctx.workspaceRegistry.list()
      const sessionToWorkspace = new Map()
      for (const ws of workspaces) {
        for (const sid of ws.sessionIds) {
