...
dsh（DeepSeek Harness）Web UI 管理插件：在官方设置界面内补齐 dsh 缺失的管理能力——**扩展插件**、**技能**、**MCP 服务器**、**子智能体**、**命令与钩子**、**Webhook 触发**、**Web 搜索**、**用量仪表盘**、**待办清单**九个独立面板，并把**历史会话**与**工作区**两块管理面板注入 dsh 官方的**已归档会话**页（不再单独占侧边栏入口）。零 dsh 导入，全部骑运行时 Cordis Context；写回统一原子写 + 串行队列，缺服务一律降级不挂死。
...
| 🗂️ 已归档会话 | **dsh 官方页**；插件把「历史会话」+「工作区」两块面板注入其中（见下） |
...
### 🗂️ 已归档会话（官方页 + 插件注入）
「已归档会话」是 dsh **官方**设置页（列出归档会话 + 取消归档）。插件不再单独注册「历史会话」「工作区」两个入口，而是把两块面板**注入该页**——打开这一页，官方归档列表下方依次是插件的历史会话面板与工作区面板。

1. 打开：设置 → 已归档会话。
2. 官方区（页面顶部）：归档会话列表（标题 + 所属工作区 + 时间），搜索框按标题/工作区过滤，每行「取消归档」。
3. 插件区 · 历史会话：搜索框同时匹配标题/摘要/工作目录/会话 ID；状态胶囊「全部 / 在线 / 已归档 / 已结束」；卡片「📌 置顶」（localStorage 持久化）；「删除」二次确认（在线会话显示「关停并删除」，先 dispose 再删日志，免重启）；「⬇ 导出」下载 Markdown 对话稿；「🩺 体检」折叠工具调用/失败/重试报告；切换「全文搜索」检索全部会话内容（部署默认关闭时点「⚡ 一键启用」→ 重启 dsh）。
4. 插件区 · 工作区：工具栏「➕ 新建工作区」→ 原生选择器选目录或手输绝对路径（可选标题）；行内「✎」改名、「⬆/⬇」重排、「🔎 检查状态」（missing-dir 提示）；「🗑 删除」双击确认（仅删注册，目录与会话本体保留）；归档集非空时顶部「📦 取消全部归档 (N)」。
5. 注入说明：插件通过 MutationObserver 检测该官方页是否为当前激活分区，是则把两块面板挂进官方页自身的滚动容器（与插件独立页同一容器，故样式与内部滚动不变）；切走分区或关闭设置弹窗即卸载。
