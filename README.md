...
- **侧边栏右键菜单（会话 & 工作区）**：
  - **会话行右键**：在原生菜单之外新增自定义右键菜单，提供**归档会话**与**删除会话**（危险操作，置于归档之后）；删除复用 `sessionAdmin.deleteSession` 的安全语义（仅限非活跃会话，定向 detach）。
...
- **Host 端（`lib/index.js`，零 dsh 依赖）**：
  ...
  - 提供并注册四个 RPC 命名空间：
    ...
    - `mcpAdmin` (`list` / `upsert` / `remove`)：管理 profile 的 `cordis.patch.yml` 中的 MCP 客户端实例（`@deepseek-ai/dsh-mcp-client`）。基于行级 YAML 块编辑（零依赖）：识别顶层 `- id:` 块并仅改动 `name` 为 MCP 客户端插件的条目，`upsert` 按 id 原位替换或追加，`remove` 整块删除，写回走**原子写**；校验 id / serverName / transport / command / url，拒绝畸形输入。
- **浏览器前端（`lib/client.js`）**：
  ...
  - 新增**侧边栏右键菜单**（纯 DOM 事件委托，无需 dsh 源码改造）：capture 阶段 `contextmenu` 监听器识别会话行（`role=treeitem` + `aria-selected`）与工作区行（`aria-expanded`），以 `sessionAdmin.list` 的标题/工作区 basename 做身份匹配，渲染自定义定位菜单（会话行：归档 + 删除；工作区行：在资源管理器打开）。
...
7. 会话删除二次确认交互；
8. **侧边栏右键菜单**：合成 `contextmenu` 事件验证会话行出现「归档会话 / 删除会话」、工作区行出现「在资源管理器打开」；
