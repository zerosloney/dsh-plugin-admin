...
**npm:** [`dsh-plugin-admin`](https://www.npmjs.com/package/dsh-plugin-admin) · v1.16.0 · MIT
...
- **💾 存储后端切换（v1.16.0，设置栏独立页 order 32，host 侧 `lib/storage-admin.js`）**：dsh 提供两个 storage backend — `@deepseek-ai/dsh-storage-json`（**shipped 默认**，一文件一单元 JSON，可读 + 原子改写，base bundle 已在 `storage-domain.config.backend: json` 写死）和 `@deepseek-ai/dsh-storage-sqlite`（**opt-in**，单 .db 文件 + 文档存表行，适合大量 session 频繁读写）。**dsh 没有任何 web UI 切换 storage backend**，用户想从 JSON 迁到 SQLite，得手改 patch + 装包 + 重启，没文档没工具。本页补上：
  ...
  - 验证：`scripts/verify-storage-admin.mjs` 七个场景（list 暴露双 backend + bundled + installed + format 版本 / swap 拒绝半装状态 / install 走 pnpm + 追加 cordis row / swap 完整流程 + 保邻居 / formatVersion 只读返回 / install 写 insert 行 + swap 守卫要求已挂载形状 / install 与 swap 都骑注入的串行队列）。
- **🧩 Agent 预设编辑器（v1.16.0，设置栏独立页 order 33，导航名「预设编辑」，host 侧 `lib/agent-presets-admin.js`）**：dsh 的 `@deepseek-ai/dsh-agent-presets` 同时读两个根 —— **`packages/preset/agent-presets/presets/`** 里的四款 shipped 预设（`cordis` / `minimal` / `ptc` / `standard`，随仓库打包，编辑器**只读不写**）和 **`<dshHome>/.agent-presets/`** 里的用户预设。**dsh 没有任何 web UI 管用户预设**，手动新建只能手工 fork shipped + 改 YAML，常常踩到陌生 preset id 命名规则和 `!!js` 表达式歧义。本页补上：
  - **默认预设 radio 卡**：单选切到任意一个 preset id，**settings 层优先**（dsh 的 `AgentPresets` 解析 `settings.default ?? config.default`，本机 `settings.yaml` 里的 `agent-presets.default` 会盖住 patch 值）——挂载了 settings 服务时 `settings.mutate('agent-presets', [{op:'set', path:['default'], value}])` 写用户层，未挂载时回退到 profile patch 的 `agent-presets.config.default` 单键 mutation（保留 dsh-bundle 行里其他键）；清空（`''`）只在 settings 路径可表达（`op:'unset'`，回落到 bundle 默认），无 settings 服务时结构化拒绝而不是写一个不存在的空 preset id；重启 dsh 生效；
  - **预设卡片列表**（user 在前，shipped 在后，order → id 升序）：每张卡显示 name / description / isDefault 徽标；user 卡附 **✎ 编辑** / **🗑 删除（双重确认）**；shipped 卡挂 🛡 dsh 默认 + 全部 mutate 按钮禁用；
  - **编辑器**：上方三个字段（name / description / order），下方大 textarea 编辑 `agent.cordis.yml` 全文 —— `!!js` 表达式、注释、缩进逐字符保留；保存路径先按 dsh 的 `entryListProblem` 规则做**键级形状校验**（每条 row 自己的 `name:` 才算数，`config:` 里嵌套的 `name:` 不算；`- name: x` / 折行键 / flow map `- { name: x }` / group 行都接受）再 atomic rename-temp-file，shape 不通过则抛行级错误，绝不写半截；
  - **id 校验锁在 `^[a-z0-9][a-z0-9-]*$`**（与上游 `PRESET_ID` 一致），杜绝路径穿越 / 大小写变体；shipped id 一律拒绝 upsert + delete；
  - **优雅降级**：`ctx.agentPresets` service 仅 web-app bundle 挂载，CLI / headless 部署也能用 —— 编辑器直接走文件系统 `dsh plugin` 自己的代码路径，不依赖 dsh 内提供的 service API。
  - **安全姿态**：`agent.cordis.yml` 与 `preset.yml` 都用 atomic temp + rename；`preset.yml` 的标量按 YAML 双引号转义（`description: "note: fast"` 之类含 `": "` 的值不再把文件写成不可解析的嵌套映射）；默认选择 settings 层优先、patch 单键 mutation 兜底（保留所有兄弟键 + 注释）；delete 只 `rm -rf <dshHome>/.agent-presets/<id>/`，不动邻居；preset 路径完全不跨 typert 边界（保留 `!!js` 等敏感原样）。
  - 验证：`scripts/verify-agent-presets-admin.mjs` 八个场景（list 露出 shipped + user + editable flag + defaultId / getDocument 读 user + 拒 shipped / upsert 写入原子性 + shape 校验 + 拒 shipped / 组成形状校验接受合法写法并拒绝嵌套-only name / shipped 组成全部通过校验 / delete 移除 + 拒 shipped / setDefault 单键改写 + 邻居保留（含内联 `config: {...}` 形态）/ settings 优先路径 + preset.yml 元数据转义往返）。
- **🪝 Codex 钩子桥（v1.16.0，设置栏独立页 order 34，host 侧 `commandHookAdmin/codexBridgeInstall` + `commandHookAdmin/codexBridgeRemove`）**：dsh monorepo 还装好了第二款 stock hooks bridge `@deepseek-ai/dsh-hooks-codex` —— **Codex 格式** hook（5 个钩点：SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop；payload snake_case；regex-only matcher），与已有的 Claude-Code 桥（7 个钩点含 SubagentStart/Stop）**完全独立**：row id 不同（`hooks-codex` vs `hooks-claude-code`），包名不同（`@deepseek-ai/dsh-hooks-codex` vs `@deepseek-ai/dsh-hooks-claude-code`），hooks 文件路径不同（`<dshHome>/hooks.codex.json` vs `<dshHome>/hooks.json`）。dsh 没有任何 web UI 启 Codex 桥，用户要 hack 进 `cordis.patch.yml` + `pnpm add` 自己接。本页补上：
  - **🪝 Codex 钩子桥卡**：显示 ✅ 已挂载 / 📦 已装未挂载 / ⚪ 未安装 状态机（依安装标 + cordis row 双重判定）；**📥 安装** = `pnpm add @deepseek-ai/dsh-hooks-codex` + 追写 `- insert:` wrapper 包裹的 `hooks-codex` row（同样的 `pnpm-stub + peer-pin` 链，已在 web-search / storage admin 上验证过）；**🗑 卸载** = 双重确认后 strip row + `pnpm remove`，两步独立容错；
  - **side-by-side**：本面板只动 `hooks-codex` 的 row + 包；**绝不摸 `hooks-claude-code` 行**——Claude 桥的安装 / 卸载仍由 「命令与钩子」section 治理；两个桥可同时挂载，配 `configPath` 指向各自文件；
  ...
  - **v1 范围明示**：本面板 v1 只做 **bridge install/remove**；不提供 Codex 格式的钩子编辑器 —— `hooks.codex.json` 用户手写，桥自己 consume（与已有的 Claude-Code 钩子编辑器不在同一文件路径上）；
...
- **Host 端（`lib/index.js`，零 dsh 依赖）**：
  ...
  - 提供并注册**十六个 RPC 命名空间**（typert 对每个 package 名只允许一次注册，全部 invocations 并入宿主的统一描述符）：
  ...
  - v1.16.0 新增的六个：`workspaceAdmin`（`list` / `create` / `rename` / `insertBefore` / `delete` / `pickDirectory` / `status` / `unarchiveSession`，`lib/workspace-admin.js`）、`skillsAdmin`（`list`，`lib/skills-admin.js`）、`webSearchAdmin`（`list` / `active` / `setActive` / `install` / `uninstall`，`lib/web-search-admin.js`）、`storageAdmin`（`list` / `swap` / `install` / `formatVersion`，`lib/storage-admin.js`）、`agentPresetsAdmin`（`list` / `getDocument` / `upsert` / `delete` / `setDefault`，`lib/agent-presets-admin.js`）与 `pluginInventoryAdmin`（`list`，`lib/plugin-inventory-admin.js`，只读快照）；`webhookAdmin` / `credentialAdmin` / `overlayAdmin` 三个见下文各自面板小节。
- **浏览器前端（`lib/client.js`）**：
  - 注入 `slots` + `connection`，注册**十七个界面贡献**：向官方插件设置页的 `settings.plugins.tab` 注册「扩展插件」页签（id `extensions`，order 20，排在「插件配置」0 与「插件列表」10 之后）；向设置弹窗注册**十三个 `settings.section`**——`workspaces`（工作区, order 22）、`skills-admin`（技能, order 23）、`mcp-servers`（MCP服务器, order 25，紧跟「Agent 预设」）、`subagent-admin`（子智能体, order 26，样式以 `data-dsh-sa-section` 作用域隔离）、`command-hook-admin`（命令与钩子, order 27，样式以 `data-cha-section` 作用域隔离）、`usage-dashboard`（用量仪表盘, order 28）、`credential-admin`（凭据管理, order 29）、`webhook-triggers`（Webhook 触发, order 30）、`web-search-admin`（Web 搜索, order 31）、`storage-admin`（存储, order 32）、`agent-presets-admin`（预设编辑, order 33）、`hooks-codex-bridge`（🪝 Codex 钩子桥, order 34）与 `session-history`（历史会话, order 100）；向聊天骨架注册 `conversation.input.dock` 待办清单（id `todo-admin`，order 5）与日程 dock（id `schedule-admin`，order 6，样式以 `data-dsh-admin-todo` / `data-dsh-admin-schedule` 作用域隔离）；并**收割空 slot** `conversation.input.right`（id `schedule-bell-admin`，order 0，样式以 `data-dsh-admin-schedule-bell` 隔离）；面板随官方页签/分区按需挂载；
...
```sh
npm test                      # 全链 21 个脚本（与 package.json 的 test 一致）
...
node scripts/verify-agent-presets-admin.mjs
...
2. 十七个 slot 注册（`settings.plugins.tab`「扩展插件」+ **`settings.section` ×13**：「工作区」/「技能」/「MCP服务器」/「子智能体」/「命令与钩子」/「用量仪表盘」/「凭据管理」/「Webhook 触发」/「Web 搜索」/「存储」/「预设编辑」/「🪝 Codex 钩子桥」/「历史会话」+ `conversation.input.dock`「待办」/「日程」+ `conversation.input.right`「日程铃」），以及各面板的按需挂载（含**导航图标注入**：13 个插件设置行逐行换成专属 16×16 图标，官方行——包括同名风险最高的「Agent 预设」——原样保留）；
...
23. `verify-agent-presets-admin.mjs`：Agent 预设——shipped 只读 + 用户预设排序与 `isDefault`、`getDocument()` 拒绝 shipped、`upsert()` 原子写两文件并校验条目形状（每个条目必须带 `name:`）、`delete()` 拒删 shipped、`setDefault()` 写 `config.default` 并保留兄弟键与注释（含内联 `config: {...}` 形态）、settings 优先路径；
24. `verify-hooks-codex-bridge.mjs`：Codex 钩子桥——`codexBridgeInstall()` 幂等 pnpm add + 补 `- insert: <hooks-codex>` 行（`configPath` 指向 `<dshHome>/hooks.codex.json`）、`[]` 占位替换、legacy 裸行升级为合规 insert 形、与 Claude-Code 桥生命周期互不干扰、`codexBridgeRemove()` 在空 / 部分状态下安全；
25. `verify-project-commands.mjs` / `verify-project-hooks.mjs` / `verify-overlays.mjs` / `integration-check.mjs`：项目级 `.agents` 命令与钩子桥契约、运行时一键启用（全文检索 / 日程）的 patch 写入，以及针对真实 dsh checkout 的 51 条源码契约探针（含统一描述符的十六个命名空间）。
