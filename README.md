...
- **🔌 插件管理**：
  ...
  - **远程更新检测**：工具栏「⬆ 检查更新」按 profile 实际使用的 npm registry（`npm_config_registry` → 项目/用户 `.npmrc` → 官方源）查询每个 **registry 安装**的插件（跳过内置与本地路径安装）的 `latest` 版本，与本地版本比对——有新版时卡片显示琥珀色「⬆ 有新版本 vX.Y.Z」徽标并出现一键「⬆ 更新」按钮（`npm install <name>@latest`）；查询带超时与 5 分钟缓存，网络失败按条目提示而非报错；
