...
dsh（DeepSeek Harness）Web UI 管理插件：在官方设置界面内补齐 dsh 缺失的管理能力——**扩展插件**、**技能**、**MCP 服务器**、**子智能体**、**命令与钩子**、**Webhook 触发**、**Web 搜索**、**用量仪表盘**、**待办清单**九个独立面板，并把**历史会话**面板注入 dsh 官方的**已归档会话**页（目录可折叠 + 批量删除，不再单独占侧边栏入口）。零 dsh 导入，全部骑运行时 Cordis Context；写回统一原子写 + 串行队列，缺服务一律降级不挂死。
