...
- **📚 技能浏览（v1.13.0，设置栏独立页 order 23，host 侧 `lib/skills-admin.js`）**：宿主 `ctx.sessionSkillCatalog`（`@deepseek-ai/dsh-api-session-controller` 的 `SessionSkillCatalog`）**不激活冷 Agent** 也能读出某一会话可见的用户可调用技能（kebab-case `name` + 可选 SKILL.md `path` + `description` + 可选 `whenToUse` + `modelInvocable` 旗标），但 dsh 没有任何 web 面板能看 — composer 弹的 `/<name>` 候选列表只展当前 sessionAgent 看到的几条，没法做技能盘点。本页补上：
  ...
  - 验证：`scripts/verify-skills-admin.mjs` 五个场景（CLI 降级 / 全卡片渲染 + 模型 vs 人类徽标 / 剪贴板写入 + 瞬时确认 / host 拒绝内联化 / 切换会话重抓）。
- **🔍 Web 搜索 provider 切换（v1.14.0，设置栏独立页 order 30，host 侧 `lib/web-search-admin.js`）**：dsh monorepo 装好了三个可选 web-search provider (`@deepseek-ai/dsh-web-search-deepseek` / `-exa` / `-perplexity`)，但 base / web-app bundle **都没在 cordis.patch.yml 里挂 exa 和 perplexity 的 row**，唯一挂的 `web` 行 `config.searchProvider: deepseek-official` 把选择权硬写死。**dsh 没有任何 web UI 切换 provider**，用户想用 Exa 的学术搜索或 Perplexity 的实时搜索，要么手改 patch、要么根本不知道可以换。本页补上：
  ...
  - 验证：`scripts/verify-web-search-admin.mjs` 六个场景（list 暴露三 provider + bundled 旗标 / active 直读 web 行 / setActive 单键改写保邻居 / install 走 pnpm + 追加 row / uninstall 拒绝默认 + strip + pnpm remove / 末态 patch 完整性）。
- **💾 存储后端切换（v1.15.0，设置栏独立页 order 31，host 侧 `lib/storage-admin.js`）**：dsh 提供两个 storage backend — `@deepseek-ai/dsh-storage-json`（**shipped 默认**，一文件一单元 JSON，可读 + 原子改写，base bundle 已在 `storage-domain.config.backend: json` 写死）和 `@deepseek-ai/dsh-storage-sqlite`（**opt-in**，单 .db 文件 + 文档存表行，适合大量 session 频繁读写）。**dsh 没有任何 web UI 切换 storage backend**，用户想从 JSON 迁到 SQLite，得手改 patch + 装包 + 重启，没文档没工具。本页补上：
  ...
  - 验证：`scripts/verify-storage-admin.mjs` 五个场景（list 暴露双 backend + bundled + installed + format 版本 / swap 拒绝半装状态 / install 走 pnpm + 追加 cordis row / swap 完整流程 + 保邻居 / formatVersion 只读返回）。
- **🧩 Agent 预设编辑器（v1.16.0，设置栏独立页 order 32，host 侧 `lib/agent-presets-admin.js`）**：dsh 的 `@deepseek-ai/dsh-agent-presets` 同时读两个根 —— **`packages/preset/agent-presets/presets/`** 里的四款 shipped 预设（`cordis` / `minimal` / `ptc` / `standard`，随仓库打包，编辑器**只读不写**）和 **`<dshHome>/.agent-presets/`** 里的用户预设。**dsh 没有任何 web UI 管用户预设**，手动新建只能手工 fork shipped + 改 YAML，常常踩到陌生 preset id 命名规则和 `!!js` 表达式歧义。本页补上：
  ...
  - 验证：`scripts/verify-agent-presets-admin.mjs` 五个场景（list 露出 shipped + user + editable flag + defaultId / getDocument 读 user + 拒 shipped / upsert 写入原子性 + shape 校验 + 拒 shipped / delete 移除 + 拒 shipped / setDefault 单键改写 + 邻居保留）。
- **🪝 Codex 钩子桥（v1.16.0，设置栏独立页 order 33，host 侧 `commandHookAdmin/codexBridgeInstall` + `commandHookAdmin/codexBridgeRemove`）**：dsh monorepo 还装好了第二款 stock hooks bridge `@deepseek-ai/dsh-hooks-codex` —— **Codex 格式** hook（5 个钩点：SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop；payload snake_case；regex-only matcher），与已有的 Claude-Code 桥（7 个钩点含 SubagentStart/Stop）**完全独立**：row id 不同（`hooks-codex` vs `hooks-claude-code`），包名不同（`@deepseek-ai/dsh-hooks-codex` vs `@deepseek-ai/dsh-hooks-claude-code`），hooks 文件路径不同（`<dshHome>/hooks.codex.json` vs `<dshHome>/hooks.json`）。dsh 没有任何 web UI 启 Codex 桥，用户要 hack 进 `cordis.patch.yml` + `pnpm add` 自己接。本页补上：
  ...
  - 验证：`scripts/verify-hooks-codex-bridge.mjs` 四个场景（install pnpm-add + 写 row + `[]` 占位替换 + `configPath` 指向 `hooks.codex.json` / legacy bare 行就地升级 + 二次 install 幂等 / 两个桥的 install + remove 完全独立互不影响 / empty patch 上 remove 是 no-op 安全降级）。
...
2. 七个 slot 注册（`settings.plugins.tab`「扩展插件」+ `settings.section`「MCP服务器」/「子智能体」/「命令与钩子」/「历史会话」+ `conversation.input.dock`「待办」/「日程」+ `conversation.input.right`「日程铃」），以及各面板的按需挂载；
...
本插件刻意零 dsh 导入，全部骑在运行时的 Cordis Context 上——其中一部分是 dsh 的公开服务面（`ctx.commands` / `ctx.shell` / `ctx.agents` / `ctx.sessionPersistence` 的 list/stat/open、`ctx.credentials` 等），另有一部分依赖 **dsh/cordis 的内部实现细节**。这些接缝在依赖的 dsh 版本上都有挂载期 fail-loud 形状探测或运行时降级，不会静默坏掉，但升级 dsh 时应关注以下五处最深耦合：
