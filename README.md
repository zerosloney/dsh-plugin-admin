...
- **🧩 Agent 预设编辑器（v1.16.0，设置栏独立页 order 33，host 侧 `lib/agent-presets-admin.js`）**：dsh 的 `@deepseek-ai/dsh-agent-presets` 同时读两个根 —— **`packages/preset/agent-presets/presets/`** 里的四款 shipped 预设（`cordis` / `minimal` / `ptc` / `standard`，随仓库打包，编辑器**只读不写**）和 **`<dshHome>/.agent-presets/`** 里的用户预设。**dsh 没有任何 web UI 管用户预设**，手动新建只能手工 fork shipped + 改 YAML，常常踩到陌生 preset id 命名规则和 `!!js` 表达式歧义。本页补上：
...
- **浏览器前端（`lib/client.js`）**：
  ...
  - **设置导航图标身份（v0.9.1）**：shell 的设置导航对未知 section id 一律画同一个通用齿轮（`navIcon()` 只硬编码四个官方 id），本插件四个设置页因此全同脸。slot 契约没有 icon 字段，插件沿用 DOM 注入姿势——MutationObserver 监听设置弹窗，按导航行 label 定位（hash 化的 css class 不参与匹配），把齿轮替换为四枚**语义化 16×16 线性图标**（MCP服务器=服务器机架、子智能体=分叉组织树、命令与钩子=终端提示符、历史会话=时钟回转），统一 currentColor + 1.3 描边 + 圆角端点，与官方 Outline16 同一视觉语言；替换件继承原 svg 的 css class，几何与对齐直接骑在 shell 自己的样式表上；只在弹窗 nav 列内匹配，幂等（`data-dsh-admin-nav-icon` 标记），官方行绝不触碰。
...
2. 十七个 slot 注册（`settings.plugins.tab`「扩展插件」+ **`settings.section` ×13**：「工作区」/「技能」/「MCP服务器」/「子智能体」/「命令与钩子」/「用量仪表盘」/「凭据管理」/「Webhook 触发」/「Web 搜索」/「存储」/「Agent 预设」/「🪝 Codex 钩子桥」/「历史会话」+ `conversation.input.dock`「待办」/「日程」+ `conversation.input.right`「日程铃」），以及各面板的按需挂载；
